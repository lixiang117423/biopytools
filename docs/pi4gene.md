...
步骤1: 按分组提取序列 → 01_mafft/{group}.fasta
    │
    ▼
步骤2: MAFFT 多序列比对 → 01_mafft/{group}_aligned.fasta
    │
    ▼
...
│   └── {group}_aligned.fasta         # 各分组 MAFFT 比对结果
└── 99_logs/
...
- **`_aligned.fasta`**：可直接用别的软件（如建树）继续分析，是可靠的中间产物
...
**Q2：换序列/分组重跑，结果没变？**
断点续传按 `01_mafft/{group}.fasta`（提取）和 `{group}_aligned.fasta`（比对）是否存在判断。改了输入后需删除 `01_mafft/` 和 `pi_results.tsv`，否则会复用旧结果。
