...
- 内置模拟数据检测（逐样品判定，模拟样品质量阈值自动降为 0，避免全丢且不污染真实样品）
...
2. 查找样本配对(双末端/单末端)
  |
  v
3. 逐样品检测模拟数据(质量值近乎全同的样品 -> 仅该样品质量阈值降为 0)
  |
  v
4. 逐样本处理:
...
两者通过 PATH 查找，并自动检测所在 conda 环境包装调用（`<同源conda绝对路径> run -p <环境绝对前缀> --no-capture-output`，见规范 §13——严禁裸调 `conda`，作业环境 PATH 上的 conda 可能是另一套安装，按环境名解析不到环境）；也支持用 `--fastp-path`/`--seqkit-path` 指定路径。
...
这通常是模拟数据（如 wgsim 输出，质量值全为 `!`）。程序会**逐样品**自动检测（前 100 条 reads 质量字符种类 ≤3 判为模拟），只把**被判定的那几个样品**的质量阈值降为 0，其余样品保持 `-q` 原值——混着模拟材料和真实样品的批次不会互相污染；日志里会逐个列出「模拟样品」名单。
...
**Q7：部分样品失败了，程序为什么直接以失败结束？流程还能续跑吗？**
只要有样品失败，程序就以**退出码 1** 结束（成功的样品输出和总结报告照常生成）。这是刻意行为：退出码 0 会让上游流程（如 `fastq2vcf_gtx`）误判质控成功并写下检查点，掩盖失败直到比对步骤才暴露。修复问题后直接重跑即可——已成功的样品因 clean 文件存在会自动跳过，只补失败的那些。
