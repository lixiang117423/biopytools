...
**Q7：1.71.2 之前的 Smudgeplot 结果可信吗？如何自查？**
不可信，需要删旧产物重跑。1.71.2 修复了一个隐蔽 bug：FastK 的 `-t` 本义是「只把计数 ≥N 的 k-mer 写进表」（**不是线程**，线程是 `-T`），旧版误把线程数传给了 `-t`，于是 k-mer 表里只剩计数 ≥ 线程数（如 88）的高覆盖 k-mer——杂合 k-mer（计数通常只有单倍体覆盖度一半）被整表滤掉，Smudgeplot 分箱全 0 或只剩高拷贝信号，但作业照常成功、图照常生成，肉眼完全看不出异常。自查旧结果：看 `*_kmerpairs.smu` 文件里 coverage 列的最小值，若恰好等于当时的线程数（88、64、12 等）即中招。GenomeScope 部分不受影响（`.hist` 直方图不经过 `-t`），基因组大小/杂合率等指标仍然有效。重跑方法：删除 `fastk/` 目录与 `03_smudgeplot/` 下对应样品的产物后重新运行即可（断点续传会自动跳过未删的旧步骤）。
