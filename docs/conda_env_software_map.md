...
5. 调用方式: 统一用 build_conda_command() 生成(同源 conda 绝对路径 + run -p <环境前缀>, 回退 -n), 必带 --no-capture-output; 严禁裸调 conda
...
```bash
conda run -p <miniforge3绝对路径>/envs/align <工具> --no-capture-output
# 实际由 build_conda_command() 生成(同源conda绝对路径+-p前缀)|generated by build_conda_command()
```
