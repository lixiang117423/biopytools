...
- **复用既往寄主剔除产物**：`--clean-fastq-dir` 直接指向含 `{样品}_1/2_nohost.fq.gz`（下划线或点号风格均可）的目录即可——程序会识别"已是 nohost 产物"并跳过寄主剔除，GTX 直接读该目录；此时 `--host-genome` 可给可不给（它只控制剔除这一步）。**不要**把 nohost 文件改名为 `_clean` 来绕（既丢语义又会被误当未剔除数据）
