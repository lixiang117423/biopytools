...
# --- [A] 核心修复：激活 Conda 环境 (解决投递作业找不到 GATK 的问题) ---
# 请根据你的实际安装路径，加载 conda 配置
CONDA_PATH="/share/org/YZWL/yzwl_lixg/miniforge3"

if [ -f "${CONDA_PATH}/etc/profile.d/conda.sh" ]; then
    source "${CONDA_PATH}/etc/profile.d/conda.sh"
    # 激活包含 GATK 和 biopytools 的环境
    conda activate GATK_v.4.6.2.0
else
    echo "❌ 错误: 找不到 conda.sh，无法激活环境，请检查 CONDA_PATH"
    exit 1
fi

# 再次强制将 GATK bin 目录加入 PATH (双重保险)
export PATH="${CONDA_PATH}/envs/GATK_v.4.6.2.0/bin/:$PATH"

# 打印环境信息以供调试
echo ">>> [环境检查]"
echo ">>> 当前 GATK 路径: $(which gatk)"
echo ">>> GATK 版本:"
gatk --version | head -n 1

# --- [B] 项目参数 (只需修改这里) ---
SPECIES="hancai"       # 结果文件前缀 (例如: hancai, rice, soybean)
THREADS=12             # 通用线程数
MAPPING_THREADS=20     # 比对步骤线程数
BCF_THREADS=40         # bcftools 压缩线程数
...
# ================= 4. Mapping & HaplotypeCaller (BWA+GATK) =================
echo ""
echo "========================================================"
echo "[1/6] Mapping & GVCF 生成..."
echo "========================================================"

# 调用外部脚本，传入绝对路径
# 注意：如果该脚本不支持 -t 参数，请忽略该参数或修改脚本
bash "${MAPPING_SCRIPT}" \
    -i "${CLEAN_DATA_DIR}" \
    -r "${REF_GENOME}" \
    -o "${OUT_MAPPING}" 
...
# ================= 5. GATK Joint Calling (联合变异检测) =================
echo ""
echo "========================================================"
echo "[2/6] GATK Joint Calling (联合变异检测)..."
echo "========================================================"

# 【重要】清理旧目录内容，防止 GATK 生成嵌套路径 (03.gatk_joint/03.gatk_joint)
if [ -d "${OUT_JOINT}" ]; then
    echo "   -> 清理旧的 Joint Call 输出目录，防止路径冲突..."
    rm -rf "${OUT_JOINT:?}"/*
fi
...
if [ -f "${INPUT_SNP_VCF}" ]; then
    ...
    echo "❌ 错误: 未找到过滤后的 SNP 文件，无法进行 MAF 筛选。"
    exit 1
fi
