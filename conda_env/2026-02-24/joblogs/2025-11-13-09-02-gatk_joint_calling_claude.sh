...
# 🐍 Python环境检查和修复
# ========================================
# GATK需要python命令，检查并创建软链接
if ! command -v python &> /dev/null; then
    echo "⚠️  检测到系统没有'python'命令，正在检查python3..."
    if command -v python3 &> /dev/null; then
        echo "✅ 找到python3，创建临时python软链接..."
        # 创建临时bin目录
        TEMP_BIN="${HOME}/.local/bin"
        mkdir -p "${TEMP_BIN}"
        ln -sf "$(which python3)" "${TEMP_BIN}/python"
        export PATH="${TEMP_BIN}:${PATH}"
        echo "✅ Python环境已配置: $(which python)"
    else
        echo "❌ 错误: 系统中既没有python也没有python3！"
        echo "请先安装Python: sudo yum install python3 或 sudo apt install python3"
        exit 1
    fi
else
    echo "✅ Python环境检查通过: $(which python)"
fi
