...
# 4. 输出文件名
OUT_FILE="${OUT_DIR}/all_samples_bcftools_joint_called.vcf.gz"
...
if [ "$CHECK_VCF_INTEGRITY" = true ]; then
    ...
    while IFS= read -r vcf_file && [ $CHECK_COUNT -lt 10 ]; do
        ...
        if bcftools view -h "$vcf_file" | grep -q "##FORMAT=.*<ID=QS"; then
            ...
            # 检查实际数据中是否有 QS
            if bcftools query -f '[%QS\n]' "$vcf_file" 2>/dev/null | grep -q "[0-9]"; then
                ((HAS_QS_DATA++))
            fi
        fi
    done < "$LIST_FILE"
    ...
    if [ $HAS_QS_HEADER -gt 0 ]; then
        ...
        if [ $HAS_QS_DATA -lt $HAS_QS_HEADER ]; then
            log "WARN" "但部分文件的数据中缺少 QS 值(GATK gVCF 的常见问题)"
            log "WARN" "这会导致 bcftools merge 失败: 'The QS annotation not present'"
            
            if [ "$AUTO_REMOVE_QS" = true ]; then
                log "INFO" "将自动移除 QS 及其他问题标签后再进行合并"
                STRIP_PROBLEMATIC_TAGS=true
            else
                log "ERROR" "请设置 AUTO_REMOVE_QS=true 以自动移除 QS 标签"
                error_exit "VCF 文件存在 QS 标签不一致问题"
            fi
        fi
    fi
...
if [ "$STRIP_PROBLEMATIC_TAGS" = true ]; then
    log "INFO" "将移除有问题的标签: QS, RGQ, MIN_DP 等"
fi
...
# 构建 bcftools annotate 参数(移除有问题的标签)
ANNOTATE_REMOVE=""
if [ "$STRIP_PROBLEMATIC_TAGS" = true ]; then
    # QS: Quality Score - GATK gVCF 中常在非变异块缺失
    # RGQ: Reference Genotype Quality - 同样可能缺失
    # MIN_DP: Minimum Depth - 在某些位点可能缺失
    ANNOTATE_REMOVE="-x FORMAT/QS,FORMAT/RGQ,FORMAT/MIN_DP"
    log "DEBUG" "移除标签参数: $ANNOTATE_REMOVE"
fi
...
if [ "$USE_NAIVE_MERGE" = true ]; then
    ...
    if [ "$STRIP_PROBLEMATIC_TAGS" = true ]; then
        # 方案: 先移除问题标签,再合并
        log "INFO" "使用管道: annotate (移除QS等) -> merge"
        ...
        if [ $EXIT_CODE -ne 0 ]; then
            rm -f "$TEMP_OUT"
            error_exit "bcftools annotate 失败" $EXIT_CODE
        fi
    else
        # 直接合并
        bcftools merge \
    ...
    if [ "$STRIP_PROBLEMATIC_TAGS" = true ]; then
        # 管道: annotate (移除QS) -> merge -> call
        log "INFO" "使用管道: merge -> annotate (移除QS等) -> call"
