...
if [ "$STRIP_PROBLEMATIC_TAGS" = true ]; then
    ...
    if command -v parallel &> /dev/null; then
        ...
        cat "$LIST_FILE" | parallel -j "$PARALLEL_JOBS" --bar \
            "bcftools annotate -x FORMAT/QS,FORMAT/RGQ,FORMAT/MIN_DP \
            -O z -o $PROCESSED_DIR/{/} {} && \
            bcftools index -t $PROCESSED_DIR/{/} && \
            echo $PROCESSED_DIR/{/}" >> "$PROCESSED_LIST" 2>&1 | tee -a "$LOG_FILE"
        
        PROCESS_FAILED=${PIPESTATUS[0]}
    else
        ...
        while IFS= read -r vcf_file; do
            ...
            if [ $((PROCESS_COUNT % 50)) -eq 0 ] || [ $PROCESS_COUNT -eq 1 ]; then
                log "INFO" "处理进度: $PROCESS_COUNT/$FILE_COUNT"
            fi
            ...
            if bcftools annotate \
                ...
                -O z \
                -o "$output_file" \
                "$vcf_file" 2>&1 | tee -a "$LOG_FILE"; then
                
                # 创建索引
                bcftools index -t "$output_file" 2>&1 | tee -a "$LOG_FILE"
    ...
    if [ $PROCESS_FAILED -gt 0 ]; then
        error_exit "有 $PROCESS_FAILED 个文件预处理失败"
    fi
    ...
    if [ "$PROCESSED_COUNT" -ne "$FILE_COUNT" ]; then
        error_exit "预处理后文件数量不匹配: 期望 $FILE_COUNT, 实际 $PROCESSED_COUNT"
    fi
