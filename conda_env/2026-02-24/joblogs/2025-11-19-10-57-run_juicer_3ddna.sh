...
TOP_DIR="/share/org/YZWL/yzwl_lixg/project/06.longliuxing_BSA/68.三代数据组装和注释/03.juicer_3ddna"
GENOME_ID="OV53_1"
...
# --- 定义所有必需文件的绝对路径 ---
GENOME_FA_FILE="${TOP_DIR}/OV53_1.primary.fa"
...
if [ "${RUN_3D_DNA}" = true ] && [ -f "${TOP_DIR}/aligned/$(basename ${GENOME_FA_FILE} .fasta).rawchrom.assembly" ]; then
    echo "  4️⃣  3D-DNA 组装结果: ${TOP_DIR}/aligned/$(basename ${GENOME_FA_FILE} .fasta).FINAL.fasta"
    echo "  5️⃣  3D-DNA Hi-C 图谱: ${TOP_DIR}/aligned/$(basename ${GENOME_FA_FILE} .fasta).FINAL.hic"
fi
