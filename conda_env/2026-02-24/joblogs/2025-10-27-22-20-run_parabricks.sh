...
ANALYSIS_BASE="${PROJECT_BASE}/03.7个样品之间的相似性"

# --- 2. 输入/输出目录 ---
# 原始 FastQ 数据所在的目录
FASTQ_INPUT_DIR="${ANALYSIS_BASE}/01.data/fastq" # 假设FastQ原始数据在这里
...
REF_GENOME_FASTA="${REF_GENOME_DIR}/Erysimum_cheiranthoides_chromosomes.fna"

# --- 4. 工具参数 ---
THREADS_MAPPING=64 # 测序比对使用的线程数
THREADS_FILTER=88  # 变异过滤使用的线程数
...
# --- 步骤 0.5: 准备参考基因组索引 ---
echo "📝 步骤 0.5: 准备参考基因组索引..."

# 检查 FastA 文件是否存在
if [ ! -f "${REF_GENOME_FASTA}" ]; then
    echo "❌ 错误: 参考基因组文件 '${REF_GENOME_FASTA}' 不存在！请检查路径或文件是否已放置在此目录。"
    exit 1
fi

# 检查 BWA 索引 (.bwt, .sa, .amb, .ann, .pac)
if [ ! -f "${REF_GENOME_FASTA}.bwt" ]; then
    echo "🔎 创建 BWA 索引: ${REF_GENOME_FASTA}.bwt 等..."
...
if [ ! -f "${REF_GENOME_FASTA}.fai" ]; then
    echo "🔎 创建 SAMtools 索引: ${REF_GENOME_FASTA}.fai..."
    samtools faidx "${REF_GENOME_FASTA}"
else
    echo "👍 SAMtools 索引 (.fai) 已存在，跳过创建。"
fi

# 检查 GATK 序列字典 (.dict)
REF_GENOME_DICT="${REF_GENOME_FASTA%.fna}.dict" # .fna -> .dict
if [ ! -f "${REF_GENOME_DICT}" ]; then
    echo "🔎 创建 GATK 序列字典: ${REF_GENOME_DICT}..."
    gatk CreateSequenceDictionary -R "${REF_GENOME_FASTA}" -O "${REF_GENOME_DICT}"
else
    echo "👍 GATK 序列字典 (.dict) 已存在，跳过创建。"
fi
echo "✅ 参考基因组索引准备完成。"


# --- 步骤 1: 测序数据比对 (Mapping) ---
echo "📊 步骤 1: 运行测序数据比对 (biopytools parabricks)..."
...
echo "🔍 步骤 2: 运行联合变异检测 (biopytools gatk-joint)..."
# 注意: parabricks的vcf目录通常是 Mapping_DIR 下的子目录，请根据实际输出调整
...
echo "🧹 步骤 3: 运行变异过滤 (biopytools filter-snp-indel)..."
# 定义输入VCF文件的完整路径
...
echo "====================================================="
echo "🎉 流程执行成功！所有步骤均已完成！"
echo "📂 最终过滤后的VCF文件可以在这里找到: ${FILTER_DIR}"
echo "====================================================="