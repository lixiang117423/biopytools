...
log_msg() {
    ...
    shift 3
    local msg="$*"
    echo -e "${color}${icon} [$(date '+%Y-%m-%d %H:%M:%S')] ${level}:${NC} ${msg}"
}
...
load_config() {
    # 路径设置
    CONFIG[WORK_DIR]="/share/org/YZWL/yzwl_lixg/project/06.longliuxing_BSA/68.三代数据组装和注释/17.YaHS_hap/hap1/"
    CONFIG[REF_FA]="${CONFIG[WORK_DIR]}/OV53_1.hap1.primary.fa"
    ...
    CONFIG[THREADS]=${THREADS:-$((cpu_cores - 2))}
    CONFIG[JAVA_RAM]=${JAVA_RAM:-"$((total_mem_gb * 6 / 10))G"}
    CONFIG[SORT_RAM]=${SORT_RAM:-"$((total_mem_gb * 2 / 10))G"}
    CONFIG[SAM_MEM]="4G"
...
print_config() {
    echo ""
    log_info "========== 流程配置信息 =========="
    log_info "工作目录: ${CONFIG[WORK_DIR]}"
    log_info "参考基因组: ${CONFIG[REF_FA]}"
    log_info "Hi-C R1: ${CONFIG[R1_FQ]}"
    log_info "Hi-C R2: ${CONFIG[R2_FQ]}"
    log_info ""
    log_info "线程数: ${CONFIG[THREADS]}"
    log_info "Java内存: ${CONFIG[JAVA_RAM]}"
    log_info "Sort内存: ${CONFIG[SORT_RAM]}"
    log_info "限制性酶: ${CONFIG[ENZYME_SEQ]}"
    log_info "最小长度: ${CONFIG[MIN_LEN]} bp"
    log_info "最小MAPQ: ${CONFIG[MIN_MAPQ]}"
    log_info "===================================="
    echo ""
}
...
step_jbat() {
    ...
    local jbat_assembly="results/out_JBAT.assembly"
    
    # 检查两个关键文件是否都已存在
    if check_file "${jbat_hic}" 100000 && check_file "${jbat_assembly}"; then
    ...
    # 生成 JBAT 文本和 assembly 文件
    log_info "[1/4] 生成 JBAT 文本格式和 assembly 文件..."
    ...
    if ! check_file "${jbat_assembly}"; then
        log_error "JBAT assembly 文件生成失败"
        log_error "请检查 logs/out_JBAT.log"
        return 1
    fi
    log_success "JBAT 文本和 assembly 文件生成完成"
    ...
    if check_file "results/out_JBAT.hic.part"; then
        ...
        log_success "JBAT .hic 文件生成成功"
        log_info "JBAT 文件输出:"
        log_info "  - ${jbat_hic} (用于 Juicebox 可视化)"
        log_info "  - ${jbat_assembly} (用于 JBAT 手动纠错)"
...
step_assessment() {
    ...
    echo ""
    log_info "========== 组装质量统计报告 =========="
    log_info "Scaffold 总数: ${n_scaffolds}"
    log_info "总长度: $(printf "%'d" ${total_length}) bp"
    log_info "最长 Scaffold: $(printf "%'d" ${max_scaffold}) bp"
    
    while IFS=$'\t' read -r metric value; do
        log_info "${metric}: $(printf "%'d" ${value}) bp"
...
main() {
    echo ""
    log_info "=========================================="
    log_info "  YaHS 高速染色体挂载流程 - 优化版 v4.0"
    log_info "=========================================="
    echo ""
    ...
    echo ""
    log_info "主要输出文件:"
    log_info "  • results/yahs_out_scaffolds_final.fa"
    log_info "  • results/yahs_out_final.hic"
    log_info "  • results/out_JBAT.hic"
    log_info "  • results/out_JBAT.assembly"
    log_info "  • results/assembly_metrics.txt"
    echo ""
