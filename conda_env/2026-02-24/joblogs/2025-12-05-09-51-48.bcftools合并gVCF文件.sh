...
CALL_OUTPUT_ALL_SITES=true  # 是否输出所有位点(包括0/0)
...
if [ "$CHECK_VCF_INTEGRITY" = true ]; then
    ...
    while IFS= read -r vcf_file && [ $CHECK_COUNT -lt 10 ]; do
        ...
        # 检查是否有 QS 字段
        if ! bcftools view -h "$vcf_file" | grep -q "##FORMAT=.*QS"; then
            ((MISSING_QS++))
        fi
    done < "$LIST_FILE"
    ...
    if [ $MISSING_QS -gt 0 ]; then
        log "WARN" "检测到输入文件缺少 QS (Quality Sum) 注释"
        log "WARN" "这些文件可能不是标准的 gVCF 格式"
        log "WARN" "将使用简单合并模式,不进行 joint calling"
...
# 注意:bcftools call 从 gVCF 进行 joint calling 时
# 实际上不需要 -f 参数(参考基因组)
# -f 参数主要用于从 pileup 格式调用变异
# 对于已有的 gVCF merge,直接使用 -m (multiallelic caller) 即可
