...
# 📋 分组信息文件
GROUP_INFO_FILE="~/project/16.荠菜/sample_groups.txt"      # 分组信息文件路径
...
echo "📊 过滤后SNP数量: ${FILTERED_SNPS}"

# 可选：提取INDEL（仅用于统计，不用于BSA分析）
echo "📤 提取INDEL变异 (仅用于统计参考)..."
gatk --java-options "-Xmx128g" SelectVariants \
    -V ${BSA_VCF} \
    -select-type INDEL \
...
echo "📊 原始INDEL数量: ${RAW_INDELS} (仅供参考)"
...
📈 变异统计:
   - 单样品变异总数: ${EACH_VARIANTS:-"N/A"}
   - BSA原始变异数: ${BSA_VARIANTS:-"N/A"}
   - 原始SNP数量: ${RAW_SNPS}
   - 过滤后SNP数量: ${FILTERED_SNPS} (保留率: $(echo "scale=1; ${FILTERED_SNPS}*100/${RAW_SNPS}" | bc)%)
   - 原始INDEL数量: ${RAW_INDELS:-"N/A"} (仅供参考)
...
⏱️ 运行时间统计:
   ...
   - 变异过滤: ${FILTER_TIME}秒 ($(date -d@${FILTER_TIME} -u +%H:%M:%S))
   - 总运行时间: ${TOTAL_TIME}秒 ($(date -d@${TOTAL_TIME} -u +%H:%M:%S))

🎯 重要输出文件:
   - 最终BSA分析文件: ${BSA_OUTPUT_DIR}/BSA_final_SNP.vcf.gz (仅SNP)
   - 过滤后SNP文件: ${BSA_OUTPUT_DIR}/snp.filtered.recode.vcf.gz
   - 单样品联合VCF: ${EACH_OUTPUT_DIR}/joint/merged_gtx.vcf.gz
   - 详细日志目录: ${LOG_DIR}/

📋 后续分析建议:
   1. 使用 BSA_final_SNP.vcf.gz 进行BSA统计分析 (仅SNP，推荐)
   2. 可用软件：QTL-seq, BSA-seq等
   3. 进行SNP-index和Δ(SNP-index)计算
   4. 滑动窗口分析定位候选区间
   5. 注意：该文件仅包含SNP，不含INDEL，符合BSA分析标准
...
echo ""
echo "💡 下一步："
echo "   使用 BSA_final_SNP.vcf.gz 进行BSA统计分析 (仅SNP，推荐)"
echo "   推荐软件：QTL-seq, BSA-seq等"
echo "   注意：该文件仅包含过滤后的SNP，符合标准BSA分析流程"
echo ""
echo "🎊 分析完成！享受你的结果吧！ 🎊"