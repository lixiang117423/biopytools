...
WORK_DIR="/share/org/YZWL/yzwl_lixg/project/22.拟南芥hic/02.小试/hicexplorer"
cd ${WORK_DIR}
...
# 限制性酶（MboI识别序列 = GATC）
ENZYME_NAME="MboI"          # Juicer使用酶名称
RESTRICTION_SITE="GATC"     # 用于查找位点
THREADS=88

# Juicer路径（需要根据实际安装位置修改）
JUICER_DIR="/share/org/YZWL/yzwl_lixg/software/juicer"  # 修改为你的Juicer安装路径
...
echo "=========================================="
echo "样品: ${SAMPLE}"
echo "限制性酶: ${ENZYME_NAME} (${RESTRICTION_SITE})"
echo "线程数: ${THREADS}"
echo "工作目录: ${WORK_DIR}"
echo ""
date
...
# 拷贝脚本
mkdir scripts
cp -r ~/software/juicer/CPU/common/ ./scripts

# 清理旧的输出（如果存在）
echo "  清理旧数据..."
rm -rf aligned splits HIC_tmp *.hic 2>/dev/null || true

# 只创建必需的目录，aligned由Juicer自己创建
mkdir -p fastq references restriction_sites
...
SITE_FILE="restriction_sites/${GENOME_NAME}_${ENZYME_NAME}.txt"

if [ ! -f "${SITE_FILE}" ]; then
    echo "  查找 ${RESTRICTION_SITE} 位点 (${ENZYME_NAME})..."
    
    # 使用python脚本查找酶切位点（不依赖Biopython）
    python3 << PYEOF
import re

genome_file = "references/${GENOME_NAME}.fasta"
site = "${RESTRICTION_SITE}"
output_file = "${SITE_FILE}"

print(f"  扫描基因组查找 {site} 位点...")

with open(genome_file, 'r') as f, open(output_file, 'w') as out:
    chrom = ""
    seq = ""
...
zcat ../01.data/raw/F25A040009292_ARAjyxceD/ARAjyxceD_hic_test/RawData/Est-1-index-1/E251102004_L01_Est-1-index-*_1.fq.gz >> ./hic_1.fq
...
if command -v juicer.sh &> /dev/null; then
    ...
    echo ""
    echo "  或使用conda安装: conda install -c bioconda juicer"
    exit 1
fi
