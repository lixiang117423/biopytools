...
# Random seed for sampling to ensure reproducibility
SEED=100
...
# Run seqkit to perform paired-end subsampling
seqkit sample -p ${FRACTION} -s ${SEED} \
    -1 "${INPUT_DIR}/${READ1}" \
    -2 "${INPUT_DIR}/${READ2}" \
    -o "${SUBSAMPLE_DIR}/${READ1_SUB}" \
    -O "${SUBSAMPLE_DIR}/${READ2_SUB}"

# Check if subsampling was successful
if [ $? -ne 0 ]; then
    echo "[ERROR] seqkit subsampling failed. Terminating script."
    exit 1
fi
