...
# 调试信息
echo "🔍 调试: 完整文件路径: ${GROUP_INFO_FILE}"
echo "🔍 调试: 展开后的路径: $(eval echo ${GROUP_INFO_FILE})"
echo "🔍 调试: 文件是否存在: $([ -f "$(eval echo ${GROUP_INFO_FILE})" ] && echo "是" || echo "否")"
echo "🔍 调试: 文件前5行内容:"
head -5 "$(eval echo ${GROUP_INFO_FILE})"
echo "🔍 调试: 提取第2列测试:"
tail -n +2 "$(eval echo ${GROUP_INFO_FILE})" | awk '{print $2}' | head -5
echo "🔍 调试: 获取唯一分组:"
tail -n +2 "$(eval echo ${GROUP_INFO_FILE})" | awk '{print $2}' | sort | uniq
...
# 获取所有分组
# GROUPS=($(tail -n +2 "${GROUP_INFO_FILE}" | awk '{print $2}' | sort | uniq))
# 获取所有分组
echo "🔍 即将执行GROUPS赋值，当前GROUP_INFO_FILE: ${GROUP_INFO_FILE}"
GROUPS_RAW=$(tail -n +2 "${GROUP_INFO_FILE}" | awk '{print $2}' | sort | uniq)
echo "🔍 GROUPS_RAW内容: ${GROUPS_RAW}"
GROUPS=(${GROUPS_RAW})
echo "🔍 GROUPS数组内容: ${GROUPS[@]}"
