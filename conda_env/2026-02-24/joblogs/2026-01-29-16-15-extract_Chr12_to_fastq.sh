...
for bamfile in "${SOURCE_DIR}"/*.sorted.bam; do
    if [ -f "$bamfile" ]; then
        ...
        # 使用samtools提取Chr12的reads并直接转换为fastq.gz
        # view: 提取Chr12的reads
        # fastq: 转换为fastq格式
        # gzip: 压缩输出
        if samtools view -@ "$THREADS" "$bamfile" "$CHR" | \
           samtools fastq -@ "$THREADS" -n - | \
           gzip -c > "$output_file" 2>> "$LOG_FILE"; then

            # 统计reads数量和文件大小
            read_count=$(samtools view -c "$bamfile" "$CHR" 2>> "$LOG_FILE")
...
if [ $SUCCESS -eq 0 ] && [ $TOTAL -gt 0 ]; then
    ...
    echo "1. 染色体名称可能不是'Chr12',而是其他格式(如'chr12', '12', 'CHR12'等)" | tee -a "$LOG_FILE"
    echo "" | tee -a "$LOG_FILE"
