...
EACH_OUTPUT_DIR="~/project/16.荠菜/02.each.2"                # 单样品变异检测输出路径
BSA_OUTPUT_DIR="~/project/16.荠菜/03.bsa.2"                  # BSA分析输出路径
...
# 使用while循环逐行读取，避免数组赋值问题
while IFS= read -r group_name; do
    if [[ -n "$group_name" ]]; then
        GROUPS+=("$group_name")
    fi
done < <(tail -n +2 "${GROUP_INFO_FILE}" | awk '{print $2}' | sort | uniq)

echo "📊 发现分组: ${GROUPS[@]}"

# 验证是否成功读取到分组
if [ ${#GROUPS[@]} -eq 0 ]; then
    echo "❌ 错误: 未能读取到有效的分组信息"
    echo "🔍 请检查分组文件格式，确保第一行为标题行，后续行为'样本名 分组名'格式"
    exit 1
fi

# 统计每组样本数量
for group in "${GROUPS[@]}"; do
    group_count=$(awk -v grp="$group" 'NR>1 && $2==grp {count++} END {print count+0}' "${GROUP_INFO_FILE}")
    echo "📊 ${group}组样本数量: ${group_count}"
    
    # 检查是否有样本
    if [ $group_count -eq 0 ]; then
        echo "⚠️ 警告: ${group}组没有找到样本"
    fi
done
...
for group in "${GROUPS[@]}"; do
    ...
    # 获取当前组的样本列表
    unset group_samples
    declare -a group_samples
    
    while IFS= read -r sample_name; do
        if [[ -n "$sample_name" ]]; then
            group_samples+=("$sample_name")
        fi
    done < <(awk -v grp="$group" 'NR>1 && $2==grp {print $1}' "${GROUP_INFO_FILE}")
    
    echo "  📋 ${group}组样本: ${group_samples[@]}"
    
    # 检查是否有样本
    if [ ${#group_samples[@]} -eq 0 ]; then
        echo "  ⚠️ 警告: ${group}组没有样本，跳过..."
        continue
    fi
...
# 检查是否有成功合并的分组
MERGED_GROUPS=$(find ${BSA_OUTPUT_DIR} -name "*_1.clean.fq" | wc -l)
if [ $MERGED_GROUPS -eq 0 ]; then
    echo "❌ 错误: 没有成功合并任何分组"
    exit 1
fi

echo "✅ 总共成功合并了 ${MERGED_GROUPS} 个分组"
...
biopytools gtx \
    ...
    --joint-threads ${GTX_JOINT_THREADS} \
    --enable-joint \
    --read1-pattern "*_1.clean.fq" \
    --read2-pattern "*_2.clean.fq" \
...
# 检查过滤结果
if [ $FILTERED_SNPS -eq 0 ]; then
    echo "⚠️ 警告: 过滤后没有保留任何SNP，可能需要调整过滤参数"
else
    RETENTION_RATE=$(echo "scale=1; ${FILTERED_SNPS}*100/${RAW_SNPS}" | bc)
    echo "📊 SNP保留率: ${RETENTION_RATE}%"
fi
...
echo "✅ BSA分组合并分析全部完成"
echo "⏱️ BSA总耗时: ${BSA_TIME} 秒 ($(date -d@${BSA_TIME} -u +%H:%M:%S))"

# 生成BSA阶段小结
echo ""
echo "📋 BSA阶段结果小结:"
echo "   - 成功处理分组: ${GROUPS[@]}"
echo "   - 合并的分组数: ${MERGED_GROUPS}"
echo "   - BSA原始变异数: ${BSA_VARIANTS:-"N/A"}"
echo "   - BSA原始SNP数: ${RAW_SNPS}"
echo "   - BSA过滤后SNP数: ${FILTERED_SNPS}"
echo "   - BSA原始INDEL数: ${RAW_INDELS}"
echo "   - 最终BSA分析文件: BSA_final_SNP.vcf.gz"
