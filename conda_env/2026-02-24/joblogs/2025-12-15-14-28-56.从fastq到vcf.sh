...
# # 1. 核心输入路径 (必须修改)
# PROJECT_BASE="${PROJECT_BASE:-/share/org/YZWL/yzwl_lixg/tmp/liuchao_gwas/Q33}"
# RAW_FASTQ_DIR="${PROJECT_BASE}/01.data/raw"
# REF_GENOME_FA="${PROJECT_BASE}/01.data/genome/Q33.genome.fa"
...
# # 6. 高级选项
# ENABLE_CHECKPOINT="${ENABLE_CHECKPOINT:-true}"  # 启用断点续传
...
#                "${FILTER_DIR}" "${SCRIPT_DIR}" "${LOG_DIR}" \
#                "${CHECKPOINT_DIR}" "${TMP_DIR}" "${GVCF_DIR}"; do
#         safe_mkdir "${dir}"
#     done
    
#     # 检查必需命令
#     log_info "检查必需工具..."
#     local required_tools=(bwa samtools gatk biopytools bcftools tabix python3)
...
# 1. 核心输入路径 (必须修改)
PROJECT_BASE="${PROJECT_BASE:-/share/org/YZWL/yzwl_lixg/tmp/liuchao_gwas/Q33_test_parabricks}"
RAW_FASTQ_DIR="${PROJECT_BASE}/01.data/raw"
REF_GENOME_FA="${PROJECT_BASE}/01.data/genome/Q33.genome.fa"
