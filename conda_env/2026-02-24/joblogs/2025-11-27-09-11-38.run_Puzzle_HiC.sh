...
# PUZZLE_INIT_TRIANGLE=6                        # Puzzle init triangle (默认6)
# SKIP_JUICER=false                             # 如果已运行过Juicer,设为true跳过
# SKIP_FASTQ_CHECK=true                         # 完全跳过 FASTQ 验证
...
seqkit sample -j 64 -p 0.01 ../04.YaHS/fastq/OV53_1-hic_R1.fastq.gz > OV53_1-hic_R1.fastq.gz
seqkit sample -j 64 -p 0.01 ../04.YaHS/fastq/OV53_1-hic_R2.fastq.gz > OV53_1-hic_R2.fastq.gz
...
if [ "${SKIP_JUICER}" = "false" ]; then
    ...
    mkdir -p ${WORK_DIR}/{scripts,references,restriction_sites,splits,aligned}
    ...
    if [ -d "${JUICER_DIR}/scripts/common" ]; then
        ...
        log_warning "未找到 ${JUICER_DIR}/scripts/common，Juicer 可能运行失败"
    fi
