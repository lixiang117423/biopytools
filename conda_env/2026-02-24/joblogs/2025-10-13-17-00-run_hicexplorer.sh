...
#==========================================
# 步骤2：生成BED文件（关键！）
# <--- 修正点：使用HiCExplorer官方工具 findRestSite ---
#==========================================
...
if [ ! -f "${BED_FILE}" ]; then
    echo "  使用 findRestSite 生成BED文件..."
    findRestSite --fasta ${GENOME_FASTA} --searchPattern ${RESTRICTION_SITE} -o ${BED_FILE}
...
#==========================================
# 步骤4：比对reads
# <--- 修正点：samtools sort 必须使用 -n 参数按名称排序！---
#==========================================
...
if [ ! -f "mapping/${SAMPLE}_R1.bam" ]; then
    ...
    samtools view -@ 32 -bS - | \
    samtools sort -n -@ 32 -o mapping/${SAMPLE}_R1.bam - # <-- 关键修正！
    
    # 按名称排序的BAM文件不需要为hicBuildMatrix建立索引
    echo "  ✓ R1比对完成"
else
    echo "  ✓ R1已比对"
fi
...
if [ ! -f "mapping/${SAMPLE}_R2.bam" ]; then
    ...
    samtools view -@ 32 -bS - | \
    samtools sort -n -@ 32 -o mapping/${SAMPLE}_R2.bam - # <-- 关键修正！
