...
if [ -f "${BED_FILE}" ]; then
    ...
    # 检查是否有长度为0的区间
    ZERO_LEN=$(awk '$2 == $3' ${BED_FILE} | wc -l)
    if [ $ZERO_LEN -gt 0 ]; then
        echo "  ⚠ 发现 ${ZERO_LEN} 个长度为0的区间，重新生成..."
        rm -f ${BED_FILE}
    else
        echo "  ✓ BED文件格式正确"
    fi
...
genome_file = "OV53_1.primary.fasta"
restriction_site = "GATC"
output_file = "OV53_1_MboI_sites.bed"

print(f"  扫描基因组中的 {restriction_site} 位点...")
...
try:
                ...
                if line.startswith('>'):
                    ...
                    if chrom and sequence:
                        ...
                        for match in re.finditer(restriction_site, seq_upper):
                            cut_pos = match.end()
                            # 关键：区间长度必须为1，不能为0
                            out.write(f"{chrom}\t{cut_pos}\t{cut_pos + 1}\n")
                            chrom_sites += 1
                            total_sites += 1
                        
                        print(f"    {chrom}: {chrom_sites:,} 个位点")
    ...
    print(f"\n  ✓ 总共找到 {total_sites:,} 个酶切位点")
    print(f"  ✓ BED文件已生成: {output_file}")
    ...
    with open(output_file) as f:
        for i, line in enumerate(f):
            if i < 3:
                ...
                print(f"    {chrom}\t{start}\t{end}\t(长度: {end - start})")
            else:
                break
...
if [ ! -f "mapping/${SAMPLE}_R1.bam" ]; then
    ...
    samtools view -@ 32 -bS - | \
    samtools sort -@ 32 -o mapping/${SAMPLE}_R1.bam -
    
    samtools index -@ 32 mapping/${SAMPLE}_R1.bam
    echo "  ✓ R1比对完成"
else
    echo "  ✓ R1已比对"
fi
...
if [ ! -f "mapping/${SAMPLE}_R2.bam" ]; then
    ...
    samtools view -@ 32 -bS - | \
    samtools sort -@ 32 -o mapping/${SAMPLE}_R2.bam -
    
    samtools index -@ 32 mapping/${SAMPLE}_R2.bam
    echo "  ✓ R2比对完成"
else
    echo "  ✓ R2已比对"
fi
...
echo "  运行命令："
echo "  hicBuildMatrix \\"
echo "    --samFiles mapping/${SAMPLE}_R1.bam mapping/${SAMPLE}_R2.bam \\"
echo "    --outFileName matrix/${SAMPLE}_${BIN_SIZE}.h5 \\"
echo "    --QCfolder qc_results/ \\"
echo "    --restrictionCutFile ${BED_FILE} \\"
echo "    --restrictionSequence ${RESTRICTION_SITE} \\"
echo "    --danglingSequence ${DANGLING_SEQ} \\"
...
chroms = h5.root.intervals[:]['name'].decode('utf-8').split('\t')
print(chroms[0] if chroms else 'chr1')
h5.close()
