...
class Fastq2VcfGTXProcessor:
    ...
    def __init__(self, **kwargs):
        ...
        self.logger_manager = Fastq2VcfGTXLogger(log_dir, self.config.verbose)
    ...
    def step1_quality_control(self):
        ...
        # 调试信息 | Debug info
        self.logger.info(f"🔍 调试信息: skip_qc = {self.config.skip_qc}")
        self.logger.info(f"🔍 调试信息: read1_pattern_fastp = {self.config.read1_pattern_fastp}")
        self.logger.info(f"🔍 调试信息: read2_pattern_fastp = {self.config.read2_pattern_fastp}")
        self.logger.info(f"🔍 调试信息: raw_fastq_dir = {self.config.raw_fastq_dir}")
    ...
    def _force_build_gtx_index(self):
        ...
        genome_dir = os.path.join(self.config.project_base, "01.data", "genome")
        ...
        if os.path.exists(genome_file_in_project):
            self.logger.info(f"✓ 使用项目目录中的基因组文件: {genome_file_in_project}")
            target_genome_file = genome_file_in_project
        else:
            self.logger.info(f"🔍 使用原始基因组文件: {self.config.ref_genome_fa}")
        ...
        self.logger.info(f"🔧 命令: {gtx_index_cmd}")
        self.logger.info(f"🔧 基因组文件: {target_genome_file}")
        self.logger.info(f"🔧 临时目录: {tmp_dir}")
    ...
    def step2_build_genome_index(self):
        ...
        self.logger_manager.step("📊 Step 2: 构建其他索引 | Build Other Indexes")

        # GTX索引已在step1中强制构建，这里跳过 | GTX index already force-built in step1, skip here
        self.logger.info("⏭️ GTX索引已在step1中构建，跳过 | GTX index already built in step1, skipping")

        # 只构建其他必要的索引（BWA, SAMtools, GATK）| Only build other necessary indexes (BWA, SAMtools, GATK)
        return self.genome_indexer.build_other_indexes()
    ...
    def run_full_pipeline(self):
        ...
        for i, (step_func, step_name) in enumerate(steps, 1):
            ...
            # 在执行每个步骤前添加调试信息 | Add debug info before each step
            if i == 2:  # 索引构建步骤 | Index building step
                self.logger.info("🔍 准备执行索引构建步骤 | Preparing to execute index building step")
                self.logger.info(f"🔍 当前配置中的基因组路径: {self.config.ref_genome_fa} | Current genome path in config")
                self.logger.info(f"🔍 GTX映射器中的基因组路径: {self.gtx_mapper.config.ref_genome_fa} | Genome path in GTX mapper")
...
def main():
    ...
    parser.add_argument('--gtx-cmd-gen-script',
    ...
    parser.add_argument('--use-gtx-wgs', action='store_true', default=True,
                       help='🔬 使用GTX WGS进行比对+变异检测一体化 (默认启用) | Use GTX WGS for alignment+variant calling integrated (default enabled)')
    parser.add_argument('--no-gtx-wgs', dest='use_gtx_wgs', action='store_false',
                       help='⏭️ 禁用GTX WGS，使用标准比对流程 | Disable GTX WGS, use standard mapping pipeline')
    parser.add_argument('--gtx-pcr-indel-model',
                       default='CONSERVATIVE',
                       help='🛠️ GTX PCR InDel模型 | GTX PCR InDel model')
    parser.add_argument('--gtx-min-confidence', type=int, default=30,
                       help='🎯 GTX最小置信度 | GTX minimum confidence')
    parser.add_argument('--gtx-min-base-qual', type=int, default=20,
                       help='🎯 GTX最小碱基质量 | GTX minimum base quality')
    parser.add_argument('--gtx-ploidy', type=int, default=2,
