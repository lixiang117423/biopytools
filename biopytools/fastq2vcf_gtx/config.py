"""
Fastq到VCF (GTX) 配置管理模块|Fastq to VCF (GTX) Configuration Management Module
"""
...
class Fastq2VcfGTXConfig:
    """Fastq到VCF (GTX) 配置类|Fastq to VCF (GTX) Configuration Class"""
    ...
    def __post_init__(self):
        ...
        # 标准化 output_dir|Normalize output_dir
        ...
        if self.raw_fastq_dir is not None:
    ...
    def validate(self):
        ...
        if self.raw_fastq_dir is not None and not os.path.exists(self.raw_fastq_dir):
        ...
        # 检查GTX WGS参数|Check GTX WGS parameters
        valid_pcr_models = ["CONSERVATIVE", "NONE", "AGGRESSIVE"]
        if self.gtx_pcr_indel_model not in valid_pcr_models:
            errors.append(f"无效的GTX PCR InDel模型: {self.gtx_pcr_indel_model}")

        if self.gtx_min_confidence < 0:
            errors.append("GTX最小置信度不能为负数|GTX minimum confidence cannot be negative")

        if self.gtx_min_base_qual < 0:
            errors.append("GTX最小碱基质量不能为负数|GTX minimum base quality cannot be negative")

        if self.gtx_ploidy < 1:
            errors.append("GTX倍性必须大于0|GTX ploidy must be greater than 0")
