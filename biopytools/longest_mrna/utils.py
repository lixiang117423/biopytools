    ...
    def run(self, cmd: str, description: str = "", check: bool = True) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(f"🔢 返回码 | Return code: {result.returncode}")
                if result.stderr.strip():
                    self.logger.error(f"⚠️ 错误信息 | Error: {result.stderr.strip()}")
                if check:
                    raise subprocess.CalledProcessError(result.returncode, cmd, result.stdout, result.stderr)
...
class TempFileManager:
    """🗂️ 临时文件管理器 | Temporary File Manager"""
    ...
    def cleanup(self):
        ...
        for temp_file in self.temp_files:
            try:
                ...
                self.logger.warning(f"⚠️ 清理临时文件失败 | Failed to cleanup temp file {temp_file}: {e}")