...
#             # self.logger.info("步骤5: 显示总结 | Step 5: Displaying summary")
...
class LongestMRNAExtractor:
    ...
    def run_extraction(self):
        ...
        try:
            ...
            # 1️⃣ 步骤1: 计算最长转录本 | Step 1: Calculate longest transcripts
            self.logger.info("1️⃣  步骤1: 分析GFF3文件，计算最长转录本 | Step 1: Analyzing GFF3 file, calculating longest transcripts")
            longest_transcripts = self.cds_calculator.calculate_from_gff(self.config.gff3_file)
            ...
            # 2️⃣ 步骤2: 生成基因信息文件 | Step 2: Generate gene info file
            self.logger.info("2️⃣  步骤2: 生成基因信息文件 | Step 2: Generating gene info file")
            self.gene_info_generator.generate_gene_info(longest_transcripts, self.cds_calculator.gene_metadata)
            
            # 3️⃣ 步骤3: 提取蛋白质序列 | Step 3: Extract protein sequences
            self.logger.info("3️⃣  步骤3: 提取蛋白质序列 | Step 3: Extracting protein sequences")
            if not self.sequence_extractor.extract_protein_sequences(longest_transcripts):
                self.logger.error("❌ 序列提取失败 | Sequence extraction failed")
                sys.exit(1)
            
            # # 4️⃣ 步骤4: 生成统计信息 | Step 4: Generate statistics
            # self.logger.info("4️⃣  步骤4: 生成统计信息 | Step 4: Generating statistics")
            
            # # 🔄 重新解析GFF3文件以获取完整的转录本信息用于统计 | Re-parse GFF3 for complete transcript info for statistics
            # gene_transcripts = self.gff3_parser.parse()
            # stats = self.stats_calculator.calculate_statistics(gene_transcripts, longest_transcripts)
            
            # # 5️⃣ 步骤5: 显示总结 | Step 5: Display summary
            # self.logger.info("5️⃣  步骤5: 显示总结 | Step 5: Displaying summary")
            # self.summary_generator.print_summary(stats)
