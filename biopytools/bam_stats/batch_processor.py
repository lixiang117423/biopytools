...
class BAMBatchProcessor:
    ...
    def parse_flagstat(self, flagstat_output: str) -> Dict:
        """从 samtools flagstat 的输出中解析关键指标 | Parse key metrics from samtools flagstat output"""
        stats = {}
    ...
    def analyze_single_bam(self, bam_file: Path) -> Dict:
        """对单个 BAM 文件执行分析 | Analyze a single BAM file"""
        try:
            ...
            self.logger.debug(f"📊 运行 samtools flagstat | Running samtools flagstat: {bam_file.name}")
            ...
            self.logger.debug(f"📈 运行 samtools coverage | Running samtools coverage: {bam_file.name}")
            coverage_proc = subprocess.run(['samtools', 'coverage', str(bam_file)],
                                        capture_output=True, text=True, check=True)

            # 3. 运行 samtools idxstats
            self.logger.debug(f"📋 运行 samtools idxstats | Running samtools idxstats: {bam_file.name}")
    ...
    def process_bam_files(self, bam_files: List[Path], processes: int = None) -> List[Dict]:
        ...
        self.logger.info(f"📁 找到 {len(bam_files)} 个 BAM 文件 | Found {len(bam_files)} BAM files")
        ...
        try:
            with Pool(processes=processes) as pool:
                ...
                all_results = list(tqdm(results_iterator, total=len(bam_files),
                                       desc="⚙️ 处理BAM文件 | Processing BAM files"))
            ...
            all_results = []
            for bam_file in tqdm(bam_files, desc="⚙️ 串行处理BAM文件 | Serial processing BAM files"):
    ...
    def _ensure_bam_index(self, bam_file: Path):
        ...
        try:
            ...
            self.logger.error("❌ samtools 命令未找到 | samtools command not found")
            return False