...
class SequenceStatsAnalyzer:
    ...
    def get_sequence_stats(self, bam_file: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"分析序列特征|Analyzing sequence features: {Path(bam_file).name}"
        )

        stats = {}
        stats.update(self._get_read_length_distribution(bam_file))
        stats.update(self._get_gc_content_distribution(bam_file))
        stats.update(self._get_base_quality_distribution(bam_file))
        return stats
    ...
    def _get_base_quality_distribution(self, bam_file: str) -> Dict[str, Any]:
        ...
        weighted_sum = sum(q * c for q, c in quality_counts.items())
        return {
            'base_quality_distribution': dict(quality_counts),
            'average_base_quality': round(
                weighted_sum / total_bases, 2
            ) if total_bases > 0 else 0,
