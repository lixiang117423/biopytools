...
def check_dependencies(logger=None):
    ...
    # 检查 Python 库 | Check Python libraries
    missing_libs = []
    try:
        import pandas
    except ImportError:
    ...
    if missing_libs:
        logger.error(f"❌ 错误: 缺少必要的Python库。请先安装 | Error: Missing required Python libraries. Please install: {' '.join(missing_libs)}")
        logger.info(f"   运行 | Run: pip install {' '.join(missing_libs)}")
        return False

    # 检查 samtools | Check samtools
    try:
        subprocess.run(['samtools', '--version'], check=True, capture_output=True)
        logger.info("✅ samtools 已找到，准备开始分析 | samtools found, ready to start analysis...")
        return True
    except (subprocess.CalledProcessError, FileNotFoundError):
        logger.error("❌ 错误: 未找到 'samtools' 命令 | Error: 'samtools' command not found")
        logger.info("   请确保 samtools 已安装并且在您的系统 PATH 环境变量中 | Please ensure samtools is installed and in your PATH")
