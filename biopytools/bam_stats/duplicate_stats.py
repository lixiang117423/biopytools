...
class DuplicateStatsAnalyzer:
    ...
    def get_duplicate_stats(self, bam_file: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"分析重复序列|Analyzing duplicates: {Path(bam_file).name}"
        )

        marked = self._check_marked_duplicates(bam_file)
        if marked:
    ...
    def _get_flagged_duplicate_stats(self, bam_file: str) -> Dict[str, Any]:
        ...
        success_total, total_output = self.cmd_runner.run(
            cmd_total, "Count total reads"
        )

        cmd_dup = f"{self.config.samtools_path} view -c -f 1024 {bam_file}"
        success_dup, dup_output = self.cmd_runner.run(
            cmd_dup, "Count duplicate reads"
        )
        ...
        try:
            ...
            duplicate_rate = (
                duplicate_reads / total_reads * 100
    ...
    def _estimate_duplicate_rate(self, bam_file: str) -> Dict[str, Any]:
        ...
        cmd = (
            ...
            f"| head -100000"
            f"| awk '{{print $3,$4,$6}}' | sort | uniq -c"
            f"| awk '$1>1{{sum+=$1}} END{{print sum+0}}'"
        )
        success, output = self.cmd_runner.run(
            cmd, "Estimate duplicate rate"
        )

        cmd_total = (
            f"{self.config.samtools_path} view -c -F 4 {bam_file}"
        )
        success_total, total_output = self.cmd_runner.run(
            cmd_total, "Count mapped reads for estimation"
        )
        ...
        try:
            estimated_duplicates = (
                int(output.strip()) if output.strip() else 0
            )
            total_sampled = min(100000, int(total_output.strip()))
            estimated_rate = (
                estimated_duplicates / total_sampled * 100
                if total_sampled > 0 else 0
            )

            return {
                'duplicates_marked': False,
                'estimation_available': True,
                'duplicate_rate': round(estimated_rate, 2),
