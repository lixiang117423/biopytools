...
class BAMAnalyzerConfig:
    ...
    def validate(self):
        ...
        if not self.input_dir:
            raise ValueError("输入目录不能为空 | Input directory cannot be empty")
        ...
        if not input_path.exists():
            raise FileNotFoundError(f"输入目录不存在 | Input directory does not exist: {input_path}")

        if not input_path.is_dir():
            raise NotADirectoryError(f"输入路径不是目录 | Input path is not a directory: {input_path}")
...
class BAMAnalyzer:
    ...
    def find_bam_files(self):
        """查找输入目录中的BAM文件 | Find BAM files in input directory"""
    ...
    def _validate_single_bam_file(self, bam_file: Path):
        ...
        if not bam_file.suffix.lower() in ['.bam']:
            self.logger.error(f"📄❌ 文件格式错误，应为 .bam 文件 | File format error, should be .bam file: {bam_file}")
            return False
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.warning(f"⚠️ BAM文件可能不完整 | BAM file may be incomplete: {bam_file}")
                self.logger.warning(f"   {result.stderr}")
                # 不返回False，继续尝试处理 | Don't return False, try to process anyway
        except FileNotFoundError:
            self.logger.warning("⚠️ samtools quickcheck 不可用，跳过BAM文件完整性检查 | samtools quickcheck not available, skipping BAM file integrity check")
    ...
    def run_analysis(self):
        ...
        try:
            ...
            if not bam_files:
                self.logger.error(f"⚠️ 在目录 '{self.config.input_dir}' 中没有找到任何 .bam 文件 | No .bam files found in directory '{self.config.input_dir}'")
                return False
            ...
            if success:
                self.logger.info(f"🎉 报告生成完毕！结果已保存至 | Report generation completed! Results saved to: {self.config.output_file}")
                return True
            else:
                return False
