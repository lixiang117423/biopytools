...
class SequenceExtractor:
    ...
    def extract_sequences_by_id_list(self):
        ...
        try:
            ...
            if missing_ids:
                self.logger.warning(f" {len(missing_ids)} 个ID在FASTA文件中未找到|{len(missing_ids)} IDs not found in FASTA file:")
    ...
    def extract_sequences_by_pattern(self):
        ...
        try:
            ...
            if not matching_sequences:
                self.logger.warning(" 没有找到匹配的序列|No matching sequences found")
                return False
    ...
    def extract_sequences_by_length(self):
        ...
        try:
            ...
            if not matching_sequences:
                self.logger.warning(" 没有找到符合长度范围的序列|No sequences found within the specified length range")
                return False
