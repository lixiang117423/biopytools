...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['build', 'install', 'sdist', 'bdist_wheel', '--help-commands']:
        print("错误|Error: This is a sequence extraction script, not a setup script.")
        print("用法|Usage: python -m sequence_toolkit.main -h")
        return 1
    ...
    parser = argparse.ArgumentParser(
        prog=prog_name,
        description='VCF序列提取工具|VCF Sequence Extraction Tool',
    ...
    parser.add_argument('--samples',
                       help='指定样本列表|Specify samples')
    parser.add_argument('--exclude-samples',
                       help='排除样本列表|Exclude samples')
