...
#                 self.logger.warning(" 检测到LTR问题，尝试使用--force参数重试|LTR issue detected, retrying with --force")
...
#             else:
#                 self.logger.warning(f" 预期输出文件不存在|Expected output file not found: {expected_file}")
...
class EDTAProcessor:
    ...
    def _run_single_edta_analysis_with_output_check(self, genome_file: str, analysis_name: str) -> Dict:
        ...
        try:
            ...
            if critical_exists >= 2 or total_exists >= 2:
                self.logger.warning(" EDTA报错但主要输出文件存在 - 视为成功|EDTA reported errors but main outputs exist - treating as success")
            ...
            # 如果关键文件都不存在，尝试重试逻辑
                ...
                if is_ltr_error:
                    self.logger.warning(" 检测到LTR相关错误，尝试使用--force重试|Detected LTR-related error, trying --force retry")
    ...
    def _retry_with_force(self, genome_file: str, analysis_dir: Path, analysis_name: str, 
        ...
        try:
            ...
            if len(final_existing) >= 1:
                self.logger.warning(" 重试失败但检测到部分输出文件，标记为部分成功|Retry failed but some outputs detected, marking as partial success")
    ...
    def _organize_edta_outputs(self, genome_file: str, analysis_dir: Path, analysis_name: str) -> Dict[str, str]:
            ...
            if source_file.exists():
                ...
                try:
                    ...
                    try:
                        ...
                        output_files[expected_file] = str(target_file)
                        self.logger.warning(f" 文件移动失败，已复制|File move failed, copied instead: {expected_file}")
        ...
        # 如果没有找到主要输出文件，记录现有文件
        if not output_files:
            self.logger.warning(" 未找到预期输出文件，列出源目录内容|No expected outputs found, listing source directory")
            try:
