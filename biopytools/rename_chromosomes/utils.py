...
class FastaRenamer:
    ...
    def rename_sequences(
        ...
        Args:
            ...
            keep_all: 是否输出所有序列|Whether to output all sequences
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info(f"输入文件: {input_file}|Input file: {input_file}")
            self.logger.info(f"输出文件: {output_file}|Output file: {output_file}")
            self.logger.info(f"染色体数量: {chromosome_number}|Number of chromosomes: {chromosome_number}")
            self.logger.info(f"保留所有序列: {'是' if keep_all else '否'}|Keep all sequences: {'Yes' if keep_all else 'No'}")

            # 使用awk进行流式处理|Use awk for stream processing
            if keep_all:
                # 保留所有序列，前n个命名为Chr01-ChrNN，其余命名为HiC_scaffold_01-NN
                # Keep all sequences, first n as Chr01-ChrNN, rest as HiC_scaffold_01-NN
                awk_script = f'''
BEGIN {{
    chr_count = 0
    scaf_count = 0
...
'''
                ...
                # 只保留前n个序列|Keep only first n sequences
                awk_script = f'''
BEGIN {{
    seq_count = 0
}}
/^>/ {{
    seq_count++
    if (seq_count <= {chromosome_number}) {{
        printf ">Chr%02d\\n", seq_count
    }}
    next
}}
...
'''
            ...
            with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
                ...
                if result.returncode != 0:
                    self.logger.error(f"AWK处理失败: {result.stderr}|AWK processing failed: {result.stderr}")
                    return False
            ...
            self.logger.info("=" * 60)
            self.logger.info(f"输出序列数: {total_seqs}|Number of output sequences: {total_seqs}")

            if keep_all:
                scaffold_total = total_seqs - chromosome_number
                self.logger.info(f"染色体: {chromosome_number}|Chromosomes: {chromosome_number}")
                self.logger.info(f"Scaffolds: {scaffold_total}")

            return True

        except Exception as e:
            self.logger.error(f"重命名失败: {str(e)}|Renaming failed: {str(e)}", exc_info=True)
            return False
    ...
    def preview_output(self, output_file: str, lines: int = 10):
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info(f"输出文件预览 (前{lines}行)|Output file preview (first {lines} lines)")
        self.logger.info("=" * 60)
