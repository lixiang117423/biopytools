...
class TreeBuilder:
    ...
    def reroot_tree(self) -> bool:
        """
        使用nw_reroot重根化系统发育树|Reroot phylogenetic tree using nw_reroot

        在指定的外群节点上重根化|Reroot on specified outgroup nodes

        Returns:
            bool: 是否成功|Whether successful
        """
        if not self.config.outgroup_list:
            self.logger.info("未指定外群，跳过重根化|No outgroup specified, skipping rerooting")
            return True

        self.logger.info("="*80)
        self.logger.info("开始重根化系统发育树|Starting phylogenetic tree rerooting")
        self.logger.info("="*80)

        try:
            # 检查输入树文件是否存在|Check if input tree file exists
            if not os.path.exists(self.config.tree_output):
                self.logger.error(f"输入树文件不存在|Input tree file does not exist: {self.config.tree_output}")
                return False

            # 检查nw_reroot工具是否存在|Check if nw_reroot tool exists
            if not os.path.exists(self.config.nw_reroot_path):
                self.logger.error(f"nw_reroot工具不存在|nw_reroot tool does not exist: {self.config.nw_reroot_path}")
                return False

            # 构建命令|Build command
            nw_reroot_cmd = os.path.basename(self.config.nw_reroot_path)
            cmd_args = [self.config.tree_output] + self.config.outgroup_list

            # 自动包装conda环境的命令|Auto-wrap conda environment commands
            wrapped_cmd = build_conda_command(nw_reroot_cmd, cmd_args)

            self.logger.info(f"运行命令|Running command: {' '.join(wrapped_cmd)}")
            self.logger.info(f"外群样本|Outgroup samples: {', '.join(self.config.outgroup_list)}")

            # 运行nw_reroot|Run nw_reroot
            ...
            if result.returncode != 0:
                self.logger.error(f"nw_reroot执行失败|nw_reroot execution failed")
                self.logger.error(f"错误信息|Error message: {result.stderr.decode('utf-8', errors='ignore')}")
                return False

            # 保存重根化后的树|Save rerooted tree
            with open(self.config.rerooted_tree_output, 'wb') as f:
                f.write(result.stdout)

            self.logger.info(f"重根化后的系统发育树已保存|Rerooted phylogenetic tree saved: {self.config.rerooted_tree_output}")

            # 验证输出文件|Verify output file
            if not os.path.exists(self.config.rerooted_tree_output):
                self.logger.error("重根化输出文件未生成|Rerooted output file not generated")
                return False

            # 统计信息|Statistics
            file_size = os.path.getsize(self.config.rerooted_tree_output)
            self.logger.info(f"重根化树文件大小|Rerooted tree file size: {file_size} bytes")

            self.logger.info("="*80)
            self.logger.info("重根化完成|Rerooting completed successfully")
            self.logger.info("="*80)

            return True

        except Exception as e:
            self.logger.error(f"重根化失败|Rerooting failed: {e}")
            import traceback
            self.logger.error(f"详细错误信息|Detailed error: {traceback.format_exc()}")
            return False
