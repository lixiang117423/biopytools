...
class ResultsManager:
    ...
    def generate_summary_report(self):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            # 重根化树文件|Rerooted tree file
            if self.config.rerooted_tree_output and os.path.exists(self.config.rerooted_tree_output):
                size = os.path.getsize(self.config.rerooted_tree_output)
                f.write(f"  - 重根化系统发育树|Rerooted phylogenetic tree: {self.config.rerooted_tree_output} ({size} bytes)\n")
                f.write(f"    外群样本|Outgroup samples: {', '.join(self.config.outgroup_list)}\n")
            ...
            # 重根化信息|Rerooting information
            if self.config.outgroup_list:
                f.write("  - 重根化|Rerooting: nw_reroot\n")
                f.write(f"    外群样本|Outgroup samples: {', '.join(self.config.outgroup_list)}\n")
    ...
    def log_output_files(self):
        ...
        # 添加重根化树文件|Rerooted tree file
        if self.config.rerooted_tree_output:
            files_to_check.insert(3, (self.config.rerooted_tree_output, "重根化系统发育树|Rerooted phylogenetic tree"))
        ...
        # 记录外群信息|Log outgroup information
        if self.config.outgroup_list:
    ...
    def validate_results(self) -> bool:
        ...
        # 检查重根化树文件|Check rerooted tree file
        if self.config.rerooted_tree_output:
            if os.path.exists(self.config.rerooted_tree_output):
                self.logger.info("+ 重根化系统发育树文件存在|Rerooted phylogenetic tree file exists")
                # 验证重根化树文件内容|Validate rerooted tree file content
                try:
                    with open(self.config.rerooted_tree_output, 'r') as f:
                        tree_content = f.read().strip()
                        if not tree_content:
                            self.logger.warning("重根化系统发育树文件为空|Rerooted phylogenetic tree file is empty")
                        elif not tree_content.endswith(';'):
                            self.logger.warning("重根化系统发育树可能格式不正确（不以;结尾）| Rerooted tree may have incorrect format (doesn't end with ;)")
                        else:
                            self.logger.info("+ 重根化系统发育树文件格式正确|Rerooted phylogenetic tree file format is correct")
                except Exception as e:
                    self.logger.warning(f"无法验证重根化系统发育树文件|Cannot validate rerooted phylogenetic tree file: {e}")
            else:
                self.logger.warning(f"重根化系统发育树文件未生成|Rerooted phylogenetic tree file not generated: {self.config.rerooted_tree_output}")
