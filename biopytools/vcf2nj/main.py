...
class VCFPhyloAnalyzer:
    ...
    def __init__(self, **kwargs):
        ...
        self.logger_manager = PhyloLogger(self.config.output_dir_path, self.config.output_prefix)
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # 步骤4: 可选的重根化|Step 4: Optional rerooting
            if self.config.outgroup_list:
                self.logger.info("-" * 40)
                self.logger.info("步骤4: 重根化系统发育树|Step 4: Reroot phylogenetic tree")
                self.logger.info("-" * 40)

                if not self.tree_builder.reroot_tree():
                    self.logger.warning("重根化失败|Rerooting failed")
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例|Examples: %(prog)s -i wild.snp.vcf -o output_dir -p wild_snp
        """
    )
    ...
    # 重根化参数|Rerooting arguments
    reroot_group = parser.add_argument_group("重根化|Rerooting")
...
def main():
    ...
    try:
        ...
        analyzer = VCFPhyloAnalyzer(
            ...
            nw_reroot_path=args.nw_reroot_path,
