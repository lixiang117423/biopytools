...
class VCFStatsCollector:
    ...
    def collect_statistics(
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("VCF文件统计|VCF file statistics")
        self.logger.info("=" * 60)
        ...
        try:
            ...
            self.logger.info(f"染色体/Scaffold数|Chromosomes/Scaffolds: {chrom_count}")

        except subprocess.CalledProcessError as e:
            self.logger.error(f"统计失败|Statistics failed: {e.stderr}")
        except Exception as e:
            self.logger.error(f"统计时出错|Error collecting statistics: {str(e)}")
    ...
    def count_filtered_variants(
        ...
        if verbose:
            self.logger.info("=" * 60)
            self.logger.info("应用过滤条件并统计|Applying filters and counting")
            self.logger.info("=" * 60)
            self.logger.info(f"过滤参数|Filter parameters:")
            self.logger.info(f"  - 最小等位基因数|Min alleles: {min_alleles}")
            self.logger.info(f"  - 最大等位基因数|Max alleles: {max_alleles}")
        ...
        try:
            ...
            if verbose:
                self.logger.info(f"符合条件的变异数|Matching variants: {count}")

            return count

        except subprocess.CalledProcessError as e:
            self.logger.error(f"过滤统计失败|Filter statistics failed: {e.stderr}")
            return 0
...
class DsuiteRunner:
    ...
    def run_dtrios(
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("运行Dsuite Dtrios|Running Dsuite Dtrios")
        self.logger.info("=" * 60)
        ...
        try:
            ...
            if dsuite_exit != 0:
                self.logger.error(f"Dsuite运行失败|Dsuite failed (退出码|exit code: {dsuite_exit})")
                self.logger.error(f"错误信息|Error message: {stderr}")
                return False

            if bcftools_exit != 0:
                self.logger.warning(f"bcftools警告|bcftools warning (退出码|exit code: {bcftools_exit})")

            self.logger.info("Dsuite运行成功|Dsuite completed successfully")
            return True

        except Exception as e:
            self.logger.error(f"运行时出错|Runtime error: {str(e)}", exc_info=True)
            return False
    ...
    def check_output_files(self, output_prefix: str) -> Dict[str, bool]:
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("结果验证|Result validation")
        self.logger.info("=" * 60)
        ...
        for file_path in expected_files:
            ...
            if exists:
                ...
                self.logger.info(f"  {os.path.basename(file_path)} (大小|size: {file_size:,} bytes)")
            else:
                self.logger.warning(f"  {os.path.basename(file_path)} 不存在|not found")
        ...
        if os.path.exists(main_result):
            ...
            self.logger.info(f"分析的三元组数量|Analyzed trios: {trio_count}")
