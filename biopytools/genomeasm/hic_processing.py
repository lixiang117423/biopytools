...
class HiCProcessor:
        ...
        for assembly_type, fasta_list in fasta_files.items():
            ...
            for fasta_file in fasta_list:
                ...
                if self.config.hic_strategy == "complete_juicer":
                    ...
                    self.logger.error(f" 未知的Hi-C策略: {self.config.hic_strategy}|Unknown Hi-C strategy: {self.config.hic_strategy}")
                    continue
                
                if result:
                    scaffold_results[assembly_type][assembly_name] = result
                    self.logger.info(f" {assembly_name} Hi-C处理完成|{assembly_name} Hi-C processing completed")
                else:
                    self.logger.error(f" {assembly_name} Hi-C处理失败|{assembly_name} Hi-C processing failed")
    ...
    def _process_complete_juicer(self, fasta_file: str, assembly_name: str) -> Optional[Dict[str, str]]:
        ...
        self.logger.info(f" 执行完整Juicer流程: {assembly_name}|Running complete Juicer pipeline: {assembly_name}")
    ...
    def _process_standard_3ddna(self, fasta_file: str, assembly_name: str) -> Optional[Dict[str, str]]:
        ...
        self.logger.info(f" 执行标准3D-DNA流程: {assembly_name}|Running standard 3D-DNA pipeline: {assembly_name}")
    ...
    def _process_simplified_salsa2(self, fasta_file: str, assembly_name: str) -> Optional[Dict[str, str]]:
        ...
        self.logger.info(f" 执行简化SALSA2流程: {assembly_name}|Running simplified SALSA2 pipeline: {assembly_name}")
    ...
    def _run_juicer_pipeline(self, work_dir: Path, assembly_name: str) -> bool:
        ...
        self.logger.info(f" 运行Juicer pipeline: {assembly_name}|Running Juicer pipeline: {assembly_name}")
    ...
    def _convert_bam_to_merged_nodups(self, bam_file: str, work_dir: Path, assembly_name: str) -> Optional[str]:
        ...
        self.logger.info(f" 格式转换: {assembly_name}|Format conversion: {assembly_name}")
        ...
        try:
            ...
            self.logger.warning(f" pairtools转换失败，使用简单转换: {e}|pairtools conversion failed, using simple conversion: {e}")
            return self._simple_bam_conversion(bam_file, merged_nodups, assembly_name)
    ...
    def _simple_bam_conversion(self, bam_file: str, output_file: Path, assembly_name: str) -> Optional[str]:
        ...
        self.logger.info(f" 执行简单BAM转换: {assembly_name}|Running simple BAM conversion: {assembly_name}")
    ...
    def _run_3ddna_pipeline(self, work_dir: Path, assembly_name: str) -> Optional[Dict[str, str]]:
        ...
        if not merged_nodups.exists():
            self.logger.error(f" 未找到Juicer输出: {merged_nodups}|Juicer output not found: {merged_nodups}")
            return None
    ...
    def _check_3ddna_outputs(self, work_dir: Path, assembly_name: str) -> Optional[Dict[str, str]]:
        ...
        for key, file_path in expected_files.items():
            if file_path.exists():
                ...
                self.logger.info(f" 找到{key}: {file_path}|Found {key}: {file_path}")
            else:
                self.logger.warning(f" 未找到{key}: {file_path}|{key} not found: {file_path}")
            ...
            with open(stats_file, 'r') as f:
                ...
                for line in content.split('\n'):
                    if 'mapped (' in line:
                        ...
                        self.logger.info(f" {assembly_name} Hi-C比对率: {mapping_rate:.2%}|{assembly_name} Hi-C mapping rate: {mapping_rate:.2%}")
                        
                        if mapping_rate < self.config.min_mapping_rate:
                            self.logger.warning(f" {assembly_name} 比对率偏低: {mapping_rate:.2%}|{assembly_name} mapping rate is low: {mapping_rate:.2%}")
                        break
        except Exception as e:
            self.logger.warning(f" 无法解析比对统计: {e}|Unable to parse alignment stats: {e}")
    ...
    def _generate_hic_report(self, scaffold_results: Dict[str, Dict[str, str]]):
        ...
        self.logger.info(f" Hi-C处理报告已生成: {report_file}|Hi-C processing report generated: {report_file}")
