...
class GenomeAssembler:
    ...
    def _phase1_environment_check(self):
        ...
        try:
            ...
            if self.config.skip_dependency_check:
                self.logger.warning(" 跳过依赖软件检查|Skipping dependency check")
            else:
                # 检查依赖软件
    ...
    def _phase4_hic_processing(self, assembly_files: Dict[str, List[str]]) -> Optional[Dict[str, Dict[str, str]]]:
        ...
        try:
            ...
            # Hi-C处理失败不应该终止整个流程
            self.logger.warning(" 继续进行质量评估，但不包含挂载结果|Continuing with quality assessment without scaffolding results")
            return None
    ...
    def _phase5_quality_assessment(self, assembly_files: Dict[str, List[str]], 
        ...
        try:
            ...
            # 质量评估失败不应终止流程，返回空结果
            self.logger.warning(" 继续进行结果整理|Continuing with results organization")
            return {}
    ...
    def _evaluate_qc_results(self, qc_results: Dict[str, Any]):
        ...
        if 'hifi' in qc_results:
            ...
            if not hifi.get('pass_qc', True):
                critical_issues.append(f"HiFi数据质量不达标|HiFi data quality below threshold: 覆盖度|coverage {hifi.get('coverage', 0):.1f}X")
        
        # 检查Hi-C数据
        if 'hic' in qc_results:
            hic = qc_results['hic']
            if 'pass_qc' in hic and not hic['pass_qc']:
                critical_issues.append(f"Hi-C数据质量不达标|Hi-C data quality below threshold: 覆盖度|coverage {hic.get('coverage', 0):.1f}X")
        
        if critical_issues:
            self.logger.warning(" 数据质量警告|Data quality warnings:")
    ...
    def _validate_assembly_outputs(self, assembly_files: Dict[str, List[str]]):
        ...
        for assembly_type, files in assembly_files.items():
            self.logger.info(f" {assembly_type} 组装|assembly: {len(files)} 个文件|files")
    ...
    def _validate_scaffold_outputs(self, scaffold_results: Optional[Dict[str, Dict[str, str]]]):
        ...
        if not scaffold_results:
            self.logger.warning(" 未生成挂载结果|No scaffolding results generated")
            return
        ...
        for assembly_type, results in scaffold_results.items():
            for assembly_name, files in results.items():
                ...
                if files:  # 至少有一个输出文件
                    success_count += 1
                    self.logger.info(f" {assembly_type}_{assembly_name} 挂载成功|scaffolding succeeded")
        
        self.logger.info(f" 挂载成功率|Scaffolding success rate: {success_count}/{total_count} ({success_count/total_count*100:.1f}%)")
    ...
    def _print_next_steps(self, final_results: Dict[str, Any]):
        ...
        if recommendations:
            print(" 改进建议|Improvement Recommendations:")
...
🔗 Hi-C处理策略|Hi-C Processing Strategies:
 ...
 更多信息请访问项目文档|For more information, visit project documentation
    ...
    try:
        ...
        print("用户中断程序执行|User interrupted program execution")
        sys.exit(1)
    except Exception as e:
