...
class DataQualityController:
    ...
    def _qc_hifi_data(self) -> Dict[str, any]:
        ...
        if not results['pass_qc']:
            self.logger.warning(f" HiFi覆盖度偏低|Low HiFi coverage: {coverage:.1f}X < {self.config.min_hifi_coverage}X")
        else:
    ...
    def _qc_hic_data(self) -> Dict[str, any]:
        ...
        for read_type, file_path in [('R1', self.config.hic_r1), ('R2', self.config.hic_r2)]:
            ...
            if not self.config.skip_fastqc:
                ...
                self.logger.info(f" 跳过FastQC检查: Hi-C {read_type} (节省时间)")
                results[read_type.lower()] = {
                    'file': file_path,
                    'fastqc_skipped': True
                }
        ...
        if results.get('r1', {}).get('stats', {}).get('sum_len'):
            ...
            if not results['pass_qc']:
                self.logger.warning(f" Hi-C覆盖度偏低|Low Hi-C coverage: {hic_coverage:.1f}X")
            else:
    ...
    def _qc_ont_data(self) -> Dict[str, any]:
        ...
        if not results['pass_qc']:
            self.logger.warning(f" ONT覆盖度偏低|Low ONT coverage: {coverage:.1f}X")
        else:
    ...
    def _analyze_read_lengths(self, reads_file: str) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.warning(f" 读长分析失败|Read length analysis failed: {e}")
            return {}
    ...
    def _assess_data_sufficiency(self, qc_results: Dict[str, any]):
        ...
        if issues:
            self.logger.warning(" 数据质量问题|Data quality issues:")
            for issue in issues:
