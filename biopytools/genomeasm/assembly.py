...
class HifiasmAssembler:
    ...
    def _check_assembly_outputs(self) -> Dict[str, List[str]]:
        ...
        if primary_bp.exists():
            ...
            self.logger.info(f" 找到基础primary contigs|Found base primary contigs: {primary_bp}")
        ...
        if 'Hi-C' in self.config.detected_data_types:
            ...
            if primary_hic.exists():
                ...
                self.logger.info(f" 找到Hi-C优化primary contigs|Found Hi-C optimized primary contigs: {primary_hic}")
        ...
        if alternate_bp.exists():
            ...
            self.logger.info(f" 找到基础alternate contigs|Found base alternate contigs: {alternate_bp}")
        ...
        if 'Hi-C' in self.config.detected_data_types:
            ...
            if alternate_hic.exists():
                ...
                self.logger.info(f" 找到Hi-C优化alternate contigs|Found Hi-C optimized alternate contigs: {alternate_hic}")
        ...
        for hap in [1, 2]:
            ...
            if hap_file.exists():
                ...
                self.logger.info(f" 找到单倍型{hap}组装|Found haplotype {hap} assembly: {hap_file}")
        ...
        for assembly_type, gfa_list in gfa_files.items():
            ...
            for i, gfa_file in enumerate(gfa_list):
                ...
                if success and fasta_path.exists():
                    ...
                    self.logger.info(f" 转换完成|Conversion completed: {fasta_path}")
                else:
                    self.logger.error(f" 转换失败|Conversion failed: {gfa_file} -> {fasta_path}")
    ...
    def _generate_assembly_stats(self, fasta_files: Dict[str, List[str]]):
        ...
        self.logger.info(f" 组装统计已保存|Assembly statistics saved: {stats_file}")
