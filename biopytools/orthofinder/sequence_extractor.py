...
class SequenceExtractor:
    ...
    def load_genome_sequences(self):
        ...
        for fasta_file in fasta_files:
            ...
            self.logger.info(f"加载 {genome_name} 序列|Loading {genome_name} sequences")
            ...
            try:
                ...
                self.logger.error(f"加载 {fasta_file} 失败|Failed to load {fasta_file}: {e}")
    ...
    def extract_single_copy_sequences(self, single_copy_genes: List[Tuple], output_dir: Path):
        ...
        single_copy_dir = Path(output_dir) / "04_single_copy_genes"
        single_copy_dir.mkdir(parents=True, exist_ok=True)
