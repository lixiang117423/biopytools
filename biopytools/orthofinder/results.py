...
class ResultsProcessor:
    ...
    def generate_comprehensive_report(self, classification_results: Dict, genome_stats: Dict,
        ...
        reports_dir = Path(output_dir) / "06_reports"
        reports_dir.mkdir(parents=True, exist_ok=True)

        report_file = reports_dir / "pangenome_analysis_report.txt"
    ...
    def _write_pangenome_overview(self, f, classification_results: Dict):
        ...
        sc_percentage = (sc_count / len(classification_results['core']) * 100) if classification_results['core'] else 0
        f.write(f"  单拷贝基因|Single copy genes (核心基因的子集|subset of core):\n")
        f.write(f"    同源基因群: {sc_count:,} ({sc_percentage:.2f}% of core)\n")
        f.write("\n")
    ...
    def _write_output_files_summary(self, f, output_dir: Path):
        ...
        output_files = [
            ("02_pangenome_classification/pangenome_gene_families.txt", "泛基因组基因家族详细信息|Detailed pangenome gene families"),
            ("02_pangenome_classification/gene_families_detailed.tsv", "基因家族详细表格|Detailed gene families table"),
            ("02_pangenome_classification/pangenome_classification_summary.txt", "泛基因组分类统计摘要|Pangenome classification summary"),
            ("02_pangenome_classification/frequency_distribution_table.tsv", "频率分布表格|Frequency distribution table"),
            ("03_rarefaction_analysis/rarefaction_curve_summary.tsv", "稀释曲线摘要|Rarefaction curve summary"),
            ("04_single_copy_genes/single_copy_genes_summary.txt", "单拷贝基因汇总|Single copy genes summary"),
            ("05_visualization/pangenome_rarefaction_curve.png", "稀释曲线图|Rarefaction curve plot"),
            ("05_visualization/pangenome_classification_pie.png", "分类饼图|Classification pie chart"),
            ("05_visualization/frequency_distribution.png", "频率分布图|Frequency distribution plot"),
            ("06_reports/pangenome_analysis_report.txt", "综合分析报告|Comprehensive analysis report"),
            ("99_logs/pangenome_analysis.log", "分析日志文件|Analysis log file")
        ]

        for filename, description in output_files:
            file_path = Path(output_dir) / filename
            status = "存在|Exists" if file_path.exists() else "缺失|Missing"
    ...
    def save_gene_families_table(self, classification_results: Dict, output_dir: Path) -> Path:
        ...
        data_rows = []
        single_copy_ogs = {gene_info[0] for gene_info in classification_results.get('single_copy', [])}
        for category in ['core', 'softcore', 'dispensable', 'private']:
            ...
            for gene_info in classification_results[category]:
                try:
                    ...
                    is_single_copy = 'Yes' if og_id in single_copy_ogs else 'No'
                    ...
                    self.logger.error(f"处理基因信息时出错|Error processing gene info {gene_info}: {e}")
                    continue

        if data_rows:
            df = pd.DataFrame(data_rows)
            output_file = classification_dir / "gene_families_detailed.tsv"
    ...
    def save_frequency_distribution_table(self, frequency_distributions: Dict, output_dir: Path) -> Path:
        ...
        output_file = classification_dir / "frequency_distribution_table.tsv"
        df.to_csv(output_file, sep='\t', index=False)
