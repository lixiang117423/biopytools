...
def setup_logger(output_dir: str) -> logging.Logger:
    ...
    # 添加控制台handler|Add console handler
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='NGenomeSyn基因组共线性分析流程|NGenomeSyn Genome Synteny Analysis Pipeline',
...
示例|Examples:
  %(prog)s -s samples.txt -o output_dir
    ...
    try:
        ...
        # 检查环境
        logger.info("检查环境|Checking environment")
        env_results = env_checker.check_environment(config.aligner)
        if not env_results["NGenomeSyn"]:
            logger.error("NGenomeSyn未安装或不在PATH中|NGenomeSyn not installed or not in PATH")
            return 1
        if not env_results.get(config.aligner, False):
            logger.error(f"{config.aligner} 未安装或不在PATH中|{config.aligner} not installed or not in PATH")
            return 1

        # 检查SyRI（如果使用）
        if config.use_syri:
            if not env_results.get("syri", False):
                logger.error("SyRI未安装或不在PATH中，但已启用--use-syri选项|SyRI not installed or not in PATH, but --use-syri is enabled")
                return 1
            logger.info("SyRI已启用|SyRI is enabled")

        # 读取样本信息
        if config.sample_map:
            logger.info("读取样本映射文件|Reading sample mapping file")
            samples = processor.read_sample_map(config.sample_map)

            if len(samples) < 2:
                logger.error("至少需要2个基因组进行共线性分析|At least 2 genomes required for synteny analysis")
                return 1

            logger.info(f"成功读取 {len(samples)} 个基因组|Successfully read {len(samples)} genomes")
            ...
            logger.error("暂不支持从配置文件读取模式|Reading from config file mode is not yet supported")
            return 1

        # 步骤1: 执行比对
        logger.info("")
        logger.info("步骤1: 执行基因组比对|Step 1: Executing genome alignment")
        logger.info("=" * 60)
        ...
        for link in config_data["links"]:
            ...
            logger.info(f"比对: {genome1['name']} vs {genome2['name']}|Alignment: {genome1['name']} vs {genome2['name']}")

            if config.use_syri:
                # 使用SyRI流程
                if config.aligner == "minimap2":
                    # 对于Minimap2 + SyRI，需要生成SAM格式
                    logger.info("使用Minimap2 + SyRI流程|Using Minimap2 + SyRI workflow")
                    ...
                    logger.info(f"SyRI分析完成: {link_file}|SyRI analysis completed: {link_file}")

                elif config.aligner == "mummer":
                    # 对于MUMmer + SyRI，使用coords文件
                    logger.info("使用MUMmer + SyRI流程|Using MUMmer + SyRI workflow")
                ...
                logger.info(f"比对完成: {link_file}|Alignment completed: {link_file}")

        # 步骤2: 生成可视化
        logger.info("")
        logger.info("步骤2: 生成可视化|Step 2: Generating visualization")
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info(f"输入基因组: {len(samples)}|Input genomes: {len(samples)}")
        logger.info(f"比对数量: {len(links)}|Number of alignments: {len(links)}")
