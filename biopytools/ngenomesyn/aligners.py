    ...
    def _check_conversion_script(self) -> bool:
        ...
        try:
            ...
            self.log_command(
                ...
                working_dir=self.output_dir,
                description=" 检查GetTwoGenomeSyn.pl脚本是否可用|Checking GetTwoGenomeSyn.pl script availability"
            )
    ...
    def align_for_syri(self, genome1: Dict, genome2: Dict, selected_chromosomes: Optional[List[int]] = None, **params) -> str:
        ...
        try:
            ...
            if os.path.getsize(sam_file) == 0:
                self.logger.warning(f" SAM文件为空，可能没有找到比对结果|SAM file is empty, no alignments found")
                # 不抛出异常，因为这可能是正常的生物学现象
    ...
    def _check_show_coords(self) -> bool:
        ...
        try:
            ...
            self.log_command(
                ...
                working_dir=self.output_dir,
                description=" 检查show-coords工具是否可用|Checking show-coords tool availability"
            )
    ...
    def run_syri_from_minimap2(
        ...
        try:
            ...
            self.log_command(
                command=cmd_syri,
                working_dir=self.output_dir,
                description=f" SyRI结构变异分析|SyRI structural variation analysis: {genome1['name']} vs {genome2['name']}"
            )
    ...
    def run_syri_from_mummer_coords(
        ...
        try:
            ...
            self.log_command(
                command=cmd_syri,
                working_dir=self.output_dir,
                description=f" SyRI结构变异分析 (from coords)|SyRI structural variation analysis (from coords): {genome1['name']} vs {genome2['name']}"
            )
    ...
    def _convert_syri_to_link(self, syri_out_file: str, link_file: str, min_length: int) -> str:
        ...
        try:
            ...
            self.log_command(
                command=cmd_convert,
                working_dir=self.output_dir,
                description=f" SyRI转LINK格式转换|SyRI to LINK format conversion: {os.path.basename(syri_out_file)} -> {os.path.basename(link_file)}"
            )
            ...
            if result.returncode != 0:
                self.logger.warning(f" SyRI转换失败|SyRI conversion failed: {result.stderr}")
