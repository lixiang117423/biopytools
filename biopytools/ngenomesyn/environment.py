...
class EnvironmentChecker:
    ...
    def save_environment_check_script(self, output_dir: str = "./"):
        ...
        with open(script_path, 'w') as f:
            ...
            # 检查可选工具
            f.write("#  检查可选工具|Check optional tools\n")
            f.write("echo \" 检查可选工具|Checking optional tools\"\n")
            for tool, description in self.optional_tools.items():
                f.write(f"if command -v {tool} >/dev/null 2>&1; then\n")
                f.write(f"    echo \" {tool} 已安装 - {description}\"\n")
                f.write("else\n")
                f.write(f"    echo \" {tool} 未安装 - {description}\"\n")
                f.write("fi\n\n")
    ...
    def check_tool_installation(self, tool_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.warning(f" {tool_name} 未安装|not installed")
                return False
    ...
    def check_syri(self) -> bool:
        ...
        self.logger.warning(" SyRI 未安装|SyRI not installed")
        return False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    self.logger.warning(f" NGenomeSyn 可执行但验证失败|NGenomeSyn executable but verification failed")
    ...
    def check_environment(self, required_aligner: str = "minimap2") -> Dict[str, bool]:
        ...
        try:
            ...
            self.logger.warning(f" 保存环境检查脚本失败|Failed to save environment check script: {e}")
