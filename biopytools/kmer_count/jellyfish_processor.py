...
class JellyfishProcessor:
    ...
    def count_kmers(self, sample_name: str, fastq_files: List[str]) -> Path:
        ...
        for file_path in input_files:
            if file_path.endswith('.gz'):
                is_complete, is_readable = self.file_processor.enhanced_file_integrity_check(file_path)
                
                if is_complete:
                    self.logger.info(f"文件完整: {os.path.basename(file_path)}")
                    processed_files.append(file_path)
                elif is_readable:
                    self.logger.warning(f"文件部分损坏，将恢复: {os.path.basename(file_path)}")
                    # 检测可读行数并恢复
                    readable_lines = self.file_processor.detect_readable_lines(file_path)
                    if readable_lines > 160000:  # 至少4万reads
                        recovered_file = self.file_processor.recover_partial_file(file_path, readable_lines)
                        processed_files.append(recovered_file)
                        recovery_needed = True
                    else:
                        self.logger.error(f"文件损坏过于严重，跳过: {os.path.basename(file_path)}")
                        from modelscope.hub.errors import FileIntegrityError
                        raise FileIntegrityError(f"文件损坏过于严重: {sample_name}")
                else:
                    self.logger.error(f"文件完全不可读，跳过: {os.path.basename(file_path)}")
                    from modelscope.hub.errors import FileIntegrityError
                    raise FileIntegrityError(f"文件完全不可读: {sample_name}")
            else:
                # 非压缩文件直接使用
                processed_files.append(file_path)
        
        # 根据文件状态选择处理方式
        if not recovery_needed and any(f.endswith('.gz') for f in processed_files):
            # 使用generator方式处理完整的压缩文件
            return self._count_with_generator(sample_name, processed_files, jf_file)
        else:
            # 使用直接方式处理恢复文件或非压缩文件
            return self._count_with_direct(sample_name, processed_files, jf_file)

    def _count_with_generator(self, sample_name: str, files: List[str], jf_file: Path) -> Path:
        """使用generator方式计数"""
        generator_file = self.config.temp_dir / f"{sample_name}_generator.txt"
        ...
        self.logger.info(f"使用generator方式处理: {generator_file}")
        ...
        self.cmd_runner.run(cmd, f"Generator K-mer计数 | Generator K-mer counting for {sample_name}")
    ...
    def _count_with_direct(self, sample_name: str, files: List[str], jf_file: Path) -> Path:
        """使用直接方式计数"""
        self.logger.info(f"使用直接方式处理恢复文件")
        ...
        self.cmd_runner.run(cmd, f"直接K-mer计数 | Direct K-mer counting for {sample_name}")
