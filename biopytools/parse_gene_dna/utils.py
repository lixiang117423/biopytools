"""
基因序列提取工具函数模块|Gene Sequence Extraction Utility Functions Module
"""
...
class SequenceUtils:
    ...
    def extract_sequence_region(genome_seq: str, start: int, end: int, strand: str) -> str:
        """从基因组中提取序列片段|Extract sequence fragment from genome"""
        # GFF坐标是1-based，Python是0-based|GFF coordinates are 1-based, Python is 0-based
        seq = genome_seq[start-1:end]

        # 如果是负链，需要反向互补|If negative strand, need reverse complement
        if strand == '-':
            ...
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    if not line or line.startswith('#'):
                        continue
                    # 检查是否有9列|Check if there are 9 columns
                    fields = line.split('\t')
                    return len(fields) == 9
            return True
        except Exception:
