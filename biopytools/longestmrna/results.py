...
class SummaryGenerator:
    ...
    def print_summary(self, stats: Dict[str, Any]):
        ...
        if hasattr(self.config, 'cds_output_file') and self.config.cds_output_file:
            self.logger.info(f"  - CDS核苷酸序列文件|CDS nucleotide sequences: {self.config.cds_output_file}")
        self.logger.info("=" * 50)
