...
class PhylogeneticMatrixBuilder:
    ...
    def create_phylogenetic_matrix(self):
        ...
        if not os.path.exists(core_genes_file):
            self.logger.error(" 未找到核心基因列表，请先运行序列提取|Core gene list not found, please run sequence extraction first")
            return False
        
        with open(core_genes_file, 'r') as f:
            core_genes = [line.strip() for line in f if line.strip()]
        
        self.logger.info(f" 使用 {len(core_genes)} 个核心基因构建矩阵|Using {len(core_genes)} core genes to build matrix")
        ...
        if not sample_sequences:
            self.logger.error(" 未找到样本序列数据|No sample sequence data found")
            return False
    ...
    def write_supermatrix(self, sample_sequences, core_genes, matrix_dir):
        ...
        self.logger.info(f" 超基因组矩阵已创建: {supermatrix_file}|Supermatrix created: {supermatrix_file}")
        self.logger.info(f" 分区文件已创建: {partition_file}|Partition file created: {partition_file}")
