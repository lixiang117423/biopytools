...
class SequenceWriter:
    ...
    def write_sample_sequences(self, sample_sequences_dict, output_base_dir, seq_type):
        ...
        self.logger.info(f" 已为 {len(sample_sequences_dict)} 个样品创建独立的{seq_type}序列文件 | Created separate {seq_type} sequence files for {len(sample_sequences_dict)} samples")
    ...
    def write_gene_sequences(self, sequences_dict, output_base_dir, seq_type):
        ...
        self.logger.info(f" 输出 {len(sequences_dict)} 个基因的{seq_type}序列 | Output {seq_type} sequences for {len(sequences_dict)} genes")
