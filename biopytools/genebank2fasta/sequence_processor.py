...
class SequenceExtractor:
    ...
    def process_single_file(self, gb_file):
        ...
        try:
            ...
            self.logger.error(f"处理文件 {gb_file} 时出错|Error processing file {gb_file}: {e}")
    ...
    def extract_cds_sequence(self, feature, record, sample_name, organism):
        ...
        try:
            ...
            if len(cds_sequence) % 3 != 0:
                self.logger.warning(f" {gene_name} CDS长度不是3的倍数 ({len(cds_sequence)} bp)|{gene_name} CDS length is not a multiple of 3 ({len(cds_sequence)} bp)")
            ...
            if len(protein_sequence) < self.config.min_protein_length:
                self.logger.warning(f" {gene_name} 蛋白质序列过短 ({len(protein_sequence)} aa)|{gene_name} protein sequence too short ({len(protein_sequence)} aa)")
                return None
            ...
            self.logger.error(f"提取 {gene_name} 序列时出错|Error extracting sequence for {gene_name}: {e}")
            return None
