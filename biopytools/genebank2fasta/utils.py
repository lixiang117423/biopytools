"""
GenBank序列提取工具函数模块|GenBank Sequence Extraction Utility Functions Module
"""
...
class FileManager:
    ...
    def find_genbank_files(self):
        ...
        if not genbank_files:
            self.logger.error(f"在{self.config.gb_dir}中未找到GenBank文件(.gb/.gbk)|No GenBank files found in {self.config.gb_dir}")
            return []

        self.logger.info(f"找到{len(genbank_files)}个GenBank文件|Found {len(genbank_files)} GenBank files")
    ...
    def create_output_directories(self):
        ...
        self.logger.info("输出目录结构已创建|Output directory structure created")
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for future in as_completed(future_to_file):
            ...
            try:
                ...
                logger.info(f"处理完成|Processed: {os.path.basename(file)}")
            except Exception as e:
                logger.error(f"处理文件{file}时出错|Error processing {file}: {e}")
