...
class GenBankExtractor:
    ...
    def run_extraction(self):
        ...
        try:
            self.logger.info("开始GenBank序列提取流程|Starting GenBank sequence extraction pipeline")
            ...
            self.logger.info("步骤1: 查找GenBank文件|Step 1: Find GenBank files")
            genbank_files = self.file_manager.find_genbank_files()
            if not genbank_files:
                self.logger.error("未找到GenBank文件，退出程序|No GenBank files found, exiting")
                return False

            # 步骤2: 创建输出目录|Step 2: Create output directories
            self.logger.info("步骤2: 创建输出目录结构|Step 2: Create output directories")
            self.file_manager.create_output_directories()

            # 步骤3: 并行提取序列|Step 3: Extract sequences in parallel
            self.logger.info(f"步骤3: 并行提取序列(使用{self.config.threads}个线程)|Step 3: Parallel extraction (using {self.config.threads} threads)")
            ...
            if not all_sample_data:
                self.logger.error("序列提取失败，退出程序|Sequence extraction failed, exiting")
                return False

            # 步骤4: 写入输出文件|Step 4: Write output files
            self.logger.info("步骤4: 写入序列文件|Step 4: Write sequence files")
            self.sequence_writer.write_sequences(all_sample_data)

            # 步骤5: 生成统计报告|Step 5: Generate statistics report
            self.logger.info("步骤5: 生成统计报告|Step 5: Generate statistics report")
            ...
            if self.config.create_phylogenetic_matrix:
                self.logger.info("步骤6: 创建系统发育分析矩阵|Step 6: Create phylogenetic matrix")
            ...
            self.logger.info(f"处理样本数|Samples processed: {len(all_sample_data)}")
            self.logger.info(f"总提取基因数|Total genes extracted: {total_genes_extracted}")
            self.logger.info(f"不重复基因数|Unique genes: {unique_genes}")
            self.logger.info(f"CDS文件输出到|CDS files output to: {self.config.cds_dir}")
            self.logger.info(f"蛋白质文件输出到|Protein files output to: {self.config.pep_dir}")
            self.logger.info("=" * 80)

            return True

        except Exception as e:
            self.logger.error(f"序列提取流程在执行过程中意外终止|Sequence extraction pipeline terminated: {e}")
            sys.exit(1)
...
示例|Examples:
  %(prog)s -i /path/to/genbank/files -o ./output
    ...
    parser.add_argument('-o', '--output-dir', default='./genbank_output',
    ...
    try:
        ...
        print(f"程序初始化失败|Program initialization failed: {e}")
        sys.exit(1)
