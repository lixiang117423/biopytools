...
class VCFRenamer:
    ...
    def rename(self):
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info("步骤1/4: 提取样品名称|Step 1/4: Extracting sample names")
            self.logger.info("=" * 60)
            ...
            self.logger.info("=" * 60)
            self.logger.info("步骤2/4: 生成新旧ID映射表|Step 2/4: Generating sample name mapping")
            self.logger.info("=" * 60)
            ...
            self.logger.info("=" * 60)
            self.logger.info("步骤3/4: 使用bcftools重命名样品|Step 3/4: Renaming samples with bcftools")
            self.logger.info("=" * 60)
            ...
            self.logger.info("=" * 60)
            self.logger.info("步骤4/4: 创建VCF索引|Step 4/4: Creating VCF index")
            self.logger.info("=" * 60)
            ...
            if not self.config.keep_mapping:
                self.logger.info(f"删除映射文件|Deleting mapping file: {self.config.mapping_file}")
...
示例|Examples:
  %(prog)s -i input.vcf.gz -o output.vcf.gz
    ...
    optional.add_argument('-p', '--prefix', default='S',
                         help='新样品名前缀|New sample name prefix')
