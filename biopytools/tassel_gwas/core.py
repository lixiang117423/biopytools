...
class TASSELGWASConfig:
    ...
    vcf_file: str = ''
    pheno_file: str = ''
    output_prefix: str = 'GWAS_Result'
    output_dir: str = '.'

    # TASSEL路径|TASSEL path
    tassel_path: Optional[str] = None
    memory_min: str = '10g'
    memory_max: str = '100g'
    ...
    miss_filter: Optional[float] = None
    skip_sort: bool = True  # 默认跳过VCF排序，适用于已过滤的大规模VCF文件
    pca_components: int = 5  # PCA主成分数量，默认为5

    # 矩阵文件|Matrix files
    q_matrix: Optional[str] = None
    kinship: Optional[str] = None

    # 其他选项|Other options
    keep_temp: bool = False
    log_level: str = 'INFO'
    filter_common_samples: bool = True  # 是否进行样本交集筛选
    ...
    def __post_init__(self):
        ...
        self.tassel_path = str(Path(self.tassel_path).expanduser()) if self.tassel_path else None
        self.q_matrix = str(Path(self.q_matrix).expanduser()) if self.q_matrix else None
        self.kinship = str(Path(self.kinship).expanduser()) if self.kinship else None

        # 确保模型名称大写|Ensure model name is uppercase
        self.model = self.model.upper()
        # 确保日志级别大写|Ensure log level is uppercase
...
class TASSELGWASAnalyzer:
    ...
    def validate_input_files(self):
        ...
        # 如果skip_sort为True，只进行基本验证，跳过耗时的统计操作
        if self.config.skip_sort:
            self.logger.info(" 跳过VCF详细验证（skip_sort=True）|Skipping detailed VCF validation (skip_sort=True)")
            vcf_path = Path(self.config.vcf_file)
            if not vcf_path.exists():
                self.logger.error(f"VCF文件不存在|VCF file does not exist: {vcf_path}")
                return False
            self.logger.info(f" VCF文件存在|VCF file exists: {vcf_path}")
        else:
            if not validate_vcf_file(Path(self.config.vcf_file), self.logger):
                return False

        # 验证表型文件|Validate phenotype file
        if not validate_phenotype_file(Path(self.config.pheno_file), self.logger):
            return False

        # 验证Q矩阵文件|Validate Q matrix file
        if self.config.q_matrix and Path(self.config.q_matrix).exists():
            q_dims = self._count_matrix_columns(Path(self.config.q_matrix))
            self.logger.info(f" Q矩阵已提供|Q matrix provided: {self.config.q_matrix} (K={q_dims})")

        # 验证Kinship矩阵文件|Validate Kinship matrix file
        if self.config.kinship and Path(self.config.kinship).exists():
    ...
    def _get_vcf_stats(self, vcf_file: Path) -> Tuple[int, int]:
        ...
        try:
            ...
            self.logger.info(f"   命令|Command: {variant_cmd}")
            ...
            self.logger.info(f"   命令|Command: {sample_cmd}")
    ...
    def _get_pheno_stats(self, pheno_file: Path) -> Tuple[int, int]:
        ...
        try:
            ...
            self.logger.warning(f" 无法获取表型统计信息|Cannot get phenotype statistics: {e}")
            return 0, 0

    def prepare_phenotype_file(self, pheno_file: Path = None) -> Path:
        ...
        if prepare_phenotype_file(pheno_file, pheno_ready, self.logger):
            return pheno_ready
        else:
            raise RuntimeError("表型文件准备失败|Phenotype file preparation failed")

    def sort_and_filter_vcf(self, vcf_file: Path = None) -> Path:
        """VCF排序和过滤|VCF sorting and filtering"""
        if vcf_file is None:
            vcf_file = Path(self.config.vcf_file)
        ...
        try:
            ...
            variant_count = len([line for line in subprocess.run(grep_cmd,
            ...
            self.logger.warning(f" bcftools不可用，使用原始VCF|bcftools not available, using original VCF")
            return vcf_file
        except FileNotFoundError:
            self.logger.warning(f" bcftools未找到，使用原始VCF|bcftools not found, using original VCF")
            return vcf_file
    ...
    def calculate_pca(self, vcf_file: Path) -> Path:
        ...
        try:
            ...
            vcf2pca_path = None
            for path_str in vcf2pca_paths:
                # 展开路径中的~符号
                expanded_path = Path(path_str).expanduser()
                if expanded_path.exists():
                    vcf2pca_path = str(expanded_path)
                    self.logger.info(f"   找到VCF2PCACluster|Found VCF2PCACluster: {vcf2pca_path}")
                    break

            if not vcf2pca_path:
                self.logger.error(f" VCF2PCACluster未找到|VCF2PCACluster not found, tried paths: {vcf2pca_paths}")
            ...
            self.logger.info(f"   命令|Command: {shell_cmd}")
    ...
    def calculate_kinship(self, vcf_file: Path) -> Path:
        ...
        try:
            ...
            if memory_gb < 20:
                self.logger.warning(f" 内存设置可能不足 ({memory_max})，对于大规模数据建议至少20GB|Memory setting may be insufficient ({memory_max}), at least 20GB recommended for large-scale data")
            ...
            try:
                ...
                self.logger.info(f"   命令|Command: {' '.join(test_cmd)}")
            ...
            try:
                ...
                self.logger.info(f"   命令|Command: {shell_cmd}")
                result = subprocess.run(shell_cmd, capture_output=True, text=True,
                                        shell=True, timeout=14400,  # 4小时超时（适合大规模数据）
                                        cwd=str(self.config.output_dir))
                ...
                self.logger.error(f"   Kinship计算超时|Kinship calculation timed out after 4 hours")
    ...
    def run_glm(self, vcf_file: Path, pheno_file: Path) -> Optional[Dict]:
        ...
        try:
            ...
            cmd = [
                ...
                '-fork1', '-vcf', str(Path(self.config.vcf_file).absolute()),
            ...
            if self.config.q_matrix and Path(self.config.q_matrix).exists():
                ...
                # Q矩阵使用绝对路径（通常在外部）
                cmd.extend([
                    '-fork3', '-q', str(Path(self.config.q_matrix).absolute()),
            ...
            self.logger.info(f"   命令|Command: {glm_cmd}")
    ...
    def _extract_glm_results(self, input_file: Path, output_file: Path):
        ...
        try:
            ...
            self.logger.warning(f" GLM结果提取失败|GLM result extraction failed: {e}")
    ...
    def run_mlm(self, vcf_file: Path, pheno_file: Path, kinship_file: Path) -> Optional[Dict]:
        ...
        try:
            ...
            if self.config.q_matrix and Path(self.config.q_matrix).exists():
                ...
                pca_covariate_file = Path(self.config.q_matrix)
                ...
                try:
                    ...
                    original_vcf = Path(self.config.vcf_file)
            ...
            self.logger.info(f"   命令|Command: {mlm_cmd}")
            result = subprocess.run(cmd, capture_output=True, text=True, check=True,
                                    cwd=str(self.config.output_dir))
            ...
            try:
                ...
                if len(txt_files) > 10:
                    self.logger.info(f"     ... 以及另外{len(txt_files)-10}个文件|... and {len(txt_files)-10} more files")
            except Exception as e:
                self.logger.warning(f"    无法列出当前目录文件|Cannot list current directory files: {e}")
                    ...
                    if matches:
                        ...
                        for f in matches:
                            try:
                                ...
                                self.logger.debug(f"    无法访问文件 {f}: {e}|Cannot access file {f}: {e}")
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # 样本交集处理|Filter to common samples
            working_vcf = Path(self.config.vcf_file)
            working_pheno = Path(self.config.pheno_file)

            if self.config.filter_common_samples:
                self.logger.info(" 样本交集筛选|Sample intersection filtering enabled")
                success, vcf_filtered, pheno_filtered, n_common = find_common_samples_and_filter(
                    Path(self.config.vcf_file),
                    Path(self.config.pheno_file),
                ...
                if not success:
                    self.logger.error("样本交集处理失败|Sample intersection filtering failed")
                    return False

                working_vcf = vcf_filtered
                working_pheno = pheno_filtered
                self.logger.info(f"使用筛选后的文件|Using filtered files: {n_common} common samples")
            else:
                self.logger.info("跳过样本交集筛选|Skipping sample intersection filtering")

            # 获取文件统计信息|Get file statistics
            vcf_variants, vcf_samples = self._get_vcf_stats(Path(self.config.vcf_file))
            pheno_samples, pheno_traits = self._get_pheno_stats(Path(self.config.pheno_file))

            self.logger.info(f" 输入VCF|Input VCF: {working_vcf}")
            self.logger.info(f"   • 变异位点|Variants: {vcf_variants}")
            self.logger.info(f"   • 样本数|Samples: {vcf_samples}")
            self.logger.info(f" 表型文件|Phenotype file: {working_pheno}")
            self.logger.info(f"   • 样本数|Samples: {pheno_samples}")
            self.logger.info(f"   • 表型数量|Traits: {pheno_traits}")

            # 准备表型文件|Prepare phenotype file
            pheno_ready = self.prepare_phenotype_file(working_pheno)

            # VCF排序和过滤（如果未跳过）| VCF sorting and filtering (if not skipped)
            if self.config.skip_sort:
                # 使用筛选后的VCF文件| Use filtered VCF file
                vcf_sorted = working_vcf
                self.logger.info(" 跳过VCF排序，使用现有文件|Skipping VCF sorting, using existing file")
            else:
                # 对于已经筛选过样本的VCF，仍然进行排序和过滤
                vcf_sorted = self.sort_and_filter_vcf(working_vcf)

            # 计算Kinship矩阵（如果需要且未预计算）| Calculate Kinship matrix (if needed and not pre-computed)
            kinship_file = None
            if self.config.kinship and Path(self.config.kinship).exists():
                kinship_file = Path(self.config.kinship)
                ...
                if self.config.skip_sort:
                    ...
                    self.logger.warning(" 未提供K矩阵但跳过了排序，请确保已预计算K矩阵|No K matrix provided but sorting skipped, please ensure K matrix is pre-computed")
                else:
