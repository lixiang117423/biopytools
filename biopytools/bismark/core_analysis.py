...
class CorePipeline:
    ...
    def build_bismark_index(self) -> bool:
        self.logger.info(f" 步骤1: 检查并构建Bismark索引于基因组目录|Step 1: Check and build Bismark index in genome directory: {self.config.genome_dir}")
    ...
    def run_mapping_and_extraction(self) -> bool:
        self.logger.info(" 步骤2: 开始甲基化比对和提取|Step 2: Starting methylation mapping and extraction...")
        ...
        for f in os.listdir(self.config.raw_dir):
            if f.endswith(r1_pattern):
                ...
                if os.path.exists(r2_path):
                    ...
                    self.logger.warning(f"   发现R1文件但未找到对应的R2文件|Found R1 but missing R2: {f} (期望|expected: {os.path.basename(r2_path)})")
    ...
    def _organize_and_split_results(self, sample: str, temp_bam_path: Path, sample_tmp_dir: Path, sample_result_dir: Path):
        ...
        if final_cx_report_path:
            ...
            self.logger.warning(f"   未找到CX报告文件进行拆分|CX_report file not found for splitting.")
