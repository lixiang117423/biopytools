...
class PlinkGWASLogger:
    ...
    def __init__(self, output_dir: Path, log_name: str = "plink_analysis.log", log_level=None):
    ...
    def setup_logging(self):
        ...
        # 创建文件handler|Create file handler
        file_handler = logging.FileHandler(str(self.log_file), encoding='utf-8')
        file_handler.setLevel(logging.DEBUG)
        file_handler.setFormatter(formatter)

        # 创建stdout handler (INFO及以下)|Create stdout handler (INFO and below)
        stdout_handler = logging.StreamHandler(sys.stdout)
        stdout_handler.setLevel(logging.DEBUG)
        stdout_handler.addFilter(lambda record: record.levelno <= logging.INFO)
        stdout_handler.setFormatter(formatter)

        # 创建stderr handler (WARNING及以上)|Create stderr handler (WARNING and above)
...
class FileManager:
    ...
    def remove_file(self, file_path: Path):
        ...
        try:
            ...
            if path_obj.exists():
                path_obj.unlink()
                self.logger.info(f"文件已删除|File removed: {file_path}")
        except Exception as e:
            self.logger.warning(f"删除文件失败|Failed to remove file {file_path}: {e}")
    ...
    def list_files(self, directory: Path, pattern: str = "*") -> list:
        ...
        try:
            ...
            self.logger.warning(f" 列出文件失败|Failed to list files in {directory}: {e}")
            return []