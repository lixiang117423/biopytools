...
class PhenotypeProcessor:
    ...
    def convert_phenotype(self):
        ...
        try:
            # 先尝试自动检测分隔符
            pheno_df = pd.read_csv("phenotype.txt", sep=None, engine='python')
            self.logger.info(f"成功读取表型文件|Successfully read phenotype file: {pheno_df.shape}")
            
            # 如果还是只有1列，尝试空格分隔
            if pheno_df.shape[1] < 2:
                pheno_df = pd.read_csv("phenotype.txt", sep=' ')
                self.logger.info(f"使用空格分隔重新读取|Re-read with space separator: {pheno_df.shape}")
...
class QualityController:
    ...
    def quality_control(self):
        ...
        # 条件性添加质控参数（只添加非None的参数）| Conditionally add QC parameters (only add non-None parameters)
        if self.config.mind is not None:
            cmd.extend(["--mind", str(self.config.mind)])

        cmd.extend(["--geno", str(self.config.geno)])
        cmd.extend(["--maf", str(self.config.maf)])

        if self.config.hwe is not None:
