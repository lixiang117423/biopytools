...
class AssociationAnalyzer:
    ...
    def _run_all_models_analysis(self, use_pca: bool, analysis_type: str) -> str:
        ...
        # 对于数量性状，--model参数不可用，需要分别运行三种模型
        # For quantitative traits, --model is not available, need to run three models separately
        if analysis_type == "linear":
            return self._run_all_models_linear(use_pca)
        else:
            return self._run_all_models_logistic(use_pca)

    def _run_all_models_linear(self, use_pca: bool) -> str:
        """运行所有遗传模型的线性回归分析|Run all genetic models linear regression analysis"""
        ...
        # 为每个模型运行分析|Run analysis for each model
        for model_name, model_params in models:
            self.logger.info(f"  运行{model_name}模型|Running {model_name} model...")
            ...
            self.cmd_runner.run(base_cmd, f"基本关联分析（{model_name}模型）|Basic association analysis ({model_name} model)")
        ...
        if use_pca and Path("pca_with_header.txt").exists():
            ...
            for model_name, model_params in models:
                self.logger.info(f"  运行{model_name}模型（PCA校正）|Running {model_name} model (PCA-adjusted)...")
                ...
                pca_cmd = [
                    ...
                    "--allow-extra-chr",
                    "--allow-no-sex",
                    "--out", f"gwas_adjusted_{model_name}"
                ]
                ...
                self.cmd_runner.run(pca_cmd, f"主成分校正关联分析（{model_name}模型）|PCA-adjusted association analysis ({model_name} model)", check=False)

            # 检查是否至少有一个模型成功|Check if at least one model succeeded
            if Path("gwas_adjusted_additive.assoc.linear").exists():
    ...
    def _run_all_models_logistic(self, use_pca: bool) -> str:
        """运行所有遗传模型的逻辑回归分析|Run all genetic models logistic regression analysis"""
        self.logger.info("基本关联分析（所有模型，无协变量）|Basic association analysis (all models, no covariates)...")

        # 对于质量性状，可以使用--model参数|For qualitative traits, can use --model parameter
        base_cmd = [
            "plink",
            "--bfile", "data_qc1",
            "--model",
