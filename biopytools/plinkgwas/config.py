...
class PlinkGWASConfig:
    ...
    # 质量控制参数|Quality control parameters
    mind: float = None  # 个体缺失率阈值|Individual missing rate threshold (None=不启用|disabled)
    geno: float = 0.05  # SNP缺失率阈值|SNP missing rate threshold
    maf: float = 0.05   # 最小等位基因频率|Minor allele frequency
    hwe: float = None   # Hardy-Weinberg平衡检验P值阈值|HWE p-value threshold (None=不启用|disabled)
    ...
    # 计算参数|Computing parameters
    threads: int = 64  # 使用的线程数|Number of threads

    # 日志参数|Logging parameters
    verbose: int = 0  # 详细输出级别 (0=WARNING, 1=INFO, 2=DEBUG)|Verbose level
    quiet: bool = False  # 静默模式，只输出ERROR|Quiet mode (only ERROR)
    log_file: str = None  # 日志文件路径|Log file path
    ...
    def validate(self):
        ...
        if self.genetic_model not in ["additive", "dominant", "recessive", "all"]:
            errors.append(f"遗传模型必须是 'additive', 'dominant', 'recessive' 或 'all'|Genetic model must be 'additive', 'dominant', 'recessive' or 'all': {self.genetic_model}")
        
        if self.correction_method not in ["bonferroni", "suggestive", "fdr", "all"]:
            errors.append(f"校正方法必须是 'bonferroni', 'suggestive', 'fdr' 或 'all'|Correction method must be 'bonferroni', 'suggestive', 'fdr' or 'all': {self.correction_method}")

        # 验证mind参数（允许None）| Validate mind parameter (allow None)
        if self.mind is not None and not (0 <= self.mind <= 1):
            errors.append(f"个体缺失率阈值必须在0-1之间|Individual missing rate threshold must be between 0-1: {self.mind}")

        if not (0 <= self.geno <= 1):
            errors.append(f"SNP缺失率阈值必须在0-1之间|SNP missing rate threshold must be between 0-1: {self.geno}")

        if not (0 <= self.maf <= 0.5):
            errors.append(f"最小等位基因频率必须在0-0.5之间|Minor allele frequency must be between 0-0.5: {self.maf}")

        # 验证hwe参数（允许None）| Validate hwe parameter (allow None)
        if self.hwe is not None and not (0 < self.hwe <= 1):
            errors.append(f"Hardy-Weinberg平衡P值阈值必须在0-1之间|HWE p-value threshold must be between 0-1: {self.hwe}")
