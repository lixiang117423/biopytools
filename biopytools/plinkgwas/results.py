...
#             else:
#                 self.logger.warning(" 加性模型无有效结果|No valid additive model results")
...
#             else:
#                 self.logger.warning(" 显性模型无有效结果|No valid dominant model results")
...
#             else:
#                 self.logger.warning(" 基因型模型无有效结果|No valid genotypic model results")
...
#         else:
#             self.logger.warning(" 未找到有效的模型结果|No valid model results found")
...
#         else:
#             self.logger.warning(f" {self.config.genetic_model}模型无有效结果|No valid {self.config.genetic_model} model results")
#             return {}
...
#         """FDR校正|FDR correction"""
#         if not HAS_SCIPY:
#             self.logger.warning(" scipy未安装，跳过FDR校正|scipy not installed, skipping FDR correction")
#             return
...
#             self.logger.warning(" 无结果数据，跳过可视化|No result data, skipping visualization")
#             return
...
#         except Exception as e:
#             self.logger.warning(f" {model_name}模型图表生成失败|Plot generation failed for {model_name}: {e}")
...
class ResultsProcessor:
    ...
    def _process_all_models_results(self, main_result: str) -> dict:
        """处理所有遗传模型的结果|Process results for all genetic models"""
        self.logger.info(" 处理所有遗传模型结果|Processing all genetic models results...")

        # 对于数量性状，需要分别处理三个独立的模型文件
        # For quantitative traits, need to process three separate model files
        if self.config.trait_type == "quantitative":
            return self._process_all_models_results_quantitative(main_result)
        else:
            return self._process_all_models_results_qualitative(main_result)

    def _process_all_models_results_quantitative(self, main_result: str) -> dict:
        """处理数量性状所有遗传模型的结果（三个独立的.assoc.linear文件）|Process all genetic models results for quantitative traits"""
        self.logger.info(" 处理数量性状所有遗传模型结果|Processing all genetic models results for quantitative traits...")

        model_results = {}
        models = ['additive', 'dominant', 'recessive']

        for model_name in models:
            result_file = f"{main_result}_{model_name}.assoc.linear"

            if not os.path.exists(result_file):
                self.logger.warning(f" 结果文件不存在|Result file does not exist: {result_file}, 跳过|skipping")
                continue

            try:
                model_df = pd.read_csv(result_file, sep=r"\s+")
                self.logger.info(f" 成功读取{model_name}模型结果文件|Successfully read {model_name} model results: {model_df.shape}")
            except Exception as e:
                self.logger.error(f" 读取{model_name}模型结果文件失败|Failed to read {model_name} model results: {e}")
                continue

            # 健壮地创建 BP 列
            model_df = self._ensure_bp_column(model_df)
            ...
            if not model_df.empty:
                ...
                self.logger.info(f" {model_name.capitalize()}模型结果: {len(model_df)} SNPs|{model_name.capitalize()} model results: {len(model_df)} SNPs")

        if model_results:
            self._generate_model_comparison_report(model_results)
        else:
            self.logger.error(" 所有遗传模型结果文件均不存在或读取失败|All genetic model result files do not exist or failed to read")
            raise FileNotFoundError("所有遗传模型结果文件均不存在或读取失败")

        return model_results

    def _process_all_models_results_qualitative(self, main_result: str) -> dict:
        """处理质量性状所有遗传模型的结果（.model文件）|Process all genetic models results for qualitative traits (.model file)"""
        self.logger.info(" 处理质量性状所有遗传模型结果|Processing all genetic models results for qualitative traits...")
    ...
    def _process_single_model_results(self, main_result: str) -> dict:
        ...
        if not model_results_df.empty:
            ...
            self.logger.warning(f" {self.config.genetic_model}模型无有效结果|No valid results for {self.config.genetic_model} model")
            return {}
    ...
    def _ensure_bp_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.warning(" 结果文件中缺少BP列，尝试从SNP名称提取位置信息 (格式: CHR:BP)|Result file is missing BP column, attempting to extract position from SNP names (format: CHR:BP)")
        if not df.empty and df['SNP'].astype(str).str.contains(':').any():
            try:
                ...
                self.logger.info(" 成功从SNP名称提取BP位置信息|Successfully extracted BP position from SNP names")
            except Exception as e:
                self.logger.warning(f" 无法从SNP名称提取BP信息: {e}。BP列将填充为0。|Failed to extract BP info from SNP names: {e}. BP column will be filled with 0.")
                df['BP'] = 0
        else:
            self.logger.warning(" SNP列不包含':'分隔符，无法提取位置。BP列将填充为0。|SNP column does not contain ':' separator, cannot extract position. BP column will be filled with 0.")
            df['BP'] = 0
        return df
    ...
    def _fdr_correction(self, df: pd.DataFrame, model_name: str):
        ...
        if not HAS_STATSMODELS:
            self.logger.warning(" 未安装statsmodels库，跳过FDR校正|statsmodels library not installed, skipping FDR correction.")
            return

        try:
            p_values = df['P'].dropna().values
            if len(p_values) == 0:
                self.logger.warning(f" {model_name}模型没有有效的P值进行FDR校正|No valid p-values for FDR correction in {model_name} model")
                return
...
#         self.logger.info(" 生成可视化图表|Generating visualization plots...")
#         if not model_results:
#             self.logger.warning(" 无结果数据，跳过可视化|No results data, skipping visualization.")
#             return
...
#         except Exception as e:
#             self.logger.warning(f" {model_name}模型图表生成失败|Plot generation failed for {model_name} model: {e}")
...
class VisualizationGenerator:
    ...
    def _create_manhattan_plot(self, df: pd.DataFrame, model_name: str):
        ...
        try:
            ...
            if len(data) == 0:
                self.logger.warning(f" {model_name}模型无有效P值数据|No valid P-value data for {model_name} model")
                return
            ...
            # 计算累积位置（按数字顺序）| Calculate cumulative positions (in numeric order)
            chr_lengths = data.groupby(['CHR_str', 'CHR_num'])['BP'].max().reset_index()
            ...
            # 计算染色体中心位置用于x轴标签（按数字顺序）| Calculate chromosome center positions for x-axis labels (in numeric order)
            chr_centers = data.groupby(['CHR_str', 'CHR_num'])['bp_cum'].mean().reset_index()
            chr_centers = chr_centers.sort_values('CHR_num')
            
            # 创建图形|Create figure
            fig, ax = plt.subplots(figsize=(14, 7))

            # 为每个染色体分配颜色（按数字顺序）| Assign colors for each chromosome (in numeric order)
            unique_chrs = data.drop_duplicates(['CHR_num']).sort_values('CHR_num')['CHR_str'].tolist()
            colors = ['#1F77B4FF', '#FF7F0EFF'] * (len(unique_chrs) // 2 + 1)

            # 绘制散点图（按数字顺序）| Plot scatter points (in numeric order)
            ...
            ax.set_xticklabels(chr_centers['CHR_str'])
    ...
    def _create_effect_size_comparison(self, model_results: dict):
        ...
        try:
            ...
            if effect_sizes:
                ...
                self.logger.warning(" 无有效的效应大小数据用于比较|No valid effect size data for comparison")
