"""
VCF统计模块|VCF Statistics Module
"""

import subprocess
from pathlib import Path


class VCFStatistics:
    """VCF文件统计器|VCF File Statistics"""

    def __init__(self, config, logger):
        """
        初始化统计器|Initialize statistics generator

        Args:
            config: 配置对象|Configuration object
            logger: 日志对象|Logger object
        """
        self.config = config
        self.logger = logger

    def count_variants(self, vcf_file_name: Path) -> int:
        """
        统计变异数量|Count variants

        Args:
            vcf_file_name: VCF文件名|VCF file name

        Returns:
            变异数量|Variant count
        """
        try:
            # 构建文件的完整路径|Construct the full path to the file
            full_vcf_path = self.config.output_path / vcf_file_name

            # 在执行命令前，先检查文件是否存在，增加健壮性
            # Check file existence before executing command for robustness
            if not full_vcf_path.exists():
                self.logger.warning(f"统计时文件未找到|File not found during statistics: {full_vcf_path}")
                return 0

            # 使用文件的完整路径执行命令|Execute command with full file path
            cmd = f"{self.config.bcftools_path} view -H {full_vcf_path}|wc -l"

            result = subprocess.run(
                cmd,
                shell=True,
                capture_output=True,
                text=True,
                check=True
            )
            count = int(result.stdout.strip())
            return count
        except Exception as e:
            self.logger.error(f"统计失败|Count failed for file {vcf_file_name}: {e}")
            return 0

    def generate_statistics_report(self, separator, filter_obj) -> bool:
        """
        生成统计报告|Generate statistics report

        Args:
            separator: 分离器对象|Separator object
            filter_obj: 过滤器对象|Filter object

        Returns:
            生成是否成功|Whether generation succeeded
        """
        self.logger.info("=" * 60)
        self.logger.info("步骤4: 生成统计报告|Step 4: Generating statistics report")
        self.logger.info("=" * 60)

        # 统计各个文件的变异数量|Count variants in each file
        stats = {}

        # 根据variant_type决定统计哪些文件|Count based on variant_type
        if self.config.variant_type in ['both', 'snp_only']:
            stats["原始SNP|Raw SNPs"] = self.count_variants(separator.raw_snp_file)
            stats["过滤后SNP|Filtered SNPs"] = self.count_variants(filter_obj.filtered_snp_file)

        if self.config.variant_type in ['both', 'indel_only']:
            stats["原始INDEL|Raw INDELs"] = self.count_variants(separator.raw_indel_file)
            stats["过滤后INDEL|Filtered INDELs"] = self.count_variants(filter_obj.filtered_indel_file)

        # 统计双等位位点SNP|Count biallelic SNPs
        if self.config.snp_biallelic and self.config.variant_type in ['both', 'snp_only']:
            stats["双等位位点SNP|Biallelic SNPs"] = self.count_variants(filter_obj.biallelic_snp_file)

        # 合并后变异（仅当variant_type为both时才有merged文件）|Merged variants (only when variant_type is both)
        if self.config.variant_type == 'both':
            stats["合并后变异|Merged variants"] = self.count_variants(filter_obj.merged_file)

        # 计算过滤率|Calculate filtering rates
        snp_rate = 0.0
        indel_rate = 0.0
        biallelic_rate = 0.0

        if "原始SNP|Raw SNPs" in stats and stats["原始SNP|Raw SNPs"] > 0:
            snp_rate = (stats["过滤后SNP|Filtered SNPs"] / stats["原始SNP|Raw SNPs"]) * 100

        if "原始INDEL|Raw INDELs" in stats and stats["原始INDEL|Raw INDELs"] > 0:
            indel_rate = (stats["过滤后INDEL|Filtered INDELs"] / stats["原始INDEL|Raw INDELs"]) * 100

        # 计算双等位位点保留率|Calculate biallelic retention rate
        if "双等位位点SNP|Biallelic SNPs" in stats and stats["过滤后SNP|Filtered SNPs"] > 0:
            biallelic_rate = (stats["双等位位点SNP|Biallelic SNPs"] / stats["过滤后SNP|Filtered SNPs"]) * 100

        # 生成报告文件|Generate report file
        report_file = self.config.output_path / "filtering_statistics.txt"

        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("VCF SNP/INDEL 过滤统计报告|VCF SNP/INDEL Filtering Statistics Report\n")
            f.write("=" * 80 + "\n\n")

            f.write("输入文件|Input file:\n")
            f.write(f"   {self.config.vcf_file}\n\n")

            f.write("输出目录|Output directory:\n")
            f.write(f"   {self.config.output_dir}\n\n")

            f.write("变异类型|Variant type:\n")
            f.write(f"   {self.config.variant_type}\n\n")

            f.write("=" * 80 + "\n")
            f.write("变异统计|Variant Statistics\n")
            f.write("=" * 80 + "\n\n")

            # SNP统计|SNP statistics
            if "原始SNP|Raw SNPs" in stats:
                f.write(f"原始SNP数量|Raw SNPs:              {stats['原始SNP|Raw SNPs']:>12,}\n")
                f.write(f"过滤后SNP数量|Filtered SNPs:       {stats['过滤后SNP|Filtered SNPs']:>12,}\n")
                f.write(f"SNP保留率|SNP retention rate:    {snp_rate:>11.2f}%\n\n")

                # 双等位位点统计|Biallelic statistics
                if self.config.snp_biallelic and "双等位位点SNP|Biallelic SNPs" in stats:
                    f.write(f"双等位位点SNP数量|Biallelic SNPs:  {stats['双等位位点SNP|Biallelic SNPs']:>12,}\n")
                    f.write(f"双等位位点保留率|Biallelic rate: {biallelic_rate:>11.2f}%\n\n")

            # INDEL统计|INDEL statistics
            if "原始INDEL|Raw INDELs" in stats:
                f.write(f"原始INDEL数量|Raw INDELs:          {stats['原始INDEL|Raw INDELs']:>12,}\n")
                f.write(f"过滤后INDEL数量|Filtered INDELs:   {stats['过滤后INDEL|Filtered INDELs']:>12,}\n")
                f.write(f"INDEL保留率|INDEL retention rate: {indel_rate:>11.2f}%\n\n")

            # 合并后统计（仅当variant_type为both时）|Merged statistics (only when variant_type is both)
            if self.config.variant_type == 'both' and "合并后变异|Merged variants" in stats:
                f.write(f"合并后变异总数|Total merged:       {stats['合并后变异|Merged variants']:>12,}\n\n")

            f.write("=" * 80 + "\n")
            f.write("过滤参数|Filtering Parameters\n")
            f.write("=" * 80 + "\n\n")

            f.write("SNP过滤参数|SNP Filtering Parameters:\n")
            f.write(f"  • 最小质量值|Min QUAL:              {self.config.snp_qual}\n")
            f.write(f"  • 最小深度|Min DP:                  {self.config.snp_dp}\n")
            f.write(f"  • 最小比对质量|Min MQ:              {self.config.snp_mq}\n")
            f.write(f"  • 质量/深度比|Min QD:               {self.config.snp_qd}\n")
            f.write(f"  • 最大FisherStrand|Max FS:          {self.config.snp_fs}\n")
            f.write(f"  • 最大SOR|Max SOR:                  {self.config.snp_sor}\n")
            f.write(f"  • 最小MQRankSum|Min MQRankSum:      {self.config.snp_mqrs}\n")
            f.write(f"  • 最小ReadPosRankSum|Min ReadPosRankSum: {self.config.snp_rprs}\n")
            f.write(f"  • 最小MAF|Min MAF:                  {self.config.snp_maf}\n")
            f.write(f"  • 双等位位点过滤|Biallelic filter:  {'启用|Enabled' if self.config.snp_biallelic else '禁用|Disabled'}\n\n")

            f.write("INDEL过滤参数|INDEL Filtering Parameters:\n")
            f.write(f"  • 最小质量值|Min QUAL:              {self.config.indel_qual}\n")
            f.write(f"  • 最小深度|Min DP:                  {self.config.indel_dp}\n")
            f.write(f"  • 最小比对质量|Min MQ:              {self.config.indel_mq}\n")
            f.write(f"  • 质量/深度比|Min QD:               {self.config.indel_qd}\n")
            f.write(f"  • 最大FisherStrand|Max FS:          {self.config.indel_fs}\n")
            f.write(f"  • 最大SOR|Max SOR:                  {self.config.indel_sor}\n")
            f.write(f"  • 最小ReadPosRankSum|Min ReadPosRankSum: {self.config.indel_rprs}\n\n")

            f.write("=" * 80 + "\n")
            f.write("输出文件列表|Output Files\n")
            f.write("=" * 80 + "\n\n")

            file_num = 1
            # SNP文件|SNP files
            if self.config.variant_type in ['both', 'snp_only']:
                f.write(f"{file_num}. 原始SNP文件|Raw SNP file:\n")
                f.write(f"   {separator.raw_snp_file.name}\n\n")
                file_num += 1

                f.write(f"{file_num}. 过滤后SNP文件|Filtered SNP file:\n")
                f.write(f"   {filter_obj.filtered_snp_file.name}\n\n")
                file_num += 1

                # 双等位位点SNP文件|Biallelic SNP file
                if self.config.snp_biallelic:
                    f.write(f"{file_num}. 双等位位点SNP文件|Biallelic SNP file:\n")
                    f.write(f"   {filter_obj.biallelic_snp_file.name}\n\n")
                    file_num += 1

            # INDEL文件|INDEL files
            if self.config.variant_type in ['both', 'indel_only']:
                f.write(f"{file_num}. 原始INDEL文件|Raw INDEL file:\n")
                f.write(f"   {separator.raw_indel_file.name}\n\n")
                file_num += 1

                f.write(f"{file_num}. 过滤后INDEL文件|Filtered INDEL file:\n")
                f.write(f"   {filter_obj.filtered_indel_file.name}\n\n")
                file_num += 1

            # 合并后文件（仅当variant_type为both时）|Merged file (only when variant_type is both)
            if self.config.variant_type == 'both':
                f.write(f"{file_num}. 合并后文件|Merged file:\n")
                f.write(f"   {filter_obj.merged_file.name}\n\n")

            f.write("=" * 80 + "\n")

        # 打印到日志|Print to log
        self.logger.info("=" * 60)
        self.logger.info("过滤统计|Filtering Statistics")
        self.logger.info("=" * 60)

        for key, value in stats.items():
            self.logger.info(f"{key}: {value:,}")

        if "原始SNP|Raw SNPs" in stats:
            self.logger.info(f"SNP保留率|SNP retention rate: {snp_rate:.2f}%")

        # 打印双等位位点保留率|Print biallelic retention rate
        if self.config.snp_biallelic and "双等位位点SNP|Biallelic SNPs" in stats:
            self.logger.info(f"双等位位点保留率|Biallelic retention rate: {biallelic_rate:.2f}%")

        if "原始INDEL|Raw INDELs" in stats:
            self.logger.info(f"INDEL保留率|INDEL retention rate: {indel_rate:.2f}%")

        self.logger.info(f"统计报告已保存|Statistics report saved: {report_file}")

        return True
