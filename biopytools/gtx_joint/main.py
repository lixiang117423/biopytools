...
class GTXJointGenerator:
    ...
    # 单机模式样品阈值|Standalone mode sample threshold
    SINGLE_MODE_THRESHOLD = 200
    ...
    def generate_commands(self):
        ...
        try:
            ...
            # 确保参考基因组索引存在|Ensure reference index exists
            self.logger.info("=" * 60)
            if not self.scanner.ensure_reference_index(self.config.reference):
                self.logger.error("无法创建参考基因组索引|Failed to create reference index")
                return False
            ...
            if self.config.use_faketime:
                ...
                self.logger.warning(f"faketime 未找到，将不使用 faketime|faketime not found")
            ...
            # 扫描GVCF文件并验证/重建索引|Scan GVCF files and validate/rebuild indexes
            self.config.gvcf_files = self.scanner.scan_and_validate_gvcf_files(
                self.config.gvcf_dir,
            ...
            # 判断使用单机模式还是批处理模式|Decide between standalone and batch mode
            sample_count = len(self.config.gvcf_files)
            use_single_mode = sample_count <= self.SINGLE_MODE_THRESHOLD

            if use_single_mode:
                self.logger.info("=" * 60)
                self.logger.info(f"样品数量 {sample_count} <= {self.SINGLE_MODE_THRESHOLD}|"
                               f"Sample count {sample_count} <= {self.SINGLE_MODE_THRESHOLD}")
                self.logger.info("使用单机模式|Using standalone mode")
                self.logger.info("=" * 60)
                success = self._generate_single_mode(script_path)
            elif self.config.window_size:
    ...
    def _generate_single_mode(self, script_path: str) -> bool:
        """
        单机模式生成命令（少量样品）|Standalone mode command generation (few samples)
        ...
        Returns:
            是否成功|Whether successful
        """
        self.logger.info("正在生成单机模式命令...|Generating standalone mode command...")

        output_file = os.path.join(self.config.output_dir, "gtx_joint_raw.vcf.gz")
        ...
        # 生成单个命令处理所有染色体|Generate single command for all chromosomes
        command = (
        ...
        self.logger.info(f"单机模式命令已生成|Standalone mode command generated")
    ...
    def _generate_by_windows(self, script_path: str) -> bool:
        ...
        for chr_name in self.config.chromosomes:
            if chr_name not in chr_lengths:
                self.logger.warning(f"无法获取 {chr_name} 的长度，跳过|Cannot get length for {chr_name}, skipping")
                continue
    ...
    def _show_usage_guide(self, script_path: str):
        ...
        sample_count = len(self.config.gvcf_files)
        is_single_mode = sample_count <= self.SINGLE_MODE_THRESHOLD

        self.logger.info("=" * 60)

        if is_single_mode:
            self.logger.info("单机模式执行|Standalone mode execution:")
            self.logger.info("=" * 60)
            self.logger.info("直接运行脚本即可|Run the script directly:")
            self.logger.info(f"   bash {script_path}")
            self.logger.info("")
        else:
            self.logger.info("执行方式|Execution methods:")
            self.logger.info("=" * 60)
            self.logger.info("串行执行|Serial execution:")
            self.logger.info(f"   bash {script_path}")
            self.logger.info("")
            self.logger.info("GNU Parallel 并行|Parallel execution:")
...
示例|Examples:
  %(prog)s -g /path/to/gtx -r genome.fa -i ./gvcf -o ./output
    ...
    required.add_argument('-g', '--gtx', default='~/software/gtx/bin/gtx',
