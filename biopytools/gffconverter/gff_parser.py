        ...
        with open(self.config.input_file, 'r') as f:
            ...
            for line in f:
                ...
                if not GFFValidator.is_valid_gff_line(line):
                    self.logger.warning(f"  跳过无效行|Skipping invalid line {line_count}: {line}")
                    continue
    ...
    def _parse_feature_line(self, line: str) -> Optional[GFFFeature]:
        ...
        try:
            ...
            if not GFFValidator.validate_coordinates(start, end):
                self.logger.warning(f"  无效坐标|Invalid coordinates: {start}-{end}")
                return None
        except ValueError:
            self.logger.warning(f"  坐标格式错误|Invalid coordinate format: {fields[3]}-{fields[4]}")
            return None
