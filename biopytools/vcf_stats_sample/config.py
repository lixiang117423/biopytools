...
class VCFStatsConfig:
    ...
    def __post_init__(self):
        """⚙️ 初始化后处理 | Post-initialization processing"""
    ...
    def validate(self):
        ...
        if not os.path.exists(self.vcf_file):
            errors.append(f"⚠️ VCF文件不存在 | VCF file does not exist: {self.vcf_file}")
        
        # 🔍 检查参数范围 | Check parameter ranges
        if self.min_depth < 0:
            errors.append(f"⚠️ 最小深度必须非负 | Minimum depth must be non-negative: {self.min_depth}")
        
        if self.min_qual < 0:
            errors.append(f"⚠️ 最小质量分数必须非负 | Minimum quality score must be non-negative: {self.min_qual}")
