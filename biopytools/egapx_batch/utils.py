...
class GenomeSplitter:
    ...
    def split_genome(self, genome_file: str, output_dir: str, chr_prefix: str = None) -> List[Tuple[str, int]]:
        """
        拆分基因组文件并统计序列长度|Split genome file and calculate sequence lengths
        ...
        Returns:
            (文件路径, 序列长度)列表|List of (file path, sequence length)
        """
        ...
        sizes_file = os.path.join(output_dir, '_seq_sizes.tsv')

        awk_script = f'''
/^>/ {{
    if (filename != "") {{
        print filename "\\t" seq_len >> "{sizes_file}"
    }}
    id = substr($1, 2)
...
export JAVA_HOME=~/miniforge3/envs/EGAPx_v.0.4.0-alpha
...
class JobGenerator:
    ...
    def generate_yaml(self, genome: str, locus_tag_prefix: str,
                      ...
                      long_reads: str = "", chr_size: int = 0) -> str:
        """
        ...
        Args:
            ...
            chr_size: 染色体长度(用于设置bam_bin参数)|Chromosome length for bam_bin params

        Returns:
            生成的YAML内容|Generated YAML content
        """
        lines = [f"genome: {genome}", f"taxid: {taxid}"]
        if short_reads:
            lines.append(f"short_reads: {short_reads}")
        if long_reads:
            lines.append(f"long_reads: {long_reads}")
        lines.append(f"locus_tag_prefix: {locus_tag_prefix}")
        lines.append("nodes: 1")
        lines.append("threads: 88")
        if chr_size > 0:
            lines.append("tasks:")
            lines.append("  bam_bin_and_sort:")
            lines.append("    bam_bin: \"-avg-size-per-bin 1000000000000\"")
        return "\n".join(lines)
