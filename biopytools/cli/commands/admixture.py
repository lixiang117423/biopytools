...
@click.option('--missing', '-M',
              type=float,
              default=0.1,
              help='🗑️  缺失率阈值 (默认: 0.1) | Missing rate threshold (default: 0.1)')
@click.option('--hwe', '-H',
              type=float,
              default=1e-6,
...
@click.option('--skip-preprocessing', '-s',
              is_flag=True,
              help='⏭️  跳过VCF预处理 | Skip VCF preprocessing')
@click.option('--keep-intermediate', '-i',
              is_flag=True,
              help='💾 保留中间文件 | Keep intermediate files')
def admixture(vcf, output, min_k, max_k, cv_folds, threads, maf, missing,
...
def lazy_command(module_path, main_func_name='main'):
    ...
    Args: 📝
        module_path (str): 🛣️ 模块路径，如 'biopytools.admixture.main'
        main_func_name (str): 🏷️ main函数名称，默认为'main'
    """
    def decorator(func):
        def wrapper(*args, **kwargs):
            # 😴 懒加载模块
