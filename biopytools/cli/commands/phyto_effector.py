...
@click.option('-e', '--evalue', default=1e-5, show_default=True, type=float,
              help='E-value阈值(已弃用)|E-value threshold (deprecated)')
@click.option('--score-threshold', default=0.0, show_default=True, type=float,
              help='HMM score阈值|HMM score threshold')
@click.option('-t', '--threads', default=12, show_default=True, type=int,
              help='线程数|Number of threads')
def phyto_effector(input, output_dir, skip_signalp, signalp_path, organism, signalp_mode,
    ...
    """
    Phytophthora效应子鉴定(RxLR+CRN+NLP+Protease+SCP+elicitin+YxSL)|Phytophthora effector identification

    鉴定类型|Types: RxLR, CRN, NLP, Protease, SCP, elicitin, YxSL
    流程|Pipeline: SignalP信号肽预测 -> 各类型HMM搜索 + 基序注释;多样本输入时自动合并各样本结果。
    Multiple samples (input is a directory) are identified per-sample and merged automatically.
    ...
    if signalp_mode != 'slow-sequential':
