"""
🧬 Fastq到VCF (GTX) 命令 | Fastq to VCF (GTX) Command
基于GTX的重测序全基因组变异检测全流程分析 | Whole genome resequencing variant detection pipeline based on GTX
...
def _lazy_import_fastq2vcf_gtx_main():
    """懒加载fastq2vcf_gtx main函数 | Lazy load fastq2vcf_gtx main function"""
    try:
...
@click.command(short_help="Fastq到VCF (GTX) 全流程分析",
...
@click.option('--no-gtx-wgs',
              is_flag=True,
              help='⏭️ 禁用GTX WGS，使用标准比对流程 | Disable GTX WGS, use standard mapping pipeline')
@click.option('--gtx-pcr-indel-model',
              default='CONSERVATIVE',
              help='🛠️ GTX PCR InDel模型 (默认: CONSERVATIVE) | GTX PCR InDel model (default: CONSERVATIVE)')
@click.option('--gtx-min-confidence',
              type=int,
              default=30,
              help='🎯 GTX最小置信度 (默认: 30) | GTX minimum confidence (default: 30)')
@click.option('--gtx-min-base-qual',
              type=int,
              default=20,
              help='🎯 GTX最小碱基质量 (默认: 20) | GTX minimum base quality (default: 20)')
@click.option('--gtx-ploidy',
              type=int,
              default=2,
              help='🔢 GTX倍性 (默认: 2) | GTX ploidy (default: 2)')
@click.option('--step', '-s',
...
def fastq2vcf_gtx(input, ref_genome, project_base, clean_fastq_dir,
                         ...
                         use_gtx_wgs, no_gtx_wgs, gtx_pcr_indel_model,
                         gtx_min_confidence, gtx_min_base_qual, gtx_ploidy,
                         gtx_bin, step, no_checkpoint, dry_run, verbose,
                         skip_qc, skip_mapping, read1_pattern_fastp, read2_pattern_fastp):
    """
    Fastq到VCF (GTX) 全流程分析工具.

    基于GTX的重测序全基因组变异检测全流程分析，包括质量控制、
    ...
    if not use_gtx_wgs:  # 如果用户明确禁用了GTX WGS
