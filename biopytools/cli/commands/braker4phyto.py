"""
braker4phyto: 疫霉菌端到端注释(annorefine 基底, 默认不屏蔽重复)|Phytophthora end-to-end annotation
"""
...
def _lazy_import_braker4phyto_main():
    """延迟加载主函数|Lazy load main"""
    try:
        from ...braker4phyto.main import main as module_main
        return module_main
...
@click.command(
    short_help='疫霉菌端到端注释(默认不屏蔽重复)|Phytophthora end-to-end annotation (no repeat masking by default)',
...
@click.option('--skip-repeat/--no-skip-repeat', default=True, show_default=True,
              help='跳过repeat屏蔽(默认开, 效应子在repeat区; --no-skip-repeat开启屏蔽)|Skip repeat masking (default on; --no-skip-repeat to enable)')
...
def braker4phyto(genome, species, prot_seq, output_dir, rnaseq_dirs, isoseq,
    ...
    """
    BRAKER 注释 + 查漏补缺 端到端 → 整合 GFF3(默认不屏蔽重复)|End-to-end (no repeat masking by default)

    与 annorefine 流程一致,默认跳过 repeat 屏蔽(疫霉效应子位于repeat区)
    |Same pipeline as annorefine, repeat masking off by default

    示例|Example: biopytools braker4phyto -g genome.fa -s psojae -p prot.fa -o out/
    """
    phyto_main = _lazy_import_braker4phyto_main()
    args = ['braker4phyto.py', '-g', genome, '-s', species, '-p', prot_seq, '-o', output_dir]
    if rnaseq_dirs:
    ...
    # skip_repeat 默认 True,仅在显式重开屏蔽时透传|Default True, forward only when masking re-enabled
    if not skip_repeat:
        args.append('--no-skip-repeat')
