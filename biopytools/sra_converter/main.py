...
class SRAConverter:
    ...
    def run_conversion(self):
        ...
        try:
            ...
            self.logger.info(f"=" * 80)
            self.logger.info(f"⏱️  总耗时 | Duration: {duration:.1f} 秒 | seconds")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例 | Examples:
  # 基本用法 (自动使用parallel-fastq-dump加速) | Basic usage (auto-accelerate with parallel-fastq-dump)
  %(prog)s -i sra_dir/ -o fastq_output
  
  # 指定线程数转换 | Specify threads
  %(prog)s -i sra_files/ -o output/ -t 16 --compress
  
  # 单个文件转换 | Single file conversion
  %(prog)s -i sample.sra -o fastq/ -t 32
  
  # 使用临时目录加速 | Use temp directory for speed
  %(prog)s -i sra/ -o out/ -t 88 --tmpdir /tmp/sra_temp
  
  # 不压缩输出 | No compression
  %(prog)s -i sra_dir/ -o output/ --no-compress
        """
    )
    ...
    parser.add_argument('-t', '--threads', type=int, default=88, 
                       help='🧵 线程数 (parallel-fastq-dump会充分利用多线程) | Number of threads (parallel-fastq-dump fully utilizes multi-threading)')
    
    parser.add_argument('-c', '--compress', action='store_true', default=True,
                       help='🗜️  压缩输出为.gz格式 | Compress output to .gz format')
    parser.add_argument('--no-compress', dest='compress', action='store_false',
                       help='📝 不压缩输出 | Do not compress output')
    
    parser.add_argument('--split', action='store_true', default=True,
                       help='✂️  拆分双端测序文件 | Split paired-end reads')
