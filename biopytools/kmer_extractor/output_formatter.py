...
class KmerOutputFormatter:
    ...
    def _convert_fasta_to_fasta(self, unik_file: str, output_fasta: str, fasta_files: List[str]) -> bool:
        ...
        # 使用 os.path.basename() 获取相对于工作目录的文件名
        unik_file_rel = os.path.basename(unik_file)
        temp_bed_rel = os.path.basename(temp_bed)
        
        locate_cmd = f"{self.config.unikmer_path} locate {unik_file_rel} -g {' '.join(fasta_files)} -o {temp_bed_rel}" # <--- 已修复
    ...
    def generate_bed_file(self, unik_file: str, output_bed: str, fasta_files: List[str]) -> bool:
        ...
        try:
            ...
            # 使用 os.path.basename() 获取相对于工作目录的文件名
            unik_file_rel = os.path.basename(unik_file)
            temp_output_rel = os.path.basename(temp_output)
            
            locate_cmd = f"{self.config.unikmer_path} locate {unik_file_rel} -g {' '.join(fasta_files)} -o {temp_output_rel}" # <--- 已修复
