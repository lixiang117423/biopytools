...
class UnikmrExtractor:
    ...
    def extract_kmers_from_fasta(self, fasta_files: List[str]) -> str:
        ...
        # 准备输出文件路径（供Python内部使用）
        output_prefix_path = self.config.output_path / self.config.base_name
        unik_file = f"{output_prefix_path}.unik"
        ...
        cmd_parts = [
            ...
            f"-k {self.config.kmer_length}",
            f"-j {self.config.threads}",
            # 因为工作目录已设为output_dir，这里只需提供文件前缀
            "-o", self.config.base_name  # <--- 修改这里
        ]
...
class JellyfishExtractor:
    ...
    def extract_kmers_from_fastq(self, merged_r1: str, merged_r2: str = None) -> str:
        ...
        # 准备输出文件路径（供Python内部使用）
        jf_file_full_path = str(self.config.output_path / f"{self.config.base_name}.jf")
        # 准备输出文件名（供命令行使用）
        jf_output_name = f"{self.config.base_name}.jf"
        ...
        cmd_parts = [
            ...
            f"-s {self.config.jellyfish_hash_size}",
            f"-t {self.config.threads}",
            # 因为工作目录已设为output_dir，这里只需提供文件名
            "-o", jf_output_name  # <--- 修改这里
        ]
