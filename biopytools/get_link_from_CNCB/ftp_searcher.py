...
class CNCBFTPSearcher:
    ...
    def __init__(self, ftp_client, base_dirs: List[str], path_cache, logger=None):
        self.ftp = ftp_client
    ...
    def find_base_path_for_id(self, archive: str, run_id: str) -> Optional[str]:
        ...
        for parent in candidate_parents:
            ...
            if cached_path:
                self.logger.debug(f"缓存命中|Cache hit for {parent}: {cached_path}")
                return cached_path

        self.logger.info(f"正在为|Performing full path search for {run_id}...")
        ...
        for parent_pattern in candidate_parents:
            for base_dir in self.base_dirs:
                ...
                try:
                    ...
                    self.logger.debug(f"无法访问目录|Cannot access directory: {level1_path_base}")
                    continue
                ...
                for l1_dir_name in level1_dirs_to_try:
                    ...
                    try:
                        ...
                        self.logger.info(f"路径找到并缓存|Path found and cached! 父目录|Parent directory '{parent_pattern}' 位于|located at: {correct_base_path}")
        ...
        self.logger.warning(f"未能为|Failed to find path for {run_id}")
        return None
    ...
    def find_files_for_run(self, run_id: str, base_path: str,
        ...
        if not parent_dir_name:
            self.logger.error(f"无法确定|Cannot determine parent directory for {run_id}")
            return found_urls

        final_dir_path = f"{base_path}/{parent_dir_name}/{run_id}"
        self.logger.debug(f"搜索目录|Searching directory: {final_dir_path}")
        ...
        for template in filename_templates:
            ...
            try:
                ...
                self.logger.debug(f"找到文件|Found file: {filename} (大小|size: {file_size:,} bytes)")

            except ftplib.error_perm:
                self.logger.debug(f"文件不存在|File not found: {filename}")
                continue
    ...
    def test_ftp_access(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"FTP访问权限测试失败|FTP access test failed: {e}")
            return False
    ...
    def get_server_info(self) -> dict:
        ...
        try:
            ...
            self.logger.error(f"获取FTP服务器信息失败|Failed to get FTP server info: {e}")
            return {}
    ...
    def _test_base_dirs(self) -> List[str]:
        ...
        for base_dir in self.base_dirs:
            try:
                ...
                self.logger.debug(f"基础目录不可访问|Base directory not accessible: {base_dir}")
