...
GSA原生Run ID(CRR前缀)通过NGDC浏览页/HTTPS通道查询(项目列需为CRA编号)。
ENA也无链接的Run ID回退到NCBI SDL API定位真实文件位置。
...
GSA-native run IDs (CRR prefix) are resolved via the NGDC browse page / HTTPS
(project column must be the CRA accession).
