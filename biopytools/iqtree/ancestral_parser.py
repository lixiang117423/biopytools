...
class AncestralStateParser:
    ...
    def parse_ancestral_results(self):
        ...
        if not os.path.exists(self.state_file):
            self.logger.warning(f" 未找到祖先状态文件|Ancestral state file not found: {self.state_file}")
            return False
    ...
    def _find_uncertain_sites(self, threshold=0.95):
        ...
        if len(uncertain) > 0:
            self.logger.info(f" 发现 {len(uncertain)} 个不确定位点|Found {len(uncertain)} uncertain sites")
            ...
            for _, row in top_uncertain.iterrows():
                self.logger.info(f"  • {row['Node']} 位点|site {row['Site']}: {row['State']} (概率|probability: {row['max_prob']:.4f})")
        else:
    ...
    def _analyze_evolution_hotspots(self):
        ...
        self.logger.info("分析演化热点|Analyzing evolution hotspots")
        ...
        if len(hotspots) > 0:
            ...
            self.logger.info("所有位点在所有节点上都是保守的|All sites are conserved across all nodes")
    ...
    def _parse_tree_structure(self):
        ...
        if not os.path.exists(tree_file):
            self.logger.warning(f" 未找到树文件|Tree file not found: {tree_file}")
            return
        
        if not HAS_BIOPYTHON:
            self.logger.warning(" 未安装BioPython，无法解析树结构|BioPython not installed")
            self.logger.info(" 安装方法：|Install: pip install biopython")
            return
        ...
        try:
            ...
            # 显示前几行示例
            self.logger.info(" 前3个物种的路径示例|Path examples for first 3 species:")
    ...
    #         self.logger.warning(" 缺少必需文件，跳过演化历史追溯")
    #         return
    ...
    def _trace_evolutionary_history(self):
        ...
        if not lineage_file.exists() or not seq_file.exists():
            self.logger.warning(" 缺少必需文件，跳过演化历史追溯|Missing required files, skipping evolutionary history trace")
            return
        ...
        try:
            ...
            self.logger.info(f" 开始追溯 {len(df_lineage)} 个物种的演化历史|Tracing evolutionary history for {len(df_lineage)} species")
            ...
            self.logger.info(f" 演化历史CSV已保存|Evolutionary history CSV saved: {trace_file}")
            ...
            self.logger.info(f" 突变树已保存|Mutation tree saved: {tree_viz_file}")
            
            # 统计信息
            total_mutations = df_trace[df_trace['Position'] != ''].shape[0]
            self.logger.info(f" 共记录 {len(all_records)} 条演化事件|Total {len(all_records)} evolutionary events recorded")
            self.logger.info(f" 共记录 {total_mutations} 个突变事件|Total {total_mutations} mutation events recorded")
            
        except Exception as e:
            self.logger.error(f" 追溯演化历史时出错|Error tracing evolutionary history: {e}")
    
    def _compare_sequences(self, seq1, seq2, node1, node2):
        """比较两个序列，找出突变位点|Compare two sequences and find mutation sites"""
        mutations = []
