...
class GenomeScopeRunner:
    ...
    def run_genomescope(self, histo_file: str, kmer_size: int, read_length: int,
        ...
        try:
            ...
            # 构建conda run命令:优先config指定环境(超算保持genomescope_v.2.0.1版本不变)，
            # 该环境无genomescope2时自动检测兜底(WSL自适应)，§13
            # Prefer config env (keeps genomescope_v.2.0.1 on supercomputer); auto-detect fallback (WSL)
            cmd = build_conda_command(
                'genomescope2',
                ['-i', histo_file_abs, '-o', output_dir_abs,
                 '-k', str(kmer_size), '-p', str(ploidy)],
                preferred_env=self.config.genomescope_env
            )
...
class SmudgeplotRunner:
    ...
    def run_fastk(self, fastq_files: list, fastk_table: str,
        ...
        try:
            ...
            #   -T才是线程数;且.smudgeplot hetmers的-t是线程,与FastK相反,勿相互照抄
            # 参数和值需要连在一起，如 -t4 -k21 -M16 -T12
            # FastK -t = table count cutoff (official smudgeplot README uses fixed 4); -T = threads.
            # Values attach directly to flags, e.g. -t4 -k21 -M16 -T12
            cmd = build_conda_command(
                'FastK',
