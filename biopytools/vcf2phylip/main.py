    ...
    def run_conversion(self):
        ...
        try:
            ...
            if self.config.min_samples_locus > num_samples:
                ...
                self.logger.info(f"⚙️ 最小样本数已调整为 | Min samples adjusted to: {num_samples}")
            ...
            self.logger.info("✍️ 开始写入输出矩阵 | Starting to write output matrices")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='🧬 VCF转换工具 (模块化版本) | VCF Converter (Modular Version)\n'
                   '将VCF格式的SNP数据转换为PHYLIP、FASTA、NEXUS或二进制NEXUS格式\n'
                   'Convert VCF format SNPs to PHYLIP, FASTA, NEXUS, or binary NEXUS formats',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
🔬 示例 | Examples:
  %(prog)s -i variants.vcf -o converted_results
  %(prog)s --input data.vcf.gz --output results --fasta --nexus
  %(prog)s -i variants.vcf -o results -m 4 -g outgroup_sample --plot
  %(prog)s -i data.vcf -o results --nexus-binary --resolve-IUPAC --threads 64
        """
    )
    ...
    parser.add_argument('-m', '--min-samples-locus', 
                       type=int, default=4,
                       help='🔢 位点最少样本数 | Minimum samples required per locus')
