                ...
                for line in vcf_chunk:
                    ...
                    if line and not line.startswith("#"):
                        ...
                        if self.parser.is_anomalous(record, num_samples):
                            self.logger.warning(f"⚠️ 跳过格式错误行 | Skipping malformed line: {line[:100]}...")
                            continue
                        ...
                        if self.parser.is_snp(record):
                            ...
                            if temp_file:
                                ...
                                if site_tmp == "malformed":
                                    self.logger.warning(f"⚠️ 跳过格式错误行 | Skipping malformed line")
                                    continue
    ...
    def cleanup_temp_files(self):
        ...
        for temp_file in self.temp_files:
            if os.path.exists(temp_file):
                ...
                self.logger.debug(f"🗑️ 临时文件已删除 | Temporary file deleted: {temp_file}")
