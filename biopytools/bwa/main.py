...
class BWAAligner:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            self.logger.info("📋 步骤 1/6: 检查依赖软件 | Step 1/6: Checking dependencies")
            self.check_dependencies()
            
            # Step 2: 检查并构建基因组索引 | Check and build genome index
            self.logger.info("📋 步骤 2/6: 检查基因组索引 | Step 2/6: Checking genome index")
            ...
            self.logger.info("📋 步骤 3/6: 查找FASTQ文件 | Step 3/6: Finding FASTQ files")
            ...
            self.logger.info(f"📋 步骤 4/6: 批量比对 ({len(fastq_pairs)} 个样品) | "
            ...
            self.logger.info("📋 步骤 6/6: 生成汇总报告 | Step 6/6: Generating summary report")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例 | Examples:
  # 基本比对 | Basic alignment
  python bwa_genome_align.py -g genome.fa -i fastq_dir -p _1.clean.fq.gz
  
  # 完整分析 | Complete analysis
  python bwa_genome_align.py -g genome.fa -i fastq_dir -p _1.clean.fq.gz \\
      -o results --markdup -t 88
  
  # 自定义BWA参数 | Custom BWA parameters
  python bwa_genome_align.py -g genome.fa -i fastq_dir -p _1.clean.fq.gz \\
    ...
    bwa_score.add_argument('--bwa-E', default="1,1", dest='bwa_gap_ext',
                          help='gap延伸罚分 | Gap extension penalty (default: 1,1)')
    bwa_score.add_argument('--bwa-L', default="5,5", dest='bwa_clip',
                          help='末端剪切罚分 | Clipping penalty (default: 5,5)')
