...
class DepthProcessor:
        ...
        try:
            ...
            if region:
                if not self._check_and_create_index(input_file):
                    ...
                    self.logger.warning(f" 索引创建失败，将分析整个BAM文件（可能较慢）|Index creation failed, will analyze entire BAM file (may be slow)")
                    region = ""
            ...
            if result.returncode != 0:
                self.logger.error(f" samtools执行失败: {result.stderr}|samtools execution failed: {result.stderr}")
                return False
            ...
            self.logger.error(f" 处理文件时出错: {e}|Error processing file: {e}")
            return False
    ...
    def _filter_and_append_data(self, temp_file: Path, sample_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f" 过滤数据时出错: {e}|Error filtering data: {e}")
            return False
    ...
    def _check_and_create_index(self, bam_file: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f" 尝试不使用索引进行分析...|Trying analysis without index...")
