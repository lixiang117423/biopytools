...
class ResultsProcessor:
    ...
    def compile_results(self, results_list: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        if not results_list:
            self.logger.warning("⚠️ 没有成功的分析结果 | No successful analysis results")
            return pd.DataFrame()
        ...
        df.columns = [
            ...
            # 'Database | 数据库',
            # 'Complete_% | 完整度%',
            # 'Complete_BUSCOs | 完整基因数',
            # 'Single_Copy_% | 单拷贝%',
            # 'Single_Copy_BUSCOs | 单拷贝基因数',
            # 'Multi_Copy_% | 多拷贝%',
            # 'Multi_Copy_BUSCOs | 多拷贝基因数',
            # 'Fragmented_% | 片段化%',
            # 'Fragmented_BUSCOs | 片段化基因数',
            # 'Missing_% | 缺失%',
            # 'Missing_BUSCOs | 缺失基因数',
            # 'Total_Markers | 标记基因总数',
            # 'Domain | 域',
            # 'Status | 状态'
            '样本名称',
            '数据库',
    ...
    def add_failed_samples(self, df: pd.DataFrame, failed_samples: List[str]) -> pd.DataFrame:
        ...
        self.logger.info(f"⚠️  添加 {len(failed_samples)} 个失败样本 | Adding {len(failed_samples)} failed samples")
        
        # 为失败样本创建行 | Create rows for failed samples
        failed_rows = []
        for sample_name in failed_samples:
            failed_row = {
                'Sample_Name | 样本名称': sample_name,
                'Database | 数据库': self.config.lineage,
                'Complete_% | 完整度%': 'N/A',
                'Complete_BUSCOs | 完整基因数': 'N/A',
                'Single_Copy_% | 单拷贝%': 'N/A',
                'Single_Copy_BUSCOs | 单拷贝基因数': 'N/A',
                'Multi_Copy_% | 多拷贝%': 'N/A',
                'Multi_Copy_BUSCOs | 多拷贝基因数': 'N/A',
                'Fragmented_% | 片段化%': 'N/A',
                'Fragmented_BUSCOs | 片段化基因数': 'N/A',
                'Missing_% | 缺失%': 'N/A',
                'Missing_BUSCOs | 缺失基因数': 'N/A',
                'Total_Markers | 标记基因总数': 'N/A',
                'Domain | 域': 'N/A',
                'Status | 状态': '失败 | Failure'
            }
    ...
    def save_results(self, df: pd.DataFrame, output_filename: str = None):
        ...
        if df.empty:
            self.logger.warning("⚠️ 结果表格为空，跳过保存 | Results table is empty, skipping save")
            return
    ...
    def print_results_summary(self, df: pd.DataFrame):
        ...
        successful_samples = len(df[df['Status | 状态'].str.contains('成功|Success', na=False)])
        ...
        if successful_samples > 0:
            ...
            success_df = df[df['Status | 状态'].str.contains('成功|Success', na=False)]
            ...
            self.logger.info(f"📈 平均完整度 | Average completeness: {success_df['Complete_% | 完整度%'].mean():.2f}%")
            self.logger.info(f"📈 平均单拷贝比例 | Average single-copy ratio: {success_df['Single_Copy_% | 单拷贝%'].mean():.2f}%")
            self.logger.info(f"📈 平均缺失比例 | Average missing ratio: {success_df['Missing_% | 缺失%'].mean():.2f}%")
...
class SummaryGenerator:
    ...
    def generate_summary_report(self, df: pd.DataFrame):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  - CPU线程数 | CPU threads: {self.config.cpu}\n")
