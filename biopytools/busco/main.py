...
class BUSCOAnalyzer:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            self.logger.info(f"🗄️  数据库 | Database: {self.config.lineage}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='🧬 BUSCO质量评估分析脚本 (模块化版本) | BUSCO Quality Assessment Analysis Script (Modular Version)',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog="""
🎯 示例 | Examples:
  %(prog)s -i genome.fa -l brassicales_odb12
  %(prog)s -i sequences/ -l eukaryota_odb12 -m proteins --cpu 64
  %(prog)s -i transcripts.fa -l fungi_odb12 -m transcriptome -o busco_out
  %(prog)s -i proteins/ -l vertebrata_odb12 -m proteins --sample-suffix "*.pep.fa"
        """
    )
    ...
    parser.add_argument('-o', '--output', default='./busco_output', 
    ...
    parser.add_argument('-c', '--cpu', type=int, default=88, 
                       help='⚡ CPU线程数 | Number of CPU threads')
    parser.add_argument('--sample-suffix', default='*.fa',
                       help='🏷️  样本名称提取后缀模式 | Sample name extraction suffix pattern')
