...
class GenomeAligner:
    ...
    def check_alignments_completed(self) -> dict:
        """检查比对是否已完成 | Check if alignments are completed"""
        self.logger.info("🔍 检查现有比对文件 | Checking existing alignment files")
        ...
        for i in range(len(self.config.sample_list) - 1):
            ...
            bam_file = self.alignment_dir / f"{pair_name}.bam"
            bai_file = self.alignment_dir / f"{pair_name}.bam.bai"
            
            if bam_file.exists() and bai_file.exists():
                existing_files[pair_name] = str(bam_file)
                self.logger.info(f"✅ 找到已完成的比对 | Found completed alignment: {pair_name}")
            else:
                self.logger.info(f"❌ 比对文件不存在 | Alignment file missing: {pair_name}")
