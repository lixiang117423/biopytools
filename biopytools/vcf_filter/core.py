...
class VCFFilter:
    ...
    def filter_vcf(self, 
        ...
        if min_depth is not None or max_depth is not None:
            logger.warning("请求了深度过滤，切换到较慢的Python筛选模式。|Depth filtering requested, switching to slower Python filtering mode.")
            return self._filter_with_python(
                chr_name, start, end, output_path, quality_threshold,
                min_depth, max_depth, keep_samples, remove_samples,
                biallelic_only, remove_indels, logger
    ...
    def _filter_with_bcftools_engine(self, chr_name, start, end, output_path,
        ...
        """使用BCFtools作为核心筛选引擎"""
        logger.info("使用BCFtools引擎进行筛选...|Filtering with BCFtools engine...")
        ...
        try:
            ...
            logger.info(f"执行BCFtools命令: {full_command_str}|Executing BCFtools command: {full_command_str}")
            ...
            if last_proc.returncode != 0:
                logger.error(f"BCFtools执行失败，退出码: {last_proc.returncode}\n错误: {stderr}|BCFtools execution failed, exit code: {last_proc.returncode}\nError: {stderr}")
                raise subprocess.CalledProcessError(last_proc.returncode, full_command_str, stderr=stderr)
            
            logger.info("BCFtools筛选完成|BCFtools filtering completed")
            return str(output_path)
        except Exception as e:
            logger.error(f"BCFtools执行过程中出现错误: {e}|Error during BCFtools execution: {e}")
            raise
    ...
    def _filter_with_python(self, chr_name, start, end, output_path, 
                           ...
                           keep_samples, remove_samples, biallelic_only, 
                           remove_indels, logger) -> str:
        """使用Python进行VCF筛选（作为备用方案）|Filter VCF with Python (as fallback)"""

        logger.info("使用Python进行VCF筛选...|Filtering VCF with Python...")
        ...
        logger.info(f"检查染色体 {chr_list} 在位置范围 {start}-{end} 内的变异...|Checking variants for chromosome(s) {chr_list} in position range {start}-{end}...")
