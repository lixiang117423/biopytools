...
class MixraceConfig:
    ...
    kmer_size: int = 21
    read_length: int = 150
    step: Optional[int] = None        # None=全跑|None=all steps, 1..8=单步|single step
    ...
    # step04 过滤|step04 filtering
    min_qual: int = 30
    min_dp: int = 15
    min_alt_reads: int = 3
    # step05 VAF中间频率占比判读阈值|step05 VAF mid-ratio verdict thresholds
    vaf_mid_low: float = 0.05
    vaf_mid_high: float = 0.15
    # step06 多等位判读阈值|step06 multiallelic verdict thresholds
    multiallelic_low: float = 0.005
    multiallelic_high: float = 0.03
    # Fws / 深度|Fws / depth
    fws_cutoff: float = 0.95
    ...
    def validate(self):
        ...
        errors = []
        if not self.fastq_dir or not os.path.isdir(self.fastq_dir):
        ...
        if self.step is not None and not (1 <= self.step <= 8):
            errors.append("step须为1-8|step must be 1-8")
        if errors:
            raise ValueError("\n".join(errors))
