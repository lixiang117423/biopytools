...
class ReferenceIndexer:
    ...
    def _check_bwa_index(self):
        ...
        if missing_indices:
            self.logger.warning(f"  缺少BWA索引文件|Missing BWA index files: {', '.join(missing_indices)}")
    ...
    def _check_samtools_index(self):
        ...
        if not fai_file.exists():
            self.logger.warning("  缺少.fai索引文件|Missing .fai index file")
    ...
    def _check_gatk_dict(self):
        ...
        if not dict_file.exists():
            self.logger.warning("  缺少.dict字典文件|Missing .dict dictionary file")
