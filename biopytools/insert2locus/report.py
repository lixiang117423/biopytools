...
def _sample_section(entry: dict, tool_versions: dict) -> str:
    """单样本报告片段(含样本标题锚点)|Per-sample report fragment (with anchor)"""
    sample = entry["sample"]
    locus = entry.get("locus")
    segments = entry.get("segments") or []
    grade = entry.get("grade", "?")
    lb_junction_reads = entry.get("lb_junction_reads", 0)
    rb_junction_reads = entry.get("rb_junction_reads", 0)
    junctions = entry.get("junctions") or []
    record_depth = entry.get("record_depth") or {}
    warnings = entry.get("warnings") or []
    junction_seqs = entry.get("junction_seqs") or {}
    anchor_label = entry.get("anchor_label")

    if entry.get("error"):
        return (f'<section class="sample" id="s_{_esc(sample)}">'
                f'<h1>{_esc(sample)}</h1>'
                f'<span class="badge FAIL">FAIL</span>'
                f'<div class="callout warn">该样本流程失败|Pipeline failed for '
                f'this sample:<br>{_esc(entry["error"])}</div></section>')
    ...
    if locus:
        cards.append(_seq_card(
            ...
            locus.seq[:locus.lead], f"seq_lb_{sample}"))
        cards.append(_seq_card(
            f"{sample}_RB_right_flank",
            f"{locus.trail} bp 右边界植物侧翼|Right flank",
            locus.seq[len(locus.seq) - locus.trail:], f"seq_rb_{sample}"))
        cards.append(_seq_card(
            f"{sample}_complete_locus",
            f"{len(locus.seq)} bp 左翼+插入+右翼|Full locus",
            locus.seq, f"seq_full_{sample}"))
    else:
    ...
    return f"""
<section class="sample" id="s_{_esc(sample)}">
<h1>{_esc(sample)} 转基因插入位点分析报告</h1>
...
def write_combined_report(entries: List[dict], out_path,
                          tool_versions: Optional[dict] = None) -> Path:
    """单一HTML文件:单样本即完整报告,多样本顶部导航切换|
    One HTML file: full report for one sample; top nav switches samples"""
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    tool_versions = tool_versions or {}
    sections = "".join(_sample_section(e, tool_versions) for e in entries)
    # 多样本(或有失败样本)时显示导航条|Nav bar when >1 entry or failures present
    nav = ""
    if len(entries) > 1:
        chips = "".join(
            f'<a class="chip {e.get("grade", "FAIL")}" href="#s_{_esc(e["sample"])}">'
            f'{_esc(e["sample"])} · {_esc(e.get("grade", "FAIL"))}</a>'
            for e in entries)
        nav = f'<nav class="wrap bar">{chips}</nav>'
    title_sample = (entries[0]["sample"] if len(entries) == 1
                    else f"{len(entries)}个样本|{len(entries)} samples")
    html_doc = f"""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{_esc(title_sample)} 转基因插入位点分析报告</title>
<style>{_STYLE}
.sample{{border-top:4px solid var(--rule);padding-top:24px;margin-top:26px}}
.sample:first-of-type{{border-top:none;margin-top:0}}
.bar{{display:flex;flex-wrap:wrap;gap:8px;padding:12px 0}}
.chip{{text-decoration:none;font-family:monospace;font-size:13px;padding:4px 12px;
border-radius:16px;color:#fff;background:#5C645B}}
.chip.PASS{{background:#2E7D32}}.chip.WARN{{background:#B8860B}}.chip.FAIL{{background:#B3402A}}
</style></head><body>
<header class="wrap"><h1 style="font-size:24px;margin:34px 0 6px">
转基因插入位点分析报告|Insertion locus report</h1>
<p class="meta" style="margin:0 0 6px">由 biopytools insert2locus 生成|
Generated by biopytools insert2locus</p></header>
{nav}<main class="wrap">{sections}</main>
<footer class="wrap"><hr><p>数值均来自实际运行结果|All values come from actual
run outputs</p></footer>
