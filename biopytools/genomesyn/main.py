...
class GenomeSynAnalyzer:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # 处理配置文件生成模式|Handle config generation mode
    ...
    def _generate_config_only(self):
        ...
        self.logger.info("请编辑Excel文件后使用--config参数运行分析|Please edit Excel file and run with --config parameter")
        self.logger.info("=" * 60)
    ...
    def _run_full_analysis(self):
        ...
        # 确定配置数据来源|Determine config data source
        if self.config.config_file:
            ...
            # 从sample_map生成|Generate from sample_map
            samples = self.processor.read_sample_map(self.config.sample_map)
        ...
        # 步骤1: 执行比对|Step 1: Perform alignment
        self.logger.info("=" * 50)
        ...
        # 步骤2: 生成可视化|Step 2: Generate visualization
        self.logger.info("=" * 50)
        ...
        # 步骤3: 生成完整的命令脚本|Step 3: Generate complete command script
        self.logger.info("=" * 50)
    ...
    def _run_alignments(self, config_data: Dict):
        ...
        # 创建比对器时传递正确的线程数和输出目录|Create aligner with correct threads and output dir
        ...
        # 创建基因组查找字典|Create genome lookup dictionary
        genomes_dict = {g["order"]: g for g in config_data["genomes"]}

        # 执行每个链接的比对|Perform alignment for each link
        for link in config_data["links"]:
            genome1 = genomes_dict[link["from"]]
            genome2 = genomes_dict[link["to"]]

            # 生成文件名时考虑染色体过滤|Generate filename considering chromosome filter
            ...
            # 检查文件是否存在且非空|Check if file exists and is non-empty
            skip_alignment = False
            if os.path.exists(link_path) and os.path.getsize(link_path) > 0:
                self.logger.info(f"LINK文件已存在且有效，跳过比对|Valid LINK file exists, skipping: {link_file}")
                skip_alignment = True
            elif os.path.exists(paf_path) and os.path.getsize(paf_path) > 0:
                # PAF 文件存在，尝试转换为 LINK 格式
                if os.path.exists(link_path) and os.path.getsize(link_path) == 0:
                    self.logger.warning(f"发现空的LINK文件，将删除并重新生成|Found empty LINK file, will regenerate: {link_file}")
                    os.remove(link_path)

                # 先尝试 Paf2Link（快速，但不适用于小基因组）
                from subprocess import run
                min_length = config_data["alignment"].get("min_length", 5000)
                cmd_convert = f"GetTwoGenomeSyn.pl Paf2Link \"{os.path.basename(paf_path)}\" {min_length} \"{os.path.basename(link_path)}\""
                result = run(cmd_convert, shell=True, capture_output=True, text=True, cwd=self.config.output_dir)

                # 检查转换是否成功
                if os.path.exists(link_path) and os.path.getsize(link_path) > 0:
                    self.logger.info(f"LINK文件转换成功(Paf2Link)，跳过比对|LINK file conversion successful (Paf2Link), skipping: {link_file}")
                    skip_alignment = True
                else:
                    # Paf2Link 失败，使用完整流程（自动处理小基因组）
                    self.logger.warning(f"PAF到LINK转换失败，使用完整流程重新生成LINK文件|PAF to LINK conversion failed, regenerating with full workflow")

                    # 获取基因组文件路径
                    genome1_file = None
                    genome2_file = None
                    for genome in config_data["genomes"]:
                        if genome["name"] == link['from_name']:
                            genome1_file = genome["file"]
                        if genome["name"] == link['to_name']:
                            genome2_file = genome["file"]

                    if genome1_file and genome2_file:
                        # 使用完整流程，设置较小的 MinLen 阈值
                        cmd_full = f"GetTwoGenomeSyn.pl -InGenomeA \"{genome1_file}\" -InGenomeB \"{genome2_file}\" -OutPrefix \"{os.path.splitext(link_file)[0]}\" -MinLenA 100000 -MinLenB 100000 -MinAlnLen {min_length} -BinDir $(dirname $(which minimap2))"
                        result = run(cmd_full, shell=True, capture_output=True, text=True, cwd=self.config.output_dir)

                        if os.path.exists(link_path) and os.path.getsize(link_path) > 0:
                            self.logger.info(f"LINK文件生成成功(完整流程)，跳过比对|LINK file generated successfully (full workflow), skipping: {link_file}")
                            skip_alignment = True
                        else:
                            self.logger.warning(f"LINK文件生成失败，将重新运行比对|LINK file generation failed, will re-run alignment")
                            if os.path.exists(paf_path):
                                os.remove(paf_path)
                    else:
                        self.logger.warning(f"无法找到基因组文件，将重新运行比对|Cannot find genome files, will re-run alignment")
                        if os.path.exists(paf_path):
                            os.remove(paf_path)
            elif os.path.exists(paf_path) and os.path.getsize(paf_path) == 0:
                self.logger.warning(f"发现空的PAF文件，将重新运行比对|Found empty PAF file, will re-run alignment: {paf_file}")
                os.remove(paf_path)  # 删除空文件|Delete empty file
            elif os.path.exists(link_path) and os.path.getsize(link_path) == 0:
                self.logger.warning(f"发现空的LINK文件，将重新运行比对|Found empty LINK file, will re-run alignment: {link_file}")
    ...
    def _run_visualization(self, config_data: Dict) -> List[str]:
        ...
        # 从第一个 link 中提取文件前缀
        file_prefix = None
        if config_data.get("links"):
            first_link = config_data["links"][0]
            # 前缀格式：{query}_vs_{ref}_
            file_prefix = f"{first_link['to_name']}_vs_{first_link['from_name']}_"
    ...
    def _generate_complete_script(self, config_data: Dict):
        ...
        with open(script_path, 'w') as f:
            ...
            f.write("set -e  #  遇到错误时停止执行|Stop on error\n")
            f.write("set -u  #  使用未定义变量时报错|Error on undefined variables\n\n")
            ...
            for link in config_data["links"]:
                ...
                if genome1_file and genome2_file:
                    ...
                    if self.config.chromosomes:
                        ...
                        # 根据use_minimap2_preset参数决定是否使用-x
                        if self.config.use_minimap2_preset:
                            f.write(f"minimap2 -x asm5 -t $THREADS \"{filtered_genome2}\" \"{filtered_genome1}\" > \"{paf_file}\"\n")
                        else:
                            f.write(f"minimap2 -t $THREADS \"{filtered_genome2}\" \"{filtered_genome1}\" > \"{paf_file}\"\n")
                    else:
                        # 根据use_minimap2_preset参数决定是否使用-x
                        if self.config.use_minimap2_preset:
                            f.write(f"minimap2 -x asm5 -t $THREADS \"{genome2_file}\" \"{genome1_file}\" > \"{paf_file}\"\n")
                        else:
                            f.write(f"minimap2 -t $THREADS \"{genome2_file}\" \"{genome1_file}\" > \"{paf_file}\"\n")
                else:
                    f.write(f"#  注意: 请替换为实际的基因组文件路径\n")
                    f.write(f"# minimap2 -x asm5 -t $THREADS GENOME2_FILE GENOME1_FILE > \"{paf_file}\"\n")
                ...
                f.write("    else\n")
                f.write("        echo \"     警告: PAF转换失败，将使用PAF文件\"\n")
                f.write("    fi\n")
                f.write("else\n")
                f.write("    echo \"     警告: GetTwoGenomeSyn.pl不可用，将使用PAF文件\"\n")
                f.write("fi\n\n")
            ...
            f.write("if [ $? -ne 0 ]; then\n")
            f.write("    echo \" 警告: NGenomeSyn返回非零退出码，但可能已生成文件\"\n")
            f.write("fi\n\n")
            ...
            if "png" in config_data["visualization"]["output_formats"]:
                f.write("echo \"步骤5: 转换为PNG格式|Step 5: Convert to PNG format\"\n")
                ...
                f.write("    else\n")
                f.write("        echo \"     警告: PNG转换失败\"\n")
                f.write("    fi\n")
                f.write("else\n")
                f.write("    echo \"     警告: ImageMagick未安装，跳过PNG转换\"\n")
                f.write("fi\n\n")
        ...
        self.logger.info("您可以使用以下命令重新运行整个分析流程|You can use the following command to re-run the entire analysis pipeline:")
        self.logger.info(f"bash {script_path}")
        self.logger.info("注意: 脚本中的某些步骤需要手动替换实际的输入文件路径|Note: Some steps in the script require manual replacement of actual input file paths")
...
def main():
 ...
 使用示例|Usage Examples:
  ...
  python run_genome_syn.py --sample-map genomes.tsv --output-dir ./output --chromosome "1-5"
    ...
    parser.add_argument('--no-minimap2-preset', action='store_true',
                       help=' 不使用minimap2的-x预设参数|Disable minimap2 -x preset parameter (default: False)')
    ...
    config = GenomeSynConfig(
        ...
        use_minimap2_preset=not args.no_minimap2_preset,  # 反转：--no-minimap2-preset表示不使用
