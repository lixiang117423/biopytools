...
class NGenomeSynVisualizer:
    ...
    def generate_ngenomesyn_config(self, config_data: Dict, output_dir: str, selected_chromosomes: Optional[List[int]] = None) -> str:
        ...
        for link in config_data["links"]:
            ...
            # 添加文件前缀（与 aligners.py 保持一致）
            file_prefix = f"{link['to_name']}_vs_{link['from_name']}_"
            base_name = f"{file_prefix}{link['from_name']}_vs_{link['to_name']}{chr_suffix}"

            # 尝试查找link文件或paf文件
            link_file = f"{base_name}.link"
            paf_file = f"{base_name}.paf"
            ...
            if os.path.exists(link_path) and os.path.getsize(link_path) > 0:
                ...
                self.logger.warning(f" 只找到PAF文件，NGenomeSyn可能需要LINK格式|Only found PAF file, NGenomeSyn may need LINK format: {paf_file}")
        ...
        for genome in config_data["genomes"]:
            ...
            config_lines.append("ChrWidth=20\n")             # 设置染色体高度
            config_lines.append("ChrNameShow=1\n")           # 显示染色体名称
            config_lines.append("ChrNameColor=black\n")      # 染色体名称颜色
            config_lines.append("ChrNameShiftX=0\n")         # X轴偏移
            config_lines.append("ChrNameShiftY=10\n")        # Y轴偏移（垂直居中：ChrWidth/2）

            # 添加区域标注文件配置（仅对第一个基因组）
            if order == 1 and viz_config.get("regions_file"):
                regions_file = viz_config["regions_file"]
                # 复制区域标注文件到输出目录
                import shutil
                regions_basename = os.path.basename(regions_file)
                regions_dest = os.path.join(output_dir, regions_basename)

                # 获取文件的绝对路径进行比较
                regions_file_abs = os.path.abspath(regions_file)
                regions_dest_abs = os.path.abspath(regions_dest)

                if os.path.exists(regions_file):
                    # 只有当源文件和目标文件不是同一个文件时才复制
                    if regions_file_abs != regions_dest_abs:
                        shutil.copy2(regions_file, regions_dest)
                        self.logger.info(f"复制区域标注文件|Copied region annotation file: {regions_basename}")
                    else:
                        self.logger.info(f"区域标注文件已在目标位置|Region annotation file already in place: {regions_basename}")

                    config_lines.append(f"SpeRegionFile={regions_basename}\n")
                    self.logger.info(f"添加区域标注文件|Added region annotation file: {regions_basename}")
                else:
                    self.logger.warning(f"区域标注文件不存在|Region annotation file not found: {regions_file}")
    ...
    def run_ngenomesyn(self, config_file: str, output_dir: str, output_formats: List[str], file_prefix: str = None) -> List[str]:
        ...
        # 确定输出文件前缀
        if file_prefix:
            output_prefix = f"{file_prefix}genome_synteny"
        else:
            # 从配置文件名推断输出文件前缀
        ...
        try:
            ...
            if os.path.exists(svg_file) and os.path.getsize(svg_file) > 0:
                ...
                if result.returncode != 0:
                    self.logger.warning(f" NGenomeSyn返回非零退出码但文件已成功生成|NGenomeSyn returned non-zero exit code but files were generated successfully")
                ...
                if "png" in output_formats:
                    ...
                    self.log_command(
                        command=cmd_convert,
                        working_dir=output_dir,
                        description=f"将SVG转换为PNG格式: {os.path.basename(svg_file)} -> {os.path.basename(png_file)}"
                    )
                    ...
                    try:
                        ...
                        if os.path.exists(png_file) and os.path.getsize(png_file) > 0:
                            ...
                            self.logger.info(f"PNG文件已生成|PNG file generated: {png_file}")
                    except subprocess.CalledProcessError as e:
                        self.logger.warning(f" PNG转换失败|PNG conversion failed: {e}")
