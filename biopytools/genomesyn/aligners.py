...
class Minimap2Aligner(BaseAligner):
    ...
    def align(self, genome1: Dict, genome2: Dict, selected_chromosomes: Optional[List[int]] = None, **params) -> str:
        ...
        use_preset = params.get("use_preset", True)  # 是否使用-x预设参数|Whether to use -x preset
        ...
        if selected_chromosomes:
            ...
            if not os.path.exists(filtered_genome1):
                ...
                self.logger.info(f"使用已存在的过滤文件|Using existing filtered file: {filtered_genome1}")
            ...
            if not os.path.exists(filtered_genome2):
                ...
                self.logger.info(f"使用已存在的过滤文件|Using existing filtered file: {filtered_genome2}")
        ...
        # 生成输出文件的绝对路径，添加前缀
        file_prefix = f"{genome2['name']}_vs_{genome1['name']}_"
        base_name = f"{file_prefix}{genome1['name']}_vs_{genome2['name']}{chr_suffix}"
        paf_file = os.path.abspath(os.path.join(self.output_dir, f"{base_name}.paf"))
        ...
        try:
            # 运行minimap2 - 使用传入的线程数(conda 域环境自动包装, stdout 重定向到 PAF)
            # Run minimap2 with conda domain wrap; stdout redirected to PAF
            minimap2_path = get_domain_tool_path('minimap2', 'minimap2', 'MINIMAP2_PATH')
            minimap2_args = ["-t", str(self.threads)]
            if use_preset:
                minimap2_args = ["-x", preset] + minimap2_args
            minimap2_args += [genome2_file, genome1_file]
            cmd_minimap2 = build_conda_command(minimap2_path, minimap2_args)
            ...
            if os.path.getsize(paf_file) == 0:
                self.logger.warning(f" PAF文件为空，可能没有找到比对结果|PAF file is empty, no alignments found")
            ...
            if self._check_conversion_script():
                ...
                if result.returncode != 0:
                    self.logger.warning(f" PAF转换失败，将使用PAF文件|PAF conversion failed, will use PAF file: {result.stderr}")
                    # 保存命令脚本以便调试
                ...
                self.logger.warning(f" GetTwoGenomeSyn.pl脚本不可用，返回PAF文件|GetTwoGenomeSyn.pl script not available, returning PAF file")
                # 保存命令脚本
                self.save_command_script(f"alignment_commands_{genome1['name']}_vs_{genome2['name']}.sh")
                return paf_file
...
class MummerAligner(BaseAligner):
    ...
    def align(self, genome1: Dict, genome2: Dict, selected_chromosomes: Optional[List[int]] = None, **params) -> str:
        ...
        # 生成输出文件的绝对路径，添加前缀
        file_prefix = f"{genome2['name']}_vs_{genome1['name']}_"
        base_name = f"{file_prefix}{genome1['name']}_vs_{genome2['name']}{chr_suffix}_mummer"
        delta_file = os.path.abspath(os.path.join(self.output_dir, f"{base_name}.delta"))
        coords_file = os.path.abspath(os.path.join(self.output_dir, f"{base_name}.coords"))
        ...
        try:
            ...
            if os.path.getsize(delta_file) == 0:
                self.logger.warning(f" Delta文件为空，可能没有找到比对结果|Delta file is empty, no alignments found")
                # 不抛出异常，因为这可能是正常的生物学现象
            ...
            # Convert to coords format (auto conda domain wrap, stdout redirected)
            if self._check_show_coords():
                ...
                self.logger.warning(f" show-coords工具不可用，返回Delta文件|show-coords tool not available, returning Delta file")
                # 保存命令脚本
                self.save_command_script(f"mummer_commands_{genome1['name']}_vs_{genome2['name']}.sh")
                return delta_file
