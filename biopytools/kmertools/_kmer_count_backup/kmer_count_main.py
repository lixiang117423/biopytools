...
class KmerCountAnalyzer:
    ...
    def _merge_sample_files(self, sample_files: List[Path]) -> pd.DataFrame:
        ...
        self.logger.info("合并所有样本结果（SQLite优化版本）")
            ...
            if self.config.bed_file:
                ...
                for i in range(0, bed_size, batch_size):
                    ...
                    if (i + batch_size) % 1000000 == 0 or i + batch_size >= bed_size:
                        conn.commit()
                        self.logger.info(f"已插入 {min(i + batch_size, bed_size):,} / {bed_size:,} 条BED记录")

                self.logger.info(f"BED数据插入完成，共 {bed_size:,} 条记录")
        ...
        for idx, sample_file in enumerate(sample_files, 1):
            ...
            try:
                ...
                if sample_file.stat().st_size < 10:
                    self.logger.warning(f"样本文件为空: {sample_name}")
                    continue
                ...
                if len(sample_df) == 0:
                    self.logger.warning(f"样本文件无数据: {sample_name}")
                    continue
                ...
                # 批量更新数据库
                self.logger.info(f"[{idx}/{len(sample_files)}] 更新样本: {sample_name} ({len(sample_df):,} 条记录)")
                ...
                self.logger.error(f"处理样本失败: {sample_name}, 错误: {e}")
        ...
        self.logger.info(f"导出 {total_rows:,} 行 × {len(columns)} 列")
        ...
        with open(output_file, 'w') as f:
            ...
            while offset < total_rows:
                ...
                if offset % 1000000 == 0 or offset >= total_rows:
                    self.logger.info(f"已导出 {min(offset, total_rows):,} / {total_rows:,} 行")
        ...
        self.logger.info(f"最终矩阵已保存: {output_file}")
    ...
    def run_analysis(self):
        ...
        try:
            ...
            if progress_mgr.is_step_completed('process_samples'):
                ...
                for sample_name, r1_file, r2_file in samples:
                    try:
                        ...
                        if ("killed by signal 13" in error_msg or 
                            ...
                            self.logger.error(f"样本 {sample_name} 遇到SIGPIPE错误")
                        ...
                        self.logger.warning(f"跳过该样本，继续处理其他样本")
                        continue
                ...
                self.logger.info(f"   跳过样本: {skipped_samples}")
                if skipped_samples > 0:
                    self.logger.warning(f"   跳过比例: {skipped_samples/total_samples:.1%}")
                self.logger.info("="*50)
            ...
            self.logger.info("="*60)
            if skipped_samples > 0:
                self.logger.warning(f"K-mer丰度分析完成，但有{skipped_samples}个样本被跳过")
            else:
                self.logger.info("K-mer丰度分析完成!")
            self.logger.info("="*60)
        ...
        if sample_columns:
            ...
            for sample in sample_columns:
                try:
                    ...
                    self.logger.warning(f" 样本 {sample} 统计计算失败: {e}")
