...
class WindowAnalyzer:
    ...
    #         self.logger.warning(" 滑动窗口分析需要BED文件|Sliding window analysis requires BED file")
    #         return None
        
    #     self.logger.info(f" 开始滑动窗口分析|Starting sliding window analysis")
    #     self.logger.info(f"窗口大小|Window size: {self.config.window_size}bp, 步长|Step size: {self.config.step_size}bp")
    ...
    #         self.logger.info(f" 滑动窗口分析完成，共{len(window_df)}个窗口|Sliding window analysis completed, {len(window_df)} windows in total")
    #         return window_df
    #     else:
    #         self.logger.warning(" 未生成滑动窗口结果|No sliding window results generated")
    #         return None
    ...
    def sliding_window_analysis(self, data_df: pd.DataFrame) -> pd.DataFrame:
        ...
        data_df['start'] = pd.to_numeric(data_df['start'], errors='coerce').astype(int)
        data_df['end'] = pd.to_numeric(data_df['end'], errors='coerce').astype(int)
        ...
        # 转换为0/1矩阵（用于计算存在的k-mer数量）
        for col in sample_columns:
            data_df[col] = (data_df[col] > 0).astype(int)
        ...
        for chrom in data_df['chr'].unique():
            ...
            while window_start <= max_pos:
                ...
                if len(window_kmers) > 0:
                    # 计算每个样本的存在的k-mer数量|Calculate present k-mer count for each sample
                    window_result = {
                        'chr': chrom,
                        'start': window_start,
                        'end': window_end,
                    ...
                    for sample in sample_columns:
                        # 窗口内存在的k-mer数量（0/1矩阵求和）
                        present_count = window_kmers[sample].sum()
                        window_result[sample] = present_count
