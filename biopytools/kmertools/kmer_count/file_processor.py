...
#                 else:
#                     self.logger.warning(f" 软链接目标不存在|Symlink target missing: {file_path.name} -> {file_path.readlink()}")
#             elif file_path.is_file():
#                 self.logger.info(f" 常规文件|Regular file: {file_path.name}")
#                 valid_r1_files.append(str(file_path))
#             else:
#                 self.logger.warning(f" 无效文件|Invalid file: {file_path.name}")
...
#                 self.logger.info(f"找到双端样本|Found paired-end sample: {sample_name}")
...
#         self.logger.warning(" 检测到文件完整性问题，开始重新验证所有文件...")
...
class FileProcessor:
    ...
    def find_fastq_files(self) -> List[tuple]:
        ...
        self.logger.info(f" 输入目录是否存在|Input directory exists: {input_dir.exists()}")

        # 列出目录内容用于调试
        if input_dir.exists():
            files_in_dir = list(input_dir.glob("*"))
            self.logger.info(f" 目录中的文件|Files in directory: {[f.name for f in files_in_dir[:10]]}")
    ...
    def _find_fasta_files(self, input_dir: Path) -> List[tuple]:
        ...
        for file_path in matched_files:
            if file_path.is_symlink():
                if file_path.exists():
                    ...
                    self.logger.warning(f" 软链接目标不存在|Symlink target missing: {file_path.name}")
            elif file_path.is_file():
        ...
        for file_path in valid_files:
            ...
            self.logger.info(f" 添加FASTA样本|Added FASTA sample: {base_name}")
    ...
    def _find_fastq_files_original(self, input_dir: Path) -> List[tuple]:
        ...
        self.logger.info(f" 找到{len(valid_r1_files)}个有效R1文件|Found {len(valid_r1_files)} valid R1 files")
        ...
        for r1_file in valid_r1_files:
            ...
            if r2_file.exists():
                ...
                samples.append((sample_name, r1_file, None))
                self.logger.info(f" 找到单端样本|Found single-end sample: {sample_name}")
                self.logger.info(f"   R1: {r1_file}")
                self.logger.info(f"   R2不存在|R2 not found: {r2_file}")
    ...
    def prepare_kmer_library(self) -> Path:
        ...
        self.logger.info(f"中间文件目录|Intermediate files directory: {intermediate_dir}")

        # 读取原始文件并转换为大写
        upper_kmer_file = intermediate_dir / "kmers_upper.fasta"
        ...
        self.logger.info(f"已保存大写k-mer文件|Saved uppercase k-mer file: {upper_kmer_file}")

        # 扩展k-mer库，添加反向互补序列
        expanded_kmer_file = self._expand_kmer_library_with_rc(upper_kmer_file, intermediate_dir)
        return expanded_kmer_file

    def _expand_kmer_library_with_rc(self, kmer_file: Path, intermediate_dir: Path) -> Path:
        """扩展k-mer库，添加反向互补序列"""
        expanded_file = intermediate_dir / "kmers_expanded.fasta"
        ...
        self.logger.info(f"k-mer库已扩展，保存到|K-mer library expanded, saved to: {expanded_file}")
    ...
    def detect_readable_lines(self, gz_file: str) -> int:
        """检测gzip文件可读取的最大行数"""
        self.logger.info(f"检测文件可读行数|Detecting readable lines: {os.path.basename(gz_file)}")
        ...
        for lines in test_lines:
            try:
                ...
                if actual_lines == lines:
                    ...
                    self.logger.info(f"检测到可读行数|Detected readable lines: {readable_lines}")
    ...
    def recover_partial_file(self, gz_file: str, readable_lines: int) -> str:
        ...
        try:
            ...
            self.logger.info(f"数据恢复完成|Data recovery complete: {recovered_file.name}")
            return str(recovered_file)
            
        except Exception as e:
            self.logger.error(f"数据恢复失败|Data recovery failed: {e}")
            raise
    ...
    def enhanced_file_integrity_check(self, file_path: str) -> tuple:
        ...
        # 快速尾部检查：验证gzip文件尾部完整性（<0.1秒）
        try:
            with open(file_path, 'rb') as f:
                f.seek(-8, 2)
                trailer = f.read(8)
                if len(trailer) == 8:
                    return True, True  # 文件完整且可读
        except:
            pass

        # 尾部检查失败但前1000行可读，认为可读
        return False, True