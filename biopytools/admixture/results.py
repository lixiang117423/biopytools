...
class CovariateGenerator:
    ...
    def generate_gwas_covariates(self, best_k: int):
        """✍️ 生成GWAS协变量文件 | Generate GWAS covariate file"""
        self.logger.info(f"✍️ 生成GWAS协变量文件 | Generating GWAS covariate file (K={best_k})")
...
class PlotGenerator:
    ...
    def generate_plots(self, q_data: pd.DataFrame, best_k: int):
        ...
        try:
            ...
            self.logger.warning("⚠️ Rscript未安装，跳过图表生成 | Rscript not installed, skipping plot generation")
...
class SummaryGenerator:
    ...
    def generate_summary(self, best_k: int, stats: dict):
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  - 🗑️ 缺失率阈值 | Missing rate threshold: {self.config.missing_rate}\n")
            f.write(f"  - ⚖️ HWE p值阈值 | HWE p-value threshold: {self.config.hwe_pvalue}\n\n")
