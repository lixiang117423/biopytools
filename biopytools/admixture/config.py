"""
⚙️ ADMIXTURE分析配置管理模块 | ADMIXTURE Analysis Configuration Management Module 📋
"""
...
class AdmixtureConfig:
    ...
    def validate(self):
        ...
        if not 0 <= self.missing_rate <= 1:
            errors.append(f"🗑️❌ 缺失率应在0-1之间 | Missing rate should be between 0-1: {self.missing_rate}")
        
        # 🚨 如果有错误，则集中抛出 | If errors exist, raise them all at once
        if errors:
            raise ValueError("配置错误 💥:\n" + "\n".join(errors))
        
        return True # ✅ 配置有效 | Configuration is valid