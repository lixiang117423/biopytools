...
class VCFProcessor:
    ...
    def _check_and_rename_chromosomes(self, vcf_file: str):
        """✏️🏷️ 检查染色体命名并重命名为数字 | Check chromosome naming and rename to numbers"""
        ...
        # 🗺️ 生成重命名映射 | Generate renaming mapping
        ...
        # ⚙️ 执行重命名 | Perform renaming
        renamed_vcf = self._rename_chromosomes(vcf_file, chr_mapping)
        
        # 🛡️ 验证重命名结果 | Verify renaming results
    ...
    def _generate_chromosome_mapping(self, chr_info: dict):
        """🗺️ 生成染色体重命名映射 | Generate chromosome renaming mapping"""
        ...
        self.logger.info("🗺️ 染色体重命名映射 | Chromosome renaming mapping:")
    ...
    def _save_chromosome_mapping(self, chr_mapping: dict):
        ...
        with open(mapping_file, 'w') as f:
            f.write("# 🗺️ 染色体重命名对应关系表 | Chromosome Renaming Mapping Table\n")
        ...
        self.logger.info("🗺️ 染色体重命名对应关系 | Chromosome renaming mapping:")
        for original, renamed in sorted(chr_mapping.items(), key=lambda x: int(x[1])):
            snp_count = chr_info.get(original, 0)
    ...
    def _verify_renamed_chromosomes(self, renamed_vcf: str):
        ...
        if non_numeric:
            self.logger.warning(f"⚠️ 警告：仍有非数字染色体 | Warning: Still have non-numeric chromosomes: {non_numeric}")
        else:
...
class PlinkProcessor:
    ...
    def quality_control(self, input_prefix: str):
        ...
        # 🗑️ 缺失率过滤 | Missing rate filtering
        cmd_missing = (
    ...
    def fix_chromosome_codes(self, plink_prefix: str):
        ...
        # 🗺️ 创建染色体映射 | Create chromosome mapping
        self.logger.warning(f"⚠️ 发现非数字染色体编号 | Found non-numeric chromosome codes: {non_numeric_chrs}")
        ...
        self.logger.info(f"🗺️  染色体编号映射 | Chromosome code mapping: {chr_mapping}")
        ...
        # ✏️ 修改bim文件中的染色体编号 | Modify chromosome codes in bim file
        # 创建映射字典，处理类型转换
        mapping_dict = {}
        for orig_chr in bim_df['CHR'].unique():
    ...
    def _verify_chromosome_fix(self, plink_prefix: str):
        """🛡️ 验证染色体编号修复结果 | Verify chromosome code fix results"""
        bim_file = f"{plink_prefix}.bim"
        bim_df = pd.read_csv(bim_file, sep='\t', header=None, 
                            names=['CHR', 'SNP', 'CM', 'BP', 'A1', 'A2'])
    ...
    def _cleanup_intermediate_files(self, prefixes: list):
        ...
        try:
            ...
            self.logger.warning(f"⚠️ 清理中间文件时出错 | Error cleaning intermediate files: {e}")