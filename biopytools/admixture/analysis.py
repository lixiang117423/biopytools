"""
🧠 ADMIXTURE分析核心模块 | ADMIXTURE Analysis Core Module ⚙️
"""
    ...
    def _run_single_k(self, bed_file: str, k: int):
        ...
        try:
            ...
            if os.path.exists(expected_q_file):
                self.logger.info(f"✔️  K={k} 分析成功完成 | K={k} analysis completed successfully")
            else:
                self.logger.warning(f"⚠️ K={k} 分析可能未完全成功，Q文件不存在 | K={k} analysis may not be fully successful, Q file missing")
...
class ResultsProcessor:
    ...
    def process_results(self, best_k: int):
        """⚙️ 处理分析结果 | Process analysis results"""
    ...
    def _read_p_file(self, p_file: str, k: int):
        ...
        if not os.path.exists(p_file):
            self.logger.warning(f"⚠️ P文件不存在 | P file not found: {p_file}")
            return None
