...
class RepeatAnalyzer:
    ...
    def build_denovo_library(self) -> Path:
        ...
        self.logger.warning(" RepeatModeler库构建失败，继续其他步骤|RepeatModeler library construction failed, continuing with other steps")
        return None
    
    def run_ltr_analysis(self) -> Path:
        """ 运行LTR序列分析|Run LTR sequence analysis"""
        if self.config.skip_ltr:
            self.logger.info(" 跳过LTR分析步骤|Skipping LTR analysis step")
            return None
        ...
        self.logger.warning(" LTR分析失败，继续其他步骤|LTR analysis failed, continuing with other steps")
        return None
