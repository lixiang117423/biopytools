...
class SampleFinder:
    ...
    def validate_sample_pairs(self, sample_pairs: List[Tuple[str, Path, Path]]) -> bool:
        ...
        for sample_name, read1_file, read2_file in sample_pairs:
            if read1_file.stat().st_size == 0:
                self.logger.warning(f"⚠️ 样本 {sample_name} 的read1文件为空 | Read1 file is empty for sample {sample_name}")
            
            if read2_file.stat().st_size == 0:
                self.logger.warning(f"⚠️ 样本 {sample_name} 的read2文件为空 | Read2 file is empty for sample {sample_name}")
