...
class ResistifyParser:
    ...
    def parse_results(self, input_dir: str = None) -> pd.DataFrame:
        """
        解析results.tsv主文件|Parse results.tsv main file

        Args:
            input_dir: Resistify输出目录，默认为config.resistify_output_dir|Resistify output directory
        ...
        results_file = Path(input_dir) / 'results.tsv'
        df = pd.read_csv(results_file, sep='\t')

        self.logger.info(f"解析完成，共{len(df)}条NLR基因|Parsing completed, {len(df)} NLR genes")
        return df

    def parse_domains(self, input_dir: str = None) -> pd.DataFrame:
        """
        ...
        domains_file = Path(input_dir) / 'domains.tsv'
    ...
    def prepare_summary_table(self, results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Sample' in results_df.columns:
            base_cols.insert(0, 'Sample')

        summary_df = results_df[base_cols].copy()
