...
class PopulationGeneticsAnalyzer:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # 步骤3: 计算多样性参数 | Step 3: Calculate diversity parameters
            # if 条件保持不变，作为总开关
            if any([self.config.calculate_pi, self.config.calculate_tajima_d]):
                self.logger.info("\n步骤3: 计算多样性参数 | Step 3: Calculate diversity parameters")
                
                # 调用新的、更智能的主方法。循环和判断逻辑已移入该方法内部。
                diversity_results = self.diversity_calculator.run_diversity_calculations(filtered_vcf)
...
示例 | Examples:
  ...
  %(prog)s -v filtered.vcf.gz -o results --ne --ibd -f csv
    ...
    # 设置分析参数 | Set analysis parameters
    if not args.all:
        # 注意：--theta-w 在argparse中会变成 theta_w（连字符变下划线）
        specific_analyses = any([
            getattr(args, 'fst', False),
            getattr(args, 'pi', False), 
            getattr(args, 'theta_w', False),
            getattr(args, 'tajima_d', False),
            getattr(args, 'ibd', False), 
            getattr(args, 'ld', False),
            getattr(args, 'ne', False)
        ])
        if not specific_analyses:
            parser.error("请至少指定一个分析参数 (如 --fst, --pi 等) 或使用 --all | "
                        "Please specify at least one analysis parameter (e.g., --fst, --pi, etc.) or use --all")
    ...
    if args.all:
        ...
        calculate_fst = args.fst
        calculate_pi = args.pi
        calculate_theta_w = args.theta_w  # 注意：--theta-w 变成 theta_w
        calculate_tajima_d = args.tajima_d  # 注意：--tajima-d 变成 tajima_d
        calculate_ibd = args.ibd
        calculate_ld = args.ld
        calculate_ne = args.ne
