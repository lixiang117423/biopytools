...
class VCFPreprocessor:
    ...
    def quality_control(self):
        # 检查是否跳过质控
        # print(f"DEBUG: skip_qc = {self.config.skip_qc}")  # 直接print，更明显
        ...
        # 检查输入文件是否压缩 | Check if input file is compressed
        vcf_param = "--gzvcf" if input_vcf.endswith('.gz') else "--vcf"
        cmd = f"{self.config.vcftools_path} {vcf_param} {input_vcf} {' '.join(filter_params)} --out {output_vcf.stem}"
