...
class GFFParser:
    ...
    def collect_gene_data(self) -> Dict[str, Dict[str, str]]:
        ...
        try:
            # <<< 3. 使用新的辅助方法打开文件 | Use the new helper method to open the file
            with self._open_gff_file() as infile:
                for line_num, line in enumerate(infile, 1):
                    # 跳过注释行 | Skip comment lines
                    if line.startswith('#'):
                        continue
    ...
    def process_transcripts(self, gene_data: Dict[str, Dict[str, str]]) -> List[List[str]]:
        ...
        try:
            # <<< 3. 同样使用新的辅助方法打开文件 | Also use the new helper method to open the file
            with self._open_gff_file() as infile:
                for line_num, line in enumerate(infile, 1):
                    # 跳过注释行 | Skip comment lines
                    if line.startswith('#'):
