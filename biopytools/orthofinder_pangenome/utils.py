...
def check_dependencies(config, logger):
    ...
    for cmd, name in dependencies:
        try:
            ...
            if not os.access(cmd, os.X_OK):
                logger.error(f"⚠️ 文件不可执行 | File not executable: {cmd}")
                missing_deps.append(name)
                continue
            ...
            if "OrthoFinder version" in result.stdout or "OrthoFinder version" in result.stderr:
                ...
                logger.error(f"⚠️ 程序响应异常 | Program response abnormal: {cmd}")
                logger.error(f"📤 标准输出: {result.stdout}")
                logger.error(f"❌ 标准错误: {result.stderr}")
                missing_deps.append(name)
                
        except subprocess.TimeoutExpired:
            logger.error(f"⏱️ 程序响应超时 | Program response timeout: {cmd}")
            missing_deps.append(name)
        except Exception as e:
            logger.error(f"❌ 检查程序时出错 | Error checking program {cmd}: {e}")
            missing_deps.append(name)
...
def validate_fasta_files(input_dir: str, logger) -> bool:
    ...
    logger.info(f"📄 找到 {count} 个FASTA文件 | Found {count} FASTA files")
    logger.info(f"🏷️  样本名称 | Sample names: {', '.join(sample_names[:5])}{'...' if count > 5 else ''}")
    ...
    if empty_files:
        logger.warning(f"⚠️ 发现空文件 | Found empty files: {', '.join(empty_files)}")
