...
def _generate_summary_report(config, runtime_min: int, runtime_sec: int):
    ...
    try:
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 60 + "\n")
            f.write(f"  BAM文件|BAM file: {config.result_dir / f'{config.sample_name}_sorted.bam'}\n")
            f.write(f"  BAM索引|BAM index: {config.result_dir / f'{config.sample_name}_sorted.bam.bai'}\n")
