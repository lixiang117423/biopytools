...
#             if failed_samples:
#                 self.logger.warning(f"失败样品列表|Failed samples list: {', '.join(failed_samples)}")
...
#             self.logger.info(f"  └── tmp/     (临时文件目录|Temporary files directory)")
...
class GTXAnalyzer:
    ...
    def run_joint_analysis(self, processed_sample_names) -> bool:
        ...
        if len(processed_sample_names) < 2:
            self.logger.warning("需要至少2个成功处理的样品才能进行Joint calling|Need at least 2 successfully processed samples for joint calling")
            return False
    ...
    def run_analysis(self):
        """运行完整的GTX WGS分析流程 |Run complete GTX WGS analysis pipeline"""
        try:
            analysis_mode = "Joint Calling模式|Joint Calling mode" if self.config.enable_joint_calling else "单样品分析模式|Individual analysis mode"
            ...
            if not self.check_and_build_reference_index():
                raise RuntimeError("参考基因组索引检查/构建失败|Reference genome index check/build failed")
            ...
            if joint_success:
                ...
                self.logger.warning("GTX WGS批处理分析部分完成 (Joint calling失败)|GTX WGS batch processing analysis partially completed (Joint calling failed)")
    ...
    def _display_final_structure(self, successful_samples):
        ...
        self.logger.info(f"  ├── vcf/     (VCF文件: {vcf_count} 个|VCF files: {vcf_count})")
        
        if self.config.enable_joint_calling:
            joint_files = list(self.config.joint_output_dir.glob('*'))
            self.logger.info(f"  ├── joint/   (Joint calling结果: {len(joint_files)} 个文件|Joint calling results: {len(joint_files)} files)")
            ...
            if joint_files:
                for joint_file in joint_files:
                    if joint_file.name.endswith('.vcf.gz'):
                        ...
                        self.logger.info(f"  │   ├── {joint_file.name} (样品映射文件|Sample mapping file)")
                    else:
...
def main():
    """主函数|Main function"""
    parser = argparse.ArgumentParser(
        description='GTX WGS批处理分析脚本|GTX WGS Batch Processing Analysis Script',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例|Examples:
  %(prog)s -i /path/to/clean/data -o /path/to/output -r /path/to/reference.fa --enable-joint
        """
    )
