# """
#  GTX WGS分析配置管理模块|GTX WGS Analysis Configuration Management Module 
# """
...
#     gtx_path: str = "~/software/gtx/GTX.CAT_2.2.1/bin/gtx"
#     threads: int = 88
...
class GTXConfig:
    ...
    def __post_init__(self):
        ...
        self.bam_output_dir = self.output_path / "bam"
        self.vcf_output_dir = self.output_path / "vcf"
        self.joint_output_dir = self.output_path / "joint"
        
        self.bam_output_dir.mkdir(parents=True, exist_ok=True)
        self.vcf_output_dir.mkdir(parents=True, exist_ok=True)
        
        # 如果启用joint calling，创建joint输出目录
        if self.enable_joint_calling:
            self.joint_output_dir.mkdir(parents=True, exist_ok=True)
        ...
        # 设置joint calling相关文件路径
        if self.enable_joint_calling:
            self.sample_map_file = self.joint_output_dir / "sample_map.txt"
            self.joint_output_file = self.joint_output_dir / self.joint_output_name
    ...
    def validate(self):
        ...
        if self.enable_joint_calling and self.joint_threads <= 0:
            errors.append(f" Joint calling线程数必须为正整数|Joint calling thread number must be positive: {self.joint_threads}")
        ...
        # 检查joint calling参数
        if self.enable_joint_calling:
            if not self.joint_output_name.endswith('.vcf.gz'):
                errors.append(f" Joint输出文件名必须以.vcf.gz结尾|Joint output filename must end with .vcf.gz: {self.joint_output_name}")
