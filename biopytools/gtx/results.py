...
#             f.write(f" 分析配置|Analysis Configuration:\n")
...
class SummaryGenerator:
    ...
    def generate_summary_report(self):
        ...
        if self.config.enable_joint_calling:
            ...
            self.logger.info(f"    ✓ Joint VCF文件|Joint VCF file: {'存在' if joint_vcf_exists else '不存在'}|{'exists' if joint_vcf_exists else 'not found'}")
            self.logger.info(f"    ✓ 样品映射文件|Sample mapping file: {'存在' if sample_map_exists else '不存在'}|{'exists' if sample_map_exists else 'not found'}")
