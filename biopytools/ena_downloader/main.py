...
class ENADownloader:
    ...
    def __init__(self, **kwargs):
        ...
        # 🛠️ 初始化各个处理器 | Initialize processors
    ...
    def run_full_pipeline(self):
        ...
        try:
            ...
            # 1️⃣ 下载元数据 | Download metadata
            self.logger.info("1️⃣ 步骤 1/3: 下载元数据 | Step 1/3: Download metadata")
            metadata_file = self.metadata_downloader.download_metadata()
            ...
            # 2️⃣ 提取下载链接 | Extract download links
            self.logger.info("2️⃣ 步骤 2/3: 提取下载链接 | Step 2/3: Extract download links")
            download_links = self.metadata_downloader.get_download_links(metadata_file)
            
            if not download_links:
                self.logger.warning("⚠️ 未找到下载链接 | No download links found")
                # 📋 仍然生成汇总报告 | Still generate summary report
                self.results_summary.generate_summary(metadata_file, download_links_count=0)
                return True
            
            # 3️⃣ 处理下载 | Process downloads
            self.logger.info("3️⃣ 步骤 3/3: 处理下载 | Step 3/3: Process downloads")
            script_file = None
            ...
            if self.config.method == "save":
                ...
                if script_file:
                    ...
                    print(f'\n\033[32m✅ 下载脚本已生成 | Download script generated: {script_file.name}\033[0m')
...
def main():
    ...
    try:
        ...
        if args.metadata_only:
            ...
            if result:
                print(f'\n\033[32m✅ 元数据下载完成 | Metadata download completed\033[0m')
                print(f'📋 元数据文件 | Metadata file: {result}')
            else:
                print('\n\033[31m❌ 元数据下载失败 | Metadata download failed\033[0m')
                sys.exit(1)
        else:
            # 🚀 完整流程模式 | Full pipeline mode
            success = downloader.run_full_pipeline()
            if success:
                print(f'\n\033[32m🎉 流程执行完成 | Pipeline execution completed\033[0m')
            else:
                print('\n\033[31m❌ 流程执行失败 | Pipeline execution failed\033[0m')
                sys.exit(1)
    
    except KeyboardInterrupt:
        print('\n\033[33m⚠️ 用户中断操作 | User interrupted operation\033[0m')
        sys.exit(1)
    except Exception as e:
        print(f'\n\033[31m❌ 发生错误 | Error occurred: {str(e)}\033[0m')
        sys.exit(1)
