...
from .config import COLOR_SCHEMES, AxesType


class MsaViz:
    """Multiple Sequence Alignment Visualization|多序列比对可视化"""
    ...
    def _plot_msa(
        ...
        # Adjust xlim to include labels if show_label is True
        # Labels use ha="right" at position (start - 1), so need extra space for text
        if self._show_label:
            # Estimate space needed for labels (approx 0.6 unit per character)
            max_label_len = max(len(id) for id in self.id_list) if self.id_list else 10
            label_space = max(5, int(max_label_len * 0.6) + 2)
            xlim_start = start - label_space
        else:
            xlim_start = start
        ax.set_xlim(xlim_start, start + self._wrap_length)
