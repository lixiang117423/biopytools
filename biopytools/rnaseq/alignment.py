...
class HISAT2Indexer:
    ...
    def extract_splice_sites(self, gtf_file: str, output_file: str) -> bool:
        ...
        try:
            ...
            # 检查output_file是否已存在
            if os.path.exists(output_file):
                self.logger.info(f"⏭️  剪接位点文件已存在，跳过 | Splice sites file already exists, skipping: {output_file}")
                return True
    ...
    def extract_exons(self, gtf_file: str, output_file: str) -> bool:
        ...
        try:
            ...
            # 检查output_file是否已存在
            if os.path.exists(output_file):
                self.logger.info(f"⏭️  外显子文件已存在，跳过 | Exons file already exists, skipping: {output_file}")
                return True
    ...
    def build_hisat2_index(self) -> str:
        ...
        # 检查索引是否已存在 | Check if index already exists
        if os.path.exists(f"{index_prefix}.1.ht2"):
            self.logger.info(f"⏭️  HISAT2索引已存在 | HISAT2 index already exists: {index_prefix}")
            return index_prefix
        ...
        # 提取剪接位点信息 | Extract splice sites
        if not self.extract_splice_sites(gtf_file, splice_sites_file):
            self.logger.warning("⚠️  剪接位点提取失败，将使用基本模式构建索引 | Splice sites extraction failed, building index without splice sites")
            splice_sites_file = None
        
        # 提取外显子信息 | Extract exons
        if not self.extract_exons(gtf_file, exons_file):
            self.logger.warning("⚠️  外显子信息提取失败，将使用基本模式构建索引 | Exons extraction failed, building index without exons")
            exons_file = None
