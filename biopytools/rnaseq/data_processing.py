...
class FastqPatternParser:
    ...
    def parse_fastq_pattern(self, pattern: str) -> Tuple[str, str, str, str]:
        ...
        # 检测读取指示符 - 扩展支持更多格式 | Detect read indicators - extended support for more formats
        read_indicators = [
        ...
        for r1, r2 in read_indicators:
            if r1 in suffix:
                read1_indicator = r1
                read2_indicator = r2
                self.logger.info(f"🔍 检测到读取指示符 | Detected read indicator: {r1} <-> {r2}")
                break

        if not read1_indicator:
            raise ValueError(
                f"❌ 无法识别读取指示符 | Cannot identify read indicator in pattern '{pattern}'\n"
...
class SampleParser:
    ...
    def parse_input_samples(self, input_path: str, fastq_pattern: str = None) -> List[Dict]:
        ...
        if os.path.isdir(input_path):
            if fastq_pattern:
                # 使用用户指定的文件模式 | Use user-specified file pattern
                self.logger.info(f"📂 使用指定文件模式 | Using specified file pattern: {fastq_pattern}")
                samples = self._parse_with_pattern(input_path, fastq_pattern)
            else:
                # 默认模式：自动搜索常见的fastq文件对 | Default mode: automatically search for common fastq file pairs
                self.logger.info("🔍 自动搜索常见的FASTQ文件格式 | Auto-detecting common FASTQ file formats")
                samples = self._parse_with_default_patterns(input_path)
        else:
            # 如果是文件，假设它是样本信息文件 | If it's a file, assume it's a sample information file
    ...
    def _parse_with_pattern(self, input_path: str, fastq_pattern: str) -> List[Dict]:
        ...
        try:
            ...
            for fq1 in sorted(fastq_files):
                ...
                # 从文件名中提取样本名称
                # 方法：移除prefix和suffix
                # 例如：对于文件 "CRR564415_f1.fq.gz"，模式 "*_f1.fq.gz"
                ...
                # 根据样本名称和模式构建read2文件名
                # 方法：将suffix中的read1指示符替换为read2指示符
                ...
                if os.path.exists(fq2):
                    ...
                    self.logger.warning(f"   ⚠️  找不到配对的read2文件 | Cannot find paired read2 file: {fq2}")
                    self.logger.warning(f"   ⏭️  跳过此样本 | Skipping this sample")
    ...
    def _parse_with_default_patterns(self, input_path: str) -> List[Dict]:
        ...
        # 默认模式列表 - 扩展支持更多格式 | Default pattern list - extended support
        patterns = [
            # 标准格式 | Standard formats
            ("*_1.fq.gz", "*_2.fq.gz"),
            ("*_R1.fq.gz", "*_R2.fq.gz"),
            ...
            with open(input_path, "r") as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    if len(parts) >= 3:
                        ...
                        self.logger.warning(f"  ⚠️  行 {line_num} 格式不正确，跳过 | Line {line_num} has incorrect format, skipping: {line.strip()}")
        except Exception as e:
            self.logger.error(f"❌ 读取样本文件时出错 | Error reading sample file: {e}")
