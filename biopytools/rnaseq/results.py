...
#         #  all_fpkm_tpm.txt ()|Generate all_fpkm_tpm.txt (direct merge of all data)
#         self.logger.info(" all_fpkm_tpm.txt|Generating all_fpkm_tpm.txt...")
#         all_output_file = os.path.join(output_dir, "all_fpkm_tpm.txt")
...
#                 f.write(f"\n|Output Files:\n")
#                 f.write(f"  - all_fpkm_tpm.txt: FPKMTPM|FPKM and TPM data for all samples\n")
#                 f.write(f"  - 02.stringtie/: StringTie|StringTie quantification results\n")
#                 f.write(f"  - 03.fpkm_tpm/: FPKM|FPKM files for each sample\n")

#                 if self.config.remove_bam.lower() not in ['yes', 'y']:
#                     f.write(f"  - 01.bam/*.sorted.bam: BAM|Alignment result BAM files\n")
...
class ExpressionMatrixMerger:
    ...
    def merge_expression_matrix(self, fpkm_files: List[str]) -> bool:
        ...
        # 生成all_fpkm_tpm.txt（直接合并所有数据）|Generate all_fpkm_tpm.txt (direct merge of all data)
        self.logger.info("生成all_fpkm_tpm.txt文件|Generating all_fpkm_tpm.txt...")
        all_output_file = os.path.join(output_dir, "all_fpkm_tpm.txt")
    ...
    def generate_summary_report(self) -> bool:
        ...
        try:
            ...
            with open(report_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"\n输出文件|Output Files:\n")
                f.write(f"  - all_fpkm_tpm.txt: 所有样本的FPKM和TPM数据|FPKM and TPM data for all samples\n")
                f.write(f"  - 02.stringtie/: StringTie定量结果|StringTie quantification results\n")
