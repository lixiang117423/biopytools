...
"""
🗄️ BLAST数据库管理模块 | BLAST Database Management Module
"""
...
class DatabaseManager:
    """🗄️ BLAST数据库管理器 | BLAST Database Manager"""
    ...
    def create_target_database(self) -> str:
        """创建目标基因数据库 | Create Target Gene Database"""
        self.logger.info("创建BLAST数据库... | Creating BLAST database...")
        
        target_file = self.config.target_file
        target_path = Path(target_file)
        
        # 在输出目录下创建blast_db子目录 | Create a blast_db subdirectory in the output directory
        blast_db_dir = self.config.output_path / "blast_db"
        blast_db_dir.mkdir(exist_ok=True)
        self.logger.info(f"数据库目录 | Database directory: {blast_db_dir}")
        
        # 设置数据库文件路径到blast_db目录 | Set the database file path to the blast_db directory
        db_name = blast_db_dir / f"{target_path.stem}_db"
        self.target_db_path = str(db_name)
        
        # 检查是否已存在数据库文件 | Check if database files already exist
        db_extensions = ['.nhr', '.nin', '.nsq'] if self.config.get_db_type() == 'nucl' else ['.phr', '.pin', '.psq']
        db_exists = all((Path(f"{self.target_db_path}{ext}").exists() for ext in db_extensions))
        
        if db_exists:
            self.logger.info("数据库文件已存在，跳过创建 | Database files already exist, skipping creation")
            return self.target_db_path
        ...
        success = self.cmd_runner.run(cmd, "创建BLAST数据库 | Creating BLAST database")
        
        if success:
            self.logger.info(f"BLAST数据库创建成功 | BLAST database created successfully: {self.target_db_path}")
        else:
            raise RuntimeError("BLAST数据库创建失败 | BLAST database creation failed")
