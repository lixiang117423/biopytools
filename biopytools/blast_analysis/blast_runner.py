...
class BLASTRunner:
    ...
    def run_blast_analysis(self, target_db_path: str) -> List[Tuple[str, str, str]]:
        ...
        # 如果启用了比对可视化，提示用户
        if self.config.needs_alignment_sequences():
            self.logger.info("💡 已启用序列比对可视化，将自动添加序列字段到输出")
    ...
    # def _build_blast_command(self, query_file: str, db_path: str, output_file: Path) -> str:
    #     """构建BLAST命令（自动处理比对可视化所需字段）"""
    #     blast_executable = self.config.get_blast_executable()
        
    #     # 使用配置中的最终outfmt（已自动添加qseq/sseq如果需要）
    #     final_outfmt = self.config.get_final_outfmt()
    ...
    def _build_blast_command(self, query_file: str, db_path: str, output_file: Path) -> str:
        ...
        self.logger.info(f"  - alignment_output = {self.config.alignment_output}")
        self.logger.info(f"  - needs_sequences = {self.config.needs_alignment_sequences()}")
        
        # 使用配置中的最终outfmt
        final_outfmt = self.config.get_final_outfmt()
        
        # 🔍 调试：打印outfmt
        self.logger.info(f"  - base outfmt = {self.config.outfmt}")
        self.logger.info(f"  - final outfmt = {final_outfmt}")
