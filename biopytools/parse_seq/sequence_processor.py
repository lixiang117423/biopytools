...
from .utils import CommandRunner, parse_regions_file, format_region_name, build_conda_command
...
class SequenceProcessor:
    ...
    def _extract_dna_sequences(self, regions: List[Tuple[str, int, int, str]]) -> bool:
        ...
        if not Path(fai_file).exists():
            self.logger.info("建立序列索引|Building sequence index")
            # 构建命令|Build command
            cmd_list = build_conda_command(self.config.samtools_path, ['faidx', self.config.sequence_file])
            index_cmd = ' '.join(cmd_list)
            # 记录完整命令（INFO级别）|Log complete command (INFO level)
            self.logger.info(f"命令|Command: {index_cmd}")
    ...
    def _extract_dna_merged(self, regions: List[Tuple[str, int, int, str]]) -> bool:
        ...
        for i in range(0, len(region_strings), batch_size):
            ...
            samtools_args = ['faidx', self.config.sequence_file] + batch
            cmd_list = build_conda_command(self.config.samtools_path, samtools_args)
            # 添加shell重定向|Add shell redirection
            cmd = ' '.join(cmd_list) + f" > {temp_output}"
            # 记录完整命令（INFO级别）|Log complete command (INFO level)
    ...
    def _extract_dna_separate(self, regions: List[Tuple[str, int, int, str]]) -> bool:
        ...
        for i, (chrom, start, end, strand) in enumerate(regions, 1):
            ...
            # 构建samtools命令|Build samtools command
            samtools_args = ['faidx', self.config.sequence_file, region_str]
            cmd_list = build_conda_command(self.config.samtools_path, samtools_args)
            # 添加shell重定向|Add shell redirection
            cmd = ' '.join(cmd_list) + f" > {output_file}"
            # 记录完整命令（INFO级别）|Log complete command (INFO level)
            self.logger.info(f"命令|Command: {cmd}")
