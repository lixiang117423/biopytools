...
class JointCaller:
    ...
    def run_joint_calling(self) -> Tuple[bool, str]:
        ...
        self.logger.info(f"  < {self.config.gatk_threshold} → GATK模式|GATK mode")
        self.logger.info(f"  {self.config.gatk_threshold}-{self.config.gtx_single_threshold} → GTX单机模式|GTX single machine mode")
            ...
            for vcf_file in FileManager.find_files(self.config.filter_dir, "*.vcf.gz"):
                if os.path.exists(vcf_file):
                    try:
                        ...
                        self.logger.warning(f"无法统计 {vcf_file}: {str(e)}|Cannot count variants for {vcf_file}: {str(e)}")
