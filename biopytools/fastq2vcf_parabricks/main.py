...
class Fastq2VcfParabricksProcessor:
    ...
    def __init__(self, **kwargs):
        ...
        verbose = getattr(self.config, 'verbose', 0)
        ...
        self.logger_manager = Fastq2VcfLogger(
            log_dir,
            log_name=log_name,
            log_level=log_level,
            quiet=quiet,
    ...
    def run_full_pipeline(self):
        ...
        self.logger.info(f"Total runtime: {int(hours):02d}:{int(minutes):02d}:{seconds:05.2f}")
        self.logger.info(" Fastq到VCF (Parabricks) 流程全部完成|Fastq to VCF (Parabricks) pipeline completed!")
        return True

    def _print_program_info(self):
        """输出程序信息|Print program information"""
        self.logger.info("=" * 70)
        self.logger.info("Fastq to VCF (Parabricks) Analysis Pipeline")
        self.logger.info(f"Version: {VERSION}")
        self.logger.info(f"Author: Xiang LI")
        self.logger.info(f"Start time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
        self.logger.info("=" * 70)
