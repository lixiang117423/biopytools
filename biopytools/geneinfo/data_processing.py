...
import csv
import gzip  # <<< 1. 引入gzip模块|Import the gzip module
from typing import Dict, List, Tuple, Any
from .utils import AttributeParser, GFFValidator
...
class GFFParser:
    ...
    def _open_gff_file(self, file_path: str = None):
        """
        根据文件名后缀智能打开普通或gzip压缩文件。
        Intelligently opens a regular or gzipped file based on the filename suffix.
        """
        gff_file = file_path or self.config.gff3_file
        if gff_file.endswith('.gz'):
            self.logger.info(f"以gzip模式打开文件|Opening file in gzip mode: {gff_file}")
            return gzip.open(gff_file, 'rt', encoding='utf-8')
        else:
            self.logger.info(f"以普通文本模式打开文件|Opening file in normal text mode: {gff_file}")
            return open(gff_file, 'r', encoding='utf-8')

    def collect_gene_data(self, file_path: str = None) -> Dict[str, Dict[str, str]]:
        """
        第一遍扫描：收集所有基因信息|First pass: collect all gene information
        """
        gene_data = {}
        ...
        try:
            with self._open_gff_file(file_path) as infile:
                for line_num, line in enumerate(infile, 1):
                    # 跳过注释行|Skip comment lines
                    if line.startswith('#'):
                        continue
    ...
    def process_transcripts(self, gene_data: Dict[str, Dict[str, str]], file_path: str = None, sample_name: str = None) -> List[List[str]]:
        """
        第二遍扫描：处理转录本信息|Second pass: process transcript information
        """
        transcript_data = []
