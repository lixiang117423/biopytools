    ...
    def generate_summary_report(self, transcript_data: List[List[str]]):
        ...
        try:
            with open(report_file, 'w', encoding='utf-8') as f:
                ...
                if os.path.isdir(self.config.gff3_file):
                    f.write(f"  - 输入目录|Input directory: {self.config.gff3_file}\n")
                    f.write(f"  - GFF3文件数|GFF3 file count: {len(self.config.gff3_files)}\n")
                    for gf in self.config.gff3_files:
                        f.write(f"    - {os.path.basename(gf)}\n")
                else:
                ...
                if transcript_data:
                    ...
                    f.write(f"  - 样品数|Samples: {len(samples)}\n")
