...
class ALLHiCSteps:
    ...
    def step0_prepare_data(self):
        ...
        if self.config.skip_steps.get("data", False):
            self.logger.log(" 跳过数据准备|Skipping data preparation")
            return
    ...
    def step1_mapping(self):
        ...
        if self.config.skip_steps["mapping"]:
            self.logger.log(" 跳过比对|Skipping mapping")
            return
    ...
    def step1_5_allele_detection(self):
        ...
        if self.config.skip_steps["allele"]:
            self.logger.log(" 跳过等位基因检测|Skipping allele detection")
            return
    ...
    def step2_pruning(self):
        ...
        if self.config.skip_steps["prune"]:
            self.logger.log(" 跳过修剪|Skipping pruning")
            return
        
        self.logger.log_section("[Step 2] 修剪嵌合contigs|Pruning chimeric contigs")
    ...
    def step3_partition(self):
        ...
        if self.config.skip_steps["partition"]:
            self.logger.log(" 跳过分区|Skipping partition")
            return
    ...
    def step3_5_extract(self):
        ...
        if self.config.skip_steps["extract"]:
            self.logger.log(" 跳过矩阵提取|Skipping matrix extraction")
            return
    ...
    def step4_rescue(self):
        ...
        if self.config.skip_steps["rescue"]:
            self.logger.log(" 跳过拯救|Skipping rescue")
            return
    ...
    def step5_optimize(self):
        ...
        if self.config.skip_steps["optimize"]:
            self.logger.log(" 跳过优化|Skipping optimization")
            return
        
        self.logger.log_section(" [Step 5] 优化contig顺序|Optimizing contig order")
    ...
    def step6_build(self):
        ...
        if self.config.skip_steps["build"]:
            self.logger.log(" 跳过构建|Skipping build")
            return
        
        self.logger.log_section(" [Step 6] 构建最终组装|Building final assembly")
    ...
    def step7_plot(self):
        ...
        if self.config.skip_steps["plot"]:
            self.logger.log(" 跳过绘图|Skipping plot")
            return
