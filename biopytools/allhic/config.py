"""
ALLHiC流水线配置管理模块 |ALLHiC Pipeline Configuration Management Module
"""
...
class PipelineConfig:
    ...
    bwa_path: str = field(default_factory=lambda: os.getenv("ALLHIC_BWA_PATH", "~/miniforge3/envs/Population_genetics/bin"))
    allhic_software_path: str = field(default_factory=lambda: os.getenv("ALLHIC_SOFTWARE_PATH", "~/software/ALLHiC/bin"))
    allhic_bin_path: str = field(default_factory=lambda: os.getenv("ALLHIC_BIN_PATH", "~/software/ALLHiC/bin"))  # ALLHiC工具在bin目录
    asmkit_path: str = field(default_factory=lambda: os.getenv("ALLHIC_ASMKIT_PATH", "~/software/asmkit/asmkit"))
    assembly_visualizer: str = field(default_factory=lambda: os.getenv("ALLHIC_VISUALIZER_PATH", "~/software/3d-dna/visualize/run-assembly-visualizer.sh"))
    ...
    def __post_init__(self):
        ...
        self.allhic_software_path = expand_path(self.allhic_software_path)
        self.allhic_bin_path = expand_path(self.allhic_bin_path)
        self.asmkit_path = expand_path(self.asmkit_path)
        self.assembly_visualizer = expand_path(self.assembly_visualizer)
    ...
    def validate(self):
        ...
        for path, name in software_paths:
            if not os.path.exists(path):
                warnings.append(f" {name}路径不存在|{name} path not found: {path}")
