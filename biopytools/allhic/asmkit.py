...
class AsmkitProcessor:
    ...
    def run_asmkit_jbat(self):
        ...
        if self.config.skip_steps["asmkit"]:
            self.logger.log(" 跳过asmkit JBAT生成|Skipping asmkit JBAT generation")
            return
        
        self.logger.log_section(" [Step 8] 使用asmkit生成JBAT文件|Generating JBAT files with asmkit")
        self.logger.log(" 策略: 使用ALLHiC结果 (sample.clean.bam + groups.agp)|Strategy: Using ALLHiC results (sample.clean.bam + groups.agp)")
    ...
    def _check_asmkit_tool(self):
        ...
        if os.path.exists(self.config.asmkit_path) and os.access(self.config.asmkit_path, os.X_OK):
            ...
            self.logger.log_success(f"找到asmkit: {asmkit_cmd}|asmkit found: {asmkit_cmd}")
        else:
            # 检查PATH中是否有asmkit
            import shutil
            ...
            if asmkit_path:
                ...
                self.logger.log_success(f"在PATH中找到asmkit: {asmkit_cmd}|asmkit found in PATH: {asmkit_cmd}")
            else:
    ...
    def _generate_links_file(self, asmkit_cmd):
        ...
        if success and check_file_exists("out.links"):
            ...
            self.logger.log_success(f"成功生成out.links ({link_count} 行, {file_size})|Successfully generated out.links ({link_count} lines, {file_size})")
    ...
    def _generate_assembly_file(self, asmkit_cmd):
        ...
        if success and check_file_exists("groups.assembly"):
            end_time = time.time()
            elapsed = int(end_time - start_time)
            self.logger.log_success(f"成功生成groups.assembly|Successfully generated groups.assembly")
            self.logger.log(f"   耗时|Time: {self.logger.format_time(elapsed)}")
        else:
    ...
    def _generate_hic_file(self):
        ...
        if not check_file_exists(self.config.assembly_visualizer):
            self.logger.log_warning(f"Visualizer未找到: {self.config.assembly_visualizer}|Visualizer not found: {self.config.assembly_visualizer}")
        ...
        if success and check_file_exists("groups.hic"):
            ...
            self.logger.log_success(f"成功生成groups.hic ({file_size})|Successfully generated groups.hic ({file_size})")
            self.logger.log(f"   耗时|Time: {self.logger.format_time(elapsed)}")
        else:
            self.logger.log_warning(" Visualizer失败。请检查日志。| Visualizer failed. Check logs.")
