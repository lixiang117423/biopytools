...
class PipelineLogger:
    ...
    def log_step_time(self):
        ...
        if self.step_start_time > 0:
            ...
            self.logger.info(f"步骤完成|Step completed in {formatted_time}")
    ...
    def run_command(self, cmd: str, description: str = "", work_dir: str = None) -> bool:
        ...
        self.log_command(wrapped_cmd, description)
    ...
    def _wrap_conda_command(self, cmd: str) -> str:
        ...
        # 提取命令的第一个词（命令名称）|Extract first word (command name)
        # 处理可能的前缀空格|Handle leading whitespace
        cmd = cmd.strip()

        # 匹配命令名称（支持路径）|Match command name (supports paths)
        # 例如: "bwa index", "samtools view", "/path/to/command"
