# # """
# #  parabricks WGS分析工具函数模块|parabricks WGS Analysis Utility Functions Module 
# # """
...
#             if not r2_file:
#                 self.logger.warning(f" 找不到对应的R2文件: {r1_file.name}，将跳过此样本。")
#                 continue
...
class FileProcessor:
    ...
    def find_fastq_files(self):
        ...
        for r1_file in r1_files:
            ...
            if not r2_file.exists():
                self.logger.warning(f" 找不到样品 '{sample_name}' 对应的R2文件: {r2_file.name}|Cannot find corresponding R2 file for sample '{sample_name}': {r2_file.name}")
                continue
