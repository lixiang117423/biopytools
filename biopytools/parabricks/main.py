...
class parabricksAnalyzer:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            if failed_samples:
                self.logger.warning(f" 失败样品|Failed: {', '.join(failed_samples)}")
    ...
    def _display_final_structure(self):
        ...
        if self.config.gvcf:
            ...
            if self.config.joint_calling:
                ...
                self.logger.info(f"  ├── vcf/     ( 个体GVCF: {individual_gvcf} 个,  Combined: {'是' if combined_exists else '否'}|Individual GVCF: {individual_gvcf}, Combined: {'Yes' if combined_exists else 'No'})")
            else:
                self.logger.info(f"  ├── vcf/     ( GVCF: {gvcf_count} 个|GVCF files: {gvcf_count})")
        else:
            vcf_count = len(list(self.config.vcf_output_dir.glob('*.vcf.gz')))
            self.logger.info(f"  ├── vcf/     ( VCF: {vcf_count} 个|VCF files: {vcf_count})")
        
        self.logger.info(f"  └── tmp/     ( 临时文件|Temporary files)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例|Examples:
  %(prog)s -i /data/clean -o /results -r /genome/ref.fa
        """
    )
