# """
#  parabricks WGS分析配置管理模块|parabricks WGS Analysis Configuration Management Module 
# """
...
#     """ parabricks分析配置类|parabricks Analysis Configuration Class"""
...
#     #  parabricks参数|parabricks parameters
#     parabricks_path: str = "~/software/containers/parabricks.sif"
#     threads: int = 88
...
#     joint_calling: bool = True  # 新增：是否进行joint calling|Whether to perform joint calling
#     combined_output_name: str = "combined.g.vcf"  # 新增：合并输出文件名|Combined output filename
...
#         # 设置临时目录 |Set temporary directory
...
class parabricksConfig:
    ...
    #  parabricks参数|parabricks parameters
    parabricks_path: str = field(
        default_factory=lambda: get_tool_path(
            'parabricks',
            '~/software/containers/parabricks.sif',
            'PARABRICKS_PATH'
        )
    )
    threads: int = 88
    
    #  工作流程控制|Workflow control
    workflow: str = "all"  # 可选|Options: "fq2bam", "haplotypecaller", "genotypegvcf", "all"
    ...
    def __post_init__(self):
        """初始化后处理 |Post-initialization processing"""
        # 标准化路径|Normalize paths
        self.parabricks_path = expand_path(self.parabricks_path)
        self.input_dir = os.path.normpath(os.path.abspath(self.input_dir))
        self.output_dir = os.path.normpath(os.path.abspath(self.output_dir))
        self.reference = os.path.normpath(os.path.abspath(self.reference))
    ...
    def validate(self):
        ...
        # 检查工作流程参数|Check workflow parameter
        valid_workflows = ["fq2bam", "haplotypecaller", "genotypegvcf", "all"]
        if self.workflow not in valid_workflows:
            errors.append(f" 无效的工作流程|Invalid workflow: {self.workflow}. 可选|Options: {', '.join(valid_workflows)}")
