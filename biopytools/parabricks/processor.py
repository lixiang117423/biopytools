...
#             self.logger.info(f"   BAM文件已存在，跳过|BAM file exists, skipping: {output_bam.name}")
#             return True
...
#             self.logger.info(f"   VCF文件已存在，跳过|VCF file exists, skipping: {output_vcf.name}")
#             return True
...
#             self.logger.warning(" genotypegvcf需要GVCF格式|genotypegvcf requires GVCF format")
#             return False
...
#         if missing_samples:
#             self.logger.warning(f" 缺少GVCF文件|Missing GVCF files: {', '.join(missing_samples)}")
...
#             self.logger.info(f" 输出VCF已存在|Output VCF exists: {combined_output_vcf.name}")
#             return True
...
#             else:
#                 self.logger.warning("   索引失败|Index failed")
...
class parabricksProcessor:
    ...
    def joint_calling(self, sample_names: list) -> bool:
        ...
        if not self.config.gvcf:
            self.logger.warning(" Joint calling需要GVCF格式，但当前未启用GVCF输出|Joint calling requires GVCF format, but GVCF output is not enabled")
            return False
        ...
        if len(sample_names) < 2:
            self.logger.warning(f" 成功处理的样本少于2个 ({len(sample_names)}个)，无法进行joint calling|Less than 2 successfully processed samples ({len(sample_names)}), cannot perform joint calling")
            return False
        ...
        if missing_samples:
            self.logger.warning(f" 以下样本的GVCF文件不存在，将被跳过|Missing GVCF files for samples: {', '.join(missing_samples)}")
        
        if len(gvcf_files) < 2:
            self.logger.warning(f" 找到的GVCF文件少于2个 ({len(gvcf_files)}个)，无法进行joint calling|Found less than 2 GVCF files ({len(gvcf_files)}), cannot perform joint calling")
            return False
        ...
        if combined_output_vcf.exists():
            self.logger.info(f" Combined VCF文件已存在，跳过|Combined VCF file already exists: {combined_output_vcf.name}")
            return True
