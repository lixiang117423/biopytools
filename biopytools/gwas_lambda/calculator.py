    ...
    def analyze_file(self, file_path: str) -> Tuple[Optional[float], int, int, Optional[str]]:
        ...
        try:
            ...
            if not os.path.exists(file_path):
                error_msg = f"文件不存在|File not found: {file_path}"
                if self.logger:
            ...
            if not p_values:
                error_msg = "无有效P值|No valid P-values found"
                if self.logger:
                    self.logger.warning(f"{file_path}: {error_msg}")
                return None, 0, 0, error_msg
            ...
            if sig_count == 0:
                error_msg = "无显著位点|No significant variants found"
                if self.logger:
                    self.logger.info(f"{file_path}: {error_msg}")
                return None, total_snps, 0, error_msg
            ...
            error_msg = str(e)
            if self.logger:
                self.logger.error(f"分析文件时出错|Error analyzing file {file_path}: {error_msg}")
            return None, 0, 0, error_msg
