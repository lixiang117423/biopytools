...
from .calculator import GWASResultAnalyzer
from .utils import GWASLambdaLogger
...
class GWASLambdaCalculator:
    ...
    def __init__(self, **kwargs):
        ...
        logger_manager = GWASLambdaLogger()
    ...
    def find_result_files(self) -> List[str]:
        ...
        if not files:
            self.logger.error(f"未找到匹配文件|No files found matching pattern: {self.config.search_pattern}")
            sys.exit(1)

        self.logger.info(f"找到 {len(files)} 个文件，开始分析|Found {len(files)} files, starting analysis")
        return files
    ...
    def analyze_single_file(self, file_path: str) -> Dict:
        ...
        if error_msg == "无显著位点|No significant variants found":
            ...
            if self.analyzer.should_highlight(result['status']):
                self.logger.warning(f"{folder_name}: Lambda={lambda_val:.4f}, SigHits={sig_count} ({result['status']})")
        else:
            result['status'] = f"Error: {error_msg}"
            result['lambda_display'] = 'NA'
            self.logger.error(f"{folder_name}: Error - {error_msg}")
    ...
    def write_results(self):
        ...
        self.logger.info(f"结果已保存至|Results saved to: {output_path}")
    ...
    def print_summary(self):
        ...
        self.logger.info(f"分析总结|Analysis Summary:")
        self.logger.info(f"   总文件数|Total files: {total_files}")
        self.logger.info(f"   无显著信号|No signals: {no_signal_count}")
        self.logger.info(f"   理想结果|Ideal: {ideal_count}")
        self.logger.info(f"   可接受结果|Acceptable: {acceptable_count}")
        self.logger.info(f"   膨胀结果|Inflated: {inflated_count}")
        self.logger.info(f"   压缩结果|Deflated: {deflated_count}")
        self.logger.info(f"   错误|Errors: {error_count}")

        self.logger.info(f"结果说明|Result Interpretation:")
        self.logger.info(f"1. Lambda_GC 显示 NA 且 Status 为 'No Signals': 说明该性状没有检测到 P<1e-5 的位点，无需关注。|Lambda_GC shows NA and Status is 'No Signals': No variants with P<1e-5 detected for this trait, no attention needed.")
        self.logger.info(f"2. 关注 Status 为 'Ideal' 或 'Acceptable' 且 Sig_SNPs 数量合理的性状。|Focus on traits with Status 'Ideal' or 'Acceptable' and reasonable Sig_SNPs counts.")
        self.logger.info(f"3. 'Inflated' 状态表明可能存在群体分层或假阳性。|'Inflated' status indicates possible population stratification or false positives.")
...
示例|Examples:
  python -m biopytools.gwas_lambda
    ...
    parser.add_argument('--p-column', '-c',
                       type=int,
                       default=3,
                       help='P值所在列索引|P-value column index (0-based)')
