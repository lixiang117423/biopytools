        ...
        try:
            ...
            with open(input_file, 'w') as f:
                for seq1_id, seq2_id, pair_name in pairs:
                    ...
                    # 写入AXT格式: 名称 + 序列1 + 序列2 + 空行|Write AXT format: name + seq1 + seq2 + blank
                    f.write(f"{pair_name}\n")
                    f.write(f"{seq1}\n")
                    f.write(f"{seq2}\n")
                    f.write("\n")
            ...
            if mismatch_count > 0:
                self.logger.warning(f"序列长度不一致|Sequence length mismatch: {mismatch_count}/{len(pairs)} 个配对|pairs (已自动截断|auto-truncated)")

            file_size = os.path.getsize(input_file)
            self.logger.debug(f"AXT输入文件大小|AXT input file size: {file_size} bytes")
