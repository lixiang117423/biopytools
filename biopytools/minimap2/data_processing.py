...
class PAFProcessor:
    ...
    def find_unmapped_regions(self, df_filtered):
        ...
        self.logger.info(f"找到 {len(unmapped_regions)} 个未比对区间（长度 >= {self.config.min_unmapped_length}bp）|Found {len(unmapped_regions)} unmapped regions (length >= {self.config.min_unmapped_length}bp)")
