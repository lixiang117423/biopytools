...
### Docs
- `fastp`: 文档补齐 v1.72.1 行为变化——功能概述与流程图改为「逐样品」模拟数据检测口径;依赖一节更新 conda 调用形态(同源 conda 绝对路径 + `run -p <环境前缀>`,见规范 §13);FAQ Q2 细化判定细节(前 100 条 reads 质量字符种类 ≤3 判为模拟,日志逐个列出模拟样品名单、其余样品保持 `-q` 原值);**新增 FAQ Q7** 说明「有样品失败即以退出码 1 结束」是刻意行为(防上游误判成功写检查点)及失败样品续跑方法
...
### Added
- `rna_iso`(模块 1.1.0): `-i/--reads` 支持**目录输入**——自动按文件名排序展开目录内 fasta/fastq(±gz),忽略子目录(如 fastp_reports),目录内无 reads 文件时报错提示;CLI help/参数表/输入文档同步更新;67 mock 单测(新增目录输入 4 例)
...
### Changed
- `mixrace`(报告层 v0.4): HTML 报告升级为自包含交互报告——页头判读构成条+四张 verdict 计数卡(一眼看清批次构成);判读汇总表横滚容器+粘性表头/首列+data-v 原始值排序(点击/键盘均可用,数值列按原始值不受显示格式影响);verdict 徽章 CVD 安全(色点+文字双编码,不单独依赖颜色,与图配色一致);图版全部 9 张图按阅读顺序排列(全景→定位→关系→机理),每张带中文标题与一句话图注(_FIGURE_META);逐样品证据链可搜索过滤(汇总表同步)、一键展开/收起、打印时自动展开且隐藏工具条;完整文档骨架(charset/viewport/打印样式/零外链零依赖,base64 内嵌图片);docs 新增 mixrace_report.html 结构说明

## [1.62.1] - 2026-09-01

### Fixed
- `fastp`(模块 1.1.1): fastp worker 线程数钳制到上限 16——实测 fastp 1.x `-w>16` 会进入死态(95 线程互卡、几乎不占 CPU、零输出、永不结束),现 `-t` 超出 16 自动钳制并打印 WARNING,单末端/双末端命令均生效;docs 线程说明与 FAQ 更新
...
### Added
- `phy2fa`: 新增 Phylip→FASTA 转换模块——极简解析:跳过首行头(`n_tax n_char`),每行按空白拆分取首字段为样本名、其余拼接为序列,同名自动追加(兼容 interleaved 分块);转换后按头行校验样本数与序列长度(不符报错);FASTA 换行宽度可调(`--line-width`,默认 60);支持 .gz 输入;断点续传输出存在即跳过;`software_versions.yml` 记录样本数/序列长度/耗时;模块版本 1.0.0,新增 11 个单元测试(tests/test_phy2fa)
...
### Added
- `check_reads`: 新增 fastq 完整性检查模块——gz 压缩完整性(流式解压校验,多线程并行)、0 字节文件、R1-R2 配对完整性(`_1/_2`、`_R1/_R2`、`_1.clean.fq.gz` 等命名,复用 eviann 配对规则);输入支持逗号分隔多目录、递归扫描子目录,非 fastq 忽略;输出人类可读报告 `check_reads_report.txt`(逐文件状态+配对检查+汇总),有问题退出码 1(作业调度可据此判成败);模块版本 1.0.0,新增 14 个单元测试(tests/test_check_reads)
...
### Added
- `admixture`: `cluster_assignment.csv` 每个 K 新增 `K<n>_qgroup` 列——回答「哪些样品祖先构成相似」:对 Q 行(K 维祖先构成向量)做层次聚类(Euclidean 距离 + average linkage,确定性无随机种子,scipy 实现),切成至多 K 组,按组大小降序重编号(1=最大组,并列按首成员序号破平);与既有 `K<n>` 列(argmax 祖先群体归属)语义不同——杂交个体多的面板里 qgroup 可能把杂交个体单独成组;单样品守卫(无法聚类时全组 1);缺 K/无 Q/dry_run/行数不齐降级行为不变;模块版本 1.4.0,新增 3 个单元测试
...
### Fixed
- `mixrace`: fastq 整对剔除失败时清理已写出的半成品 nohost fastq(防残留误导下游/断点自愈误判); 版本 0.2.1
...
### Fixed
- `vcf2tree`: 转 FASTA 前自动移除「仅杂合变异位点」——此类位点对 IQ-TREE(简并码视为缺失)是恒定位点、零建树信息，且会让 IQ-TREE 3.x 的 +ASC 直接报错 "Invalid use of +ASC"（417 样本实例数据实测移除 26639/147693 位点）；分块转置实现避免整矩阵物化；版本 1.0.2
...
### Fixed
- `cim`：RF 计算口径改为**双纯合对**（含 H 样本对排除出分子分母）——旧"含 H 记 0.5"在 F2 群体（H 比例 ~50%）把完全连锁标记对的 RF 基线抬到 ~0.25–0.3，实测平均 RF `median=0.464, range=[0.382,0.601]`，`max_mean_rf=0.35` 直接"1104→0"全部误删；新口径下完全连锁对 RF=0、独立分离对 ~0.5（F2 模拟实测 0.52）
- `cim`：规则 2 由全染色体平均 RF 重构为 **K 近邻局部平均 RF**（`--rf-knn`，默认 10，物理位置前后各 5 个）——全染色体平均在正确口径下无区分力（远距离对 ~0.5 主导，正常标记全局均值也 ~0.45–0.5）；局部判据下坏标记（随机基因型）对近邻 RF~0.5 被删、好标记在 LD 邻域内 RF~0 保留（模拟实测：好标记 0.0000、坏标记 0.58，阈值 0.35 恰好只删坏标记）；实现用偏移法向量化，复杂度 O(n²)→O(n·k·samples)；热点过滤（Step 2.5a）的 `_calc_single_rf` 调用同步受益新口径
